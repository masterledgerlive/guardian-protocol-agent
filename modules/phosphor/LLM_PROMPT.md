...
- The clickable LIBRARY imprints `sample/pong.route`. PLAY must recall those bytes and step the `tick` lines. Do not hardcode a second pong match in the page. Doom-sized binaries stay on the large-file path; this player is the small route listing.
