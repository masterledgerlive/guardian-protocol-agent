...
- Every inject writes a **SYSTEM_INJECTED** receipt and an append-only `inject-log.jsonl` line only after the system recalls the bytes with the key. Click `/phosphor/receipt?c=` and `/phosphor/block?c=&i=` to read the exact machine data field. Block 0 `prev=GENESIS` and is the **leader**: first line carries `utc|local|unix|filing` so date/time/location are never lost. Trailing blocks do not repeat the stamp — follow-the-leader. One known path (block loc, filing, or directory final) surfaces every connected path. Find by time via the leader-time index. One-block and five-block recalls must match the original bytes. A wrong key does not piece the code together.
...
- The clickable LIBRARY imprints `sample/pong.route` and a tiled PPM. PLAY recalls the route and steps the `tick` lines. PICTURE recalls the PPM and draws it. Do not pad a file with comments to fake a smaller squash. The unwrap formula is `§PHOSDIR§v1|k=…|kind=data|n=…|L=stark://…+stark://…`. It must be shorter than one data field. The reader final loc is the filing loc of that line. Open key attached means no prompt. `LOCK` asks for `/phosphor key PREFIX PASSPHRASE`. A directory that does not fit one data field returns `{ok:false}` and does not fail the write. Wide chains stay on `blocks.ndjson`. Doom-sized binaries stay on the large-file path.
- `baseLocation` and `basescan` stay null. Do not invent a Basescan link. A real seal is a later 0-ETH self-tx whose input is the short directory UTF-8, and only the hash the node returns.
