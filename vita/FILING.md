...
| `vita/proven-player.js` | `PROVEN_PLAYER` | Own AV1 player · SVT-AV1 chunks · 448-byte SHA-256 receipt · follow-the-leader registry · dav1d after unlock |
...
| `vita/proven-player/ChunkBinding.circom` | `PROVEN_PLAYER` | Conceptual binding circuit. Not an AV1 encoder |
| `vita/proven-player/libvlc_access.rs` | `PROVEN_PLAYER` | Native access gate that refuses bytes until the receipt matches, then hands them to dav1d |
| `vita/proven-player/media/` | `PROVEN_PLAYER` | Three 1-second SVT-AV1 MP4 chunks plus source-digest sidecars |
...
| `vita/strands/proven-player.json` | `PROVEN_PLAYER` | Sparse strand: encode → receipt → unlock → dav1d |
...
| `PROVEN_PLAYER` | Proven Player (`§VITAPROVENPLAY§`) — own AV1 route; SHA-256 chunk receipt; Groth16 slot unwired; availability until a real loc |
