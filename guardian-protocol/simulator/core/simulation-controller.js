/**
 * SimulationController — Sprint 1 end-to-end preservation pipeline.
 */
...
import "../../strategies/fifo/index.js";
...
export class SimulationController {
  ...
  run(canonicalInput, { objectId = "obj-0001" } = {}) {
    const t0 = this.now();
    const canonical =
    ...
    const job = { objectId, byteLength: canonical.byteLength, sha256: canonical.sha256 };
    ...
    emit(tr.previous, tr.next, { reason_code: "INPUT_OK", cryptographic_references: { input_sha256: canonical.sha256 } });
    ...
    for (const chunk of encoded) {
      for (let r = 0; r < plan.redundancy; r++) {
        ...
        placements.push({ fragmentId, ...result, index: chunk.index });
      }
    }
    ...
    const recovered = [];
    for (const chunk of encoded) {
      const fragmentId = `${objectId}:${chunk.index}:r0`;
      let found = null;
      for (const node of this.storage.nodes) {
      ...
      if (!found) {
        ...
        throw new Error(`Missing fragment ${fragmentId}`);
      }
