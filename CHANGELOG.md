...
- Probe other V3 fees when catalog fee misses (wider fees before 100/500). Bind the fill to the DexScreener Uni V3 WETH pair. Never rank by gameable factory `liquidity()`. Sells bind to that pair only when it clears `$25k` (no freeze; thin V3 falls back to catalog/probe). DexScreener empty/outage **skips the buy** (`NO_DEX_PAIRS`) without freezing leftover sells. Preferred-pool bind fails closed on factory flake. Live fee costlier than gated RT% skips the buy (sell leftover still exits, hitch skipped). Ghost DexScreener mega-liq $0-vol prints (TOSHI Pancake VIRTUAL class) are not the primary book and cannot `PRIMARY_NOT_V3_WETH` freeze a real Uni V3 WETH pool. Quote timeout/revert does not walk the RPC pool; timeouts clear the timer.
