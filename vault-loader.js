...
// ── All vault-managed keys ────────────────────────────────────────────────────
// These are every key Guardian Protocol uses.
// For each one: if Railway env var exists → use it (primary).
// If missing but VAULT_<KEYNAME> tx hash exists → fetch from Base (fallback).
// Anyone deploying Guardian can migrate keys to blockchain one at a time.
const MANAGED_KEYS = [
  // ── Test key — always start here to verify the system works ───────────────
  "TEST_KEY",               // safe test — no real impact if fails

  // ── Telegram ──────────────────────────────────────────────────────────────
  "TELEGRAM_BOT_TOKEN",     // bot API token — migrate last (needed to send alerts)
  "TELEGRAM_CHAT_ID",       // your chat ID — not a secret but proves the flow

  // ── CDP / Coinbase Developer Platform ─────────────────────────────────────
  "CDP_API_KEY_ID",         // short UUID — safe to migrate early
  "CDP_API_KEY_SECRET",     // PEM key — high value, migrate after testing
  "CDP_WALLET_SECRET",      // wallet encryption key — highest value

  // ── GitHub ────────────────────────────────────────────────────────────────
  "GITHUB_TOKEN",           // state persistence token — moderate risk
  "GITHUB_REPO",            // repo name e.g. "david/guardian" — not a secret
  "GITHUB_BRANCH",          // branch name — not a secret
  "STATE_BRANCH",           // state branch name — not a secret
];
...
export function getVaultStatusMessage() {
  const groups = [
    { label: "🧪 Test",     keys: ["TEST_KEY"] },
    { label: "💬 Telegram", keys: ["TELEGRAM_CHAT_ID", "TELEGRAM_BOT_TOKEN"] },
    { label: "🔑 CDP",      keys: ["CDP_API_KEY_ID", "CDP_API_KEY_SECRET", "CDP_WALLET_SECRET"] },
    { label: "📦 GitHub",   keys: ["GITHUB_TOKEN", "GITHUB_REPO", "GITHUB_BRANCH", "STATE_BRANCH"] },
  ];

  let msg = "🔐 <b>VAULT STATUS</b>\n━━━━━━━━━━━━━━━━━━━━\n";
  msg += (process.env.DECRYPT_PASSWORD ? "🔑 Password: ✅ set\n" : "⚠️ Password: NOT SET — add DECRYPT_PASSWORD to Railway\n");
  msg += "\n";
  ...
  for (const group of groups) {
    ...
    for (const keyName of group.keys) {
      ...
      if (!v) {
        // Not yet checked at boot — check live
        const hasRailway = !!process.env[keyName];
        const hasTxHash  = !!process.env["VAULT_" + keyName];
        if (hasRailway)     msg += "✅ " + keyName + " — Railway\n";
        else if (hasTxHash) msg += "🔗 " + keyName + " — vault hash set\n";
        else                msg += "⚪ " + keyName + " — not set\n";
      } else if (v.source === "railway")    {
        msg += "✅ <b>" + keyName + "</b> — Railway\n";
      } else if (v.source === "blockchain") {
        msg += "🔗 <b>" + keyName + "</b> — Base blockchain\n";
        msg += "   <a href=\"https://basescan.org/tx/" + v.txHash + "\">↗ inscription</a>\n";
      } else if (v.source === "missing")    {
        msg += "⚪ " + keyName + " — not set\n";
      } else {
        msg += "❌ " + keyName + " — failed: " + (v.error || "") + "\n";
      }
    }
    msg += "\n";
  }
  ...
  msg += "━━━━━━━━━━━━━━━━━━━━\n";
  msg += "<i>Start: /newvault TEST_KEY\n";
  msg += "Then migrate one at a time.\n";
  msg += "Delete Railway var after vault confirmed ✅</i>";
  return msg;
}
