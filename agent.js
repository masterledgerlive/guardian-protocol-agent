...
const MIN_POS_USD       = 2.00;   // lowered from $3 — wallet at ~$2.68 was blocking all buys
...
const TRAIL_PCT            = 0.025;  // 2.5% drop from high water mark triggers trailing stop sell
...
function initWaveState(symbol) {
  if (!waveState[symbol]) waveState[symbol] = { peaks: [], troughs: [], peakScores: [], troughScores: [], highWater: null };
...
async function processToken(cdp, token, bal) {
  try {
    ...
    // ── FIX 4: TRAILING STOP — track high water mark per token ────────────
    // Updates every loop when price rises. Sell if price drops TRAIL_PCT from
    // that peak AND we're in profit. Rides new highs without selling early,
    // captures profit on the way back down.
    const ws = waveState[token.symbol] || {};
    if (balance > 1 && price > 0) {
      if (!ws.highWater || price > ws.highWater) ws.highWater = price;
    }
    const highWater = ws.highWater;
    const trailingStopHit = entry && highWater
      && price < highWater * (1 - TRAIL_PCT)
      && netIfSellNow > breakEvenBuffer;

    // ── FIX 3: PHANTOM ENTRY — SEAM-style: holding but no entry recorded ──
    // If we hold tokens, have no entry price, AND price is at the max peak,
    // sell anyway using the legacy netIfSellNow=1 bypass (already built in above).
    // atMaxPeak covers this — netIfSellNow defaults to 1 when entry is null.

    const shouldSell = (atMaxPeak || trailingStopHit) && sellable > 1 && netIfSellNow > breakEvenBuffer;

    const atMinTrough  = minTrgh && price <= minTrgh * 1.005;
    const indConfirmed = ind.score >= 1;

    // ── FIX 4: CASCADE DEEPER — buy at each new lower low ─────────────────
    // If price breaks below the current MIN trough, that IS a new trough.
    // We want to buy more (DCA down), not wait. This lowers average cost so
    // even a partial recovery puts us in profit.
    // New lower low = price is MORE than WAVE_MIN_MOVE below current minTrgh
    const newLowerLow  = minTrgh && price < minTrgh * (1 - WAVE_MIN_MOVE) && price > 0;
    const atBuyZone    = atMinTrough || newLowerLow;

    // Allow buy if: at trough or new lower low, armed, have funds, cooldown clear
    // Allow re-buy even if holding — riding a wave doesn't mean we can't add at next trough
    // Block buy only if we already have a LARGE position (invested > 60% of tradeable)
    const positionSizeEth = (token.totalInvestedEth || 0);
    const atMaxPosition   = positionSizeEth > bal.tradeableWithWeth * 0.60;
    const shouldBuy    = atBuyZone && arm.armed && !atMaxPosition && !shouldSell
    ...
    const zone = shouldSell && trailingStopHit ? "🔴 TRAILING STOP — SELLING" :
                 shouldSell     ? "🔴 AT MAX PEAK — SELLING" :
                 stopLossHit    ? "🛑 STOP LOSS" :
                 shouldBuy && newLowerLow ? `🟢 NEW LOWER LOW — BUYING (DCA)` :
    ...
    if (highWater && hasPosition) {
      const trailDist = ((highWater - price) / highWater * 100).toFixed(2);
      const trailTrigger = (TRAIL_PCT * 100).toFixed(1);
      lines.push(`  │ 🌊 HW:$${highWater.toFixed(8)} | ${trailDist}% from trail stop (trigger: ${trailTrigger}%)`);
    }
    ...
    if (shouldSell) {
      ...
      const sellReason = trailingStopHit
        ? `🎯 TRAILING STOP HIT — ${(TRAIL_PCT*100).toFixed(1)}% drop from $${highWater?.toFixed(8)}`
        : `[🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩] 100% — MAX PEAK HIT!`;
      await tg(
