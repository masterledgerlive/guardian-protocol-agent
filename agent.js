...
const DEFAULT_TOKENS = [
  ...
  { symbol: "BALD",    address: "0x27D2DECb4bFC9C76F0309b8E88dec3a601Fe25a8", feeTier: 10000, poolFeePct: 0.010, minNetMargin: 0.010,
    score: { liquidity:4, waveQuality:5, fundamentals:4, coinbaseFit:5, community:6, total:24 },
    notes: "BALD - original Base meme. Pool liquidity thin — demoted to SCOUT tier. May not have tradeable V3 pool." },
...
async function loadHistoricalData(days = 90) {
  ...
  for (const token of allTokens) {
    ...
    try {
      // ── Try GeckoTerminal + DexScreener in PARALLEL per token ─────────────
      // Sequential was hitting GT rate limits on token #3+. Running both at once
      // means we always get whichever responds first, and both count against the
      // same rate-limit window instead of doubling our sequential request count.
      let candles = null;
    ...
    // 1.2s between tokens. GT rate limit is ~30 req/min = 2s/req minimum for the
    // pool-lookup step. With parallel GT+DS we fire 2 requests per token, so 1.2s
    // gives ~50 req/min — just under the free tier ceiling.
    await sleep(1200);
    // Every 8 tokens take a longer breath so the rate-limit window resets
    if ((loaded + skipped) % 8 === 0) await sleep(2000);
  }
...
async function main() {
  ...
  const BATCH2_SYMBOLS = ["ZORA","BNKR","TYBG","MIGGLES","BALD","NORMIE","BENJI","ROOST","TALENT"];
