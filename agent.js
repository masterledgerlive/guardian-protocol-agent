...
async function processToken(cdp, token, ethBalance) {
  try {
    ...
    if (sellable > 1 && entry) {
      ...
      // ── RACEHORSE PROGRESS ────────────────────────────────────────────
      // How far is price between entry and sell target? (0% = just bought, 100% = at target)
      const raceTarget    = sellTarget || avgPeak;
      const raceRange     = raceTarget ? raceTarget - entry : 0;
      const raceProgress  = raceRange > 0 ? Math.min(((price - entry) / raceRange) * 100, 100) : 0;
      const halfSellPrice = token.halfSellPrice || null; // floor set after first half sell

      // Progress bar for logs
      const barFill  = Math.max(0, Math.min(10, Math.floor(raceProgress / 10)));
      const raceBar  = "🟩".repeat(barFill) + "⬜".repeat(10 - barFill);
      if (raceRange > 0) {
        console.log(`  🏇 ${token.symbol} race: [${raceBar}] ${raceProgress.toFixed(1)}% → $${raceTarget.toFixed(8)}`);
      }

      // ── HALF CASCADE CHECK ────────────────────────────────────────────
      // Conditions: profit gate met + better opportunity elsewhere + riding half still viable
      ...
      const betterOpExists  = bestScore > myScore + 1; // meaningfully better elsewhere
      const alreadyHalfSold = token.halfSoldThisCycle || false;

      // Half cascade: price is in profit, better opportunity exists, not already half-sold this ride,
      // other token broke 2+ wave lows (already in uptrend), riding half still covers fees
      const halfCascadeReady = (
        !alreadyHalfSold &&
        raceProgress >= 20 &&                          // at least 20% of the way to target
        price > entry * 1.005 &&                       // at least 0.5% above entry (in profit)
        betterOpExists &&                              // better cascade target exists
        bestCascade && countWaveLowsBrokenAbove(bestCascade.symbol, bestCascadePrice) >= 2 &&
        sellable >= 4                                  // enough tokens to split
      );

      if (halfCascadeReady) {
        // Verify riding half will still be profitable when it eventually sells
        ...
        if (ridingGate.ok) {
          console.log(`  🏇 HALF CASCADE: ${token.symbol} → ${bestCascade.symbol} (race: ${raceProgress.toFixed(0)}%, score: ${bestScore.toFixed(1)} vs ${myScore.toFixed(1)})`);
          const proceeds = await executeSell(cdp, token, 0.5, `🏇 HALF CASCADE at ${raceProgress.toFixed(0)}% race progress → ${bestCascade.symbol}`, price, false);
          if (proceeds > 0) {
            token.halfSoldThisCycle = true;
            token.halfSellPrice     = price; // floor for early exit of riding half
      ...
      // ── EARLY EXIT OF RIDING HALF ─────────────────────────────────────
      // If we already half-sold, watch for any exit signal on the riding portion
      if (halfSellPrice) {
        let earlyExit = false, earlyReason = "";

        // Signal A: momentum died — 2min stall with no new high
        if (nearSell && sellConfirmations[token.symbol]) {
          const elapsed = Date.now() - sellConfirmations[token.symbol].firstSeenMs;
          if (elapsed >= SELL_CONFIRM_MS && price <= sellConfirmations[token.symbol].highSeen) {
            earlyExit = true; earlyReason = "riding half — 2min stall at top";
          }
        }

        // Signal B: price dropped back below the half-sell price floor
        if (!earlyExit && price < halfSellPrice * 0.998) {
          earlyExit = true; earlyReason = `riding half — dropped below floor $${halfSellPrice.toFixed(8)}`;
        }

        // Signal C: a deep buy zone appeared (score spike from another token)
        if (!earlyExit && bestScore > myScore + 3 && price > entry * 1.003) {
          earlyExit = true; earlyReason = `riding half — deep buy opportunity in ${bestCascade?.symbol}`;
        }

        if (earlyExit) {
          console.log(`  🏇 EARLY EXIT riding half: ${token.symbol} — ${earlyReason}`);
          token.halfSoldThisCycle = false;
          token.halfSellPrice     = null;
          delete sellConfirmations[token.symbol];
          const proceeds = await executeSell(cdp, token, SELL_ALL_PCT, `🏇 EARLY EXIT: ${earlyReason}`, price, false);
          if (proceeds > 0) await triggerCascade(cdp, token.symbol, proceeds, ethBalance);
          return;
        }
      }

      // ── WAVE DYING SIGNALS (full exit) ────────────────────────────────
      ...
      // Signal 2: price drops back below 3rd peak level
      if (!waveDying && peaks.length >= 3 && price < peaks[TOP_EXIT_RANK] && price > entry * 1.01) {
        waveDying = true;
        waveDyingReason = `broke below 3rd peak $${peaks[TOP_EXIT_RANK].toFixed(8)}`;
      }

      // Signal 3: near sell target — 2min settle, no new high = sell
      if (nearSell) {
        if (!sellConfirmations[token.symbol]) {
          ...
          await sendAlert(
            ...
            `💲 $${price.toFixed(8)}\n🎯 $${raceTarget?.toFixed(8)||"?"}\n` +
            `🏇 Race: ${raceProgress.toFixed(1)}%\n⏳ No new high in 2min → FULL SELL`
          );
        } else {
          ...
          if (price > sc.highSeen) {
            sc.highSeen = price;
            console.log(`  ⬆️  ${token.symbol} new high $${price.toFixed(8)} — trailing`);
          } else if (elapsed >= SELL_CONFIRM_MS) {
            waveDying = true;
            waveDyingReason = "2min at top, no new high";
          } else {
            console.log(`  ⏳ ${token.symbol} settle: ${Math.ceil((SELL_CONFIRM_MS-elapsed)/1000)}s | race: ${raceProgress.toFixed(0)}%`);
          }
        }
      } else {
