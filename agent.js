...
async function processSurfers(cdp, bal, ethUsd) {
  ...
  for (const [name, s] of Object.entries(surfers)) {
    ...
    if (s.status === "WAITING") {
      // Enter when at min trough + armed + good indicator score
      const atTrough = minT && price <= minT * 1.02; // within 2% of confirmed trough
      const goodEntry = arm.armed && arm.net >= 0.02 && ind.score >= 40 && atTrough;
      if (goodEntry) {
        s.entryPrice = price;
      ...
      if (atPeak || (waveLost && !kahuna?.active) || stopLoss) {
        ...
        s.totalOut += netEth;
        s.lastWave = s.symbol;
        s.status   = pnl > 0 ? "RESTING" : "WAITING"; // rest after a win, immediately look after a loss
        s.entryPrice = null;
        s.entryTime  = null;
