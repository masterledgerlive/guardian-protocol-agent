...
const DRAWDOWN_HALT_PCT    = 0.20;   // halt new buys if portfolio down 20% from peak
...
function updatePortfolioPeak(currentUsd) {
  if (currentUsd > portfolioPeakUsd) {
    ...
    if (drawdownHaltActive) {
      const recovery = (portfolioPeakUsd - currentUsd) / portfolioPeakUsd;
      if (recovery < DRAWDOWN_RESUME_PCT) {
        drawdownHaltActive = false;
        console.log(`✅ DRAWDOWN HALT LIFTED — portfolio recovered`);
      }
    }
  }
}
...
function buildRaceDisplay(token, price, balance, ethUsd) {
  ...
  return {
    ...
    lines: [
      ...
      `💓 Indicators: ${ind.detail || "building..."}`,
...
async function githubSave(path, content, sha, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      const body = {
        message: `Guardian v13 ${new Date().toISOString()}`,
...
async function loadFromGitHub() {
  ...
  if (pf?.content) {
    ...
    portfolioPeakUsd = pos.portfolioPeakUsd || 0;
    for (const t of tokens) {
...
async function saveToGitHub() {
  try {
    ...
    positionsSha = await githubSave("positions.json", {
      lastSaved: new Date().toISOString(), piggyBank, totalSkimmed, tradeCount, portfolioPeakUsd,
...
async function sendFullReport(bal, ethUsd, title) {
  try {
    ...
    for (const t of tokens) {
      ...
      const tbal  = await getTokenBalance(t.address);
    ...
    for (const upd of data.result) {
      ...
      if (text.startsWith("/buy ")) {
        ...
        await tg(
          ...
          `<b>Commands:</b>\n` +
          `/status /waves /race /positions /eth /piggy\n` +
          `/trades /drawdown /gas /indicators\n` +
...
async function main() {
  ...
  let cachedBal    = balInit;
  let cachedEthUsd = ethUsdInit;
  ...
  while (true) {
    try {
      ...
      for (const t of tokens) {
        if (t.entryPrice) {
          const p = history[t.symbol]?.lastPrice;
          const b = p ? await getTokenBalance(t.address) : 0;
