...
function btpNextChunk(tradeLabel) {
  ...
  return { full, seq, tot, name: msg.name, isComplete, isDefault: btpDefaultLoaded && msg.name === "VITA" };
}

// How many chunks are pending across all queued messages (excluding default auto-fill)
function btpPendingCount() {
  return btpQueue
    .filter(m => m.name !== "VITA" || !btpDefaultLoaded)
...
async function btpInscribe(cdp, tradeLabel) {
  try {
    ...
    // ── Read receipts ────────────────────────────────────────────────────────
    if (!isDefault) {
      // Chunk-by-chunk receipt for custom messages
      await tg(
    ...
    if (isComplete && isDefault) {
      // Silently reload default after it completes — keeps every trade inscribed
      btpDefaultLoaded = false;
      btpEnsureDefault();
    }
  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      // ── 📡 BLOCKCHAIN TELEGRAM PROTOCOL COMMANDS ─────────────────────────────
        ...
        if (btpQueue.length === 0 || (btpQueue.length === 1 && btpQueue[0].name === "VITA")) {
          await tg(
        ...
        await tg(
          ...
          `/transmit [msg] — 📡 send message onto blockchain via trades\n` +
