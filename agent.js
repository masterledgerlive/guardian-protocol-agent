...
// ═══════════════════════════════════════════════════════════════════════
// ⚔️  GUARDIAN CASCADE SYSTEM v11.3 — EXECUTION EDITION
// ═══════════════════════════════════════════════════════════════════════
...
async function profitGate(proceedsEth, entryEth, isProtective) {
  ...
  if (proceedsEth * ETH_USD < 1.00) return { ok: false, reason: `Too small: $${(proceedsEth*ETH_USD).toFixed(2)}` };
  if (!isProtective && entryEth > 0) {
    const required = entryEth + (gasCost * GAS_PROFIT_MULT) + (proceedsEth * PIGGY_SKIM_PCT);
    if (proceedsEth < required) {
      return { ok: false, reason: `Short by $${((required-proceedsEth)*ETH_USD).toFixed(3)} (entry+gas×${GAS_PROFIT_MULT}+piggy)` };
    }
  }
  return { ok: true };
}
...
async function processToken(cdp, token, ethBal) {
  try {
    ...
    // ── DETERMINE ZONE ─────────────────────────────────────────────────
    const nearSell = validSellTarget && isNearPrice(price, sellTarget, 0.015);  // within 1.5% of target
    const atPeak   = validSellTarget && price >= sellTarget * 0.98;             // within 2% of target
    const nearBuy  = buyTarget && isNearPrice(price, buyTarget, 0.015);         // within 1.5% of buy target
    const sellZone = (nearSell || atPeak || slowing) && entry && price > entry * 1.003 && sellable > 1;
    const buyZone  = nearBuy && !entry && totalAvail >= MIN_ETH_TRADE;

    const zone = sellZone ? "🔴 SELL ZONE" : buyZone ? "🟢 BUY ZONE" : entry ? "🏇 RIDING" : "⬜ WATCHING";
    ...
    if (sellZone) {
      ...
      // Stop loss (protective, bypasses gate)
      ...
      // Half cascade opportunity
      const bestTarget      = await findCascadeTarget(token.symbol);
      const bestTargetPrice = bestTarget ? history[bestTarget.symbol]?.lastPrice : null;
      const bestScore       = bestTarget && bestTargetPrice ? getCascadeScore(bestTarget.symbol, bestTargetPrice) : 0;
      const myScore         = getCascadeScore(token.symbol, price);
      const halfReady       = (
        !token.halfSoldThisCycle &&
        price > entry * 1.005 &&
        bestScore > myScore + 2 &&
        bestTarget && bestTargetPrice &&
        countWaveLowsBroken(bestTarget.symbol, bestTargetPrice) >= 2 &&
        sellable >= 4 &&
        (Date.now() - (token.halfCascadeTime || 0)) > HALF_CASCADE_COOLDOWN
      );

      if (halfReady) {
        const halfProcEth = (sellable * 0.5 * price) / ETH_USD;
        const halfEntryEth = (token.totalInvestedEth || 0) * 0.5;
        const gate = await profitGate(halfProcEth, halfEntryEth, false);
        if (gate.ok) {
          console.log(`  🏇 HALF CASCADE → ${bestTarget.symbol}`);
          await sendAlert(
            `🏇 <b>HALF CASCADE: ${token.symbol} → ${bestTarget.symbol}</b>\n\n` +
            (rd?.lines || `💲 $${price.toFixed(8)}`) + "\n\n" +
            `Score: ${bestScore.toFixed(1)} vs ${myScore.toFixed(1)} | Selling 50%\n` +
            `Other half keeps riding toward target`
          );
          const p = await executeSell(cdp, token, 0.5, `🏇 HALF CASCADE → ${bestTarget.symbol}`, price, false);
          if (p > 0) {
            token.halfSoldThisCycle = true;
            token.halfCascadeTime   = Date.now();
            await triggerCascade(cdp, token.symbol, p, ethBal);
          }
          return;
        }
      }
      ...
      // ── SELL COUNTDOWN ─────────────────────────────────────────────
      const st = sellTimers[token.symbol];
      let reason = atPeak ? `at avg peak $${sellTarget?.toFixed(8)}` :
                   slowing ? `momentum slowing at $${price.toFixed(8)}` :
                   `near sell target $${sellTarget?.toFixed(8)}`;

      if (!st) {
        // Start sell timer
        sellTimers[token.symbol] = { startMs: Date.now(), highSeen: price };
        console.log(`  ⏱️  ${token.symbol} SELL TIMER STARTED — ${reason}`);
        await sendAlert(
          `🏇 <b>${token.symbol} SELL ZONE — 2min countdown starting</b>\n\n` +
          (rd?.lines || `💲 $${price.toFixed(8)}`) + "\n\n" +
          `📊 ${reason}\n` +
          `📱 /sell ${token.symbol} to sell IMMEDIATELY`
        );
        // Launch countdown on independent loop
        runSellCountdown(cdp, token, price, reason, rd?.lines).catch(e =>
          console.log(`  ⚠️ Sell countdown error (${token.symbol}): ${e.message}`)
        );
      } else {
        // Timer already running — update high if new
        if (price > st.highSeen) {
          st.highSeen = price;
          console.log(`  ⬆️  ${token.symbol} new high in sell zone $${price.toFixed(8)}`);
        }
        // Countdown loop handles execution — just log status
        const elapsed = (Date.now() - st.startMs) / 1000;
        console.log(`  ⏳ ${token.symbol} sell countdown running ${elapsed.toFixed(0)}s | $${price.toFixed(8)}`);
      }
...
async function main() {
  console.log("═══════════════════════════════════════════════════════");
  console.log("⚔️   GUARDIAN CASCADE SYSTEM v11.3 — EXECUTION EDITION");
  console.log("═══════════════════════════════════════════════════════\n");
  ...
  await sendAlert(
    `⚔️ <b>GUARDIAN v11.3 — EXECUTION EDITION ONLINE</b>\n\n` +
