...
// Timeout wrapper — kills any hung promise after ms milliseconds
...
let lastHeartbeat  = Date.now(); // watchdog — updated every cycle
...
https://api.github.com/repos/${GITHUB\_REPO}/contents/${path}?ref=${GITHUB\_BRANCH}&t=${Date.now()},
...
if (sha) body.sha = sha;
const res = await fetch(
https://api.github.com/repos/${GITHUB\_REPO}/contents/${path},
{ method: “PUT”,
...
// ── SELL LOGIC ──────────────────────────────────────────────────────
// Skip sell path if token has no tracked position and dust value is tiny
// This allows seed buy to happen even if there are old dust tokens
...
// ── BUY LOGIC ────────────────────────────────────────────────────────
  ...
  if (inBuyZone) {
    ...
    // $3 seed entry if no tracked position
    // Note: token may have dust from old trades — we seed anyway if entryPrice is null
    if (!token.entryPrice && token.status === "active") {
      ...
      if (totalAvail >= minEntryEth) {
        ...
        console.log(`  ⚠️  ${token.symbol} needs seed but only ${(totalAvail*ETH_USD).toFixed(2)} available`);
      }
    }
  }
}
...
// Watchdog — if main loop freezes for 3+ min, force exit so Railway restarts
(async function watchdog() {
await sleep(60000); // give bot 60s to start up
while (true) {
await sleep(30000);
const elapsed = Date.now() - lastHeartbeat;
if (elapsed > 180000) { // 3 minutes
console.log(💀 WATCHDOG: no heartbeat for ${Math.floor(elapsed/1000)}s — forcing restart);
await sendAlert(“⚠️ GUARDIAN watchdog triggered — bot was frozen, auto-restarting now”);
process.exit(1); // Railway auto-restarts on exit
}
}
})();
...
while (true) {
try {
lastHeartbeat = Date.now(); // heartbeat — proves loop is alive
