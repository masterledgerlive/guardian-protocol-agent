  ...
  for (const [name, s] of Object.entries(surfers)) {
    ...
    if (s.status === "WAITING") {
      ...
      if (atPeak || (waveLost && !kahuna?.active) || stopLoss) {
        ...
        const feeEth     = grossEth * (tokens.find(t=>t.symbol===s.symbol)?.poolFeePct || 0.006) * 2;
        const netEth     = grossEth - feeEth - gasCost;
        const pnl        = (netEth - s.ethAlloc) * ethUsd;

        s.pnlEth   += (netEth - s.ethAlloc);
        s.pnlUsd   += pnl;
        s.totalOut += netEth;
        s.lastWave  = s.symbol;
        // Surfer's capital accumulates — gains roll into next wave allocation
        // This is the "money follows the surfer" mechanic
        s.ethAlloc = netEth; // ride-proceeds become next wave's capital
        if (s.ethAlloc < 0.0001) { // nearly wiped — retire
          s.status = "RETIRED";
          console.log(`🏄 [${name}] Wiped out — retiring. Final P&L: ${s.pnlUsd>=0?"+$":"-$"}${Math.abs(s.pnlUsd).toFixed(3)}`);
        } else {
          s.status   = pnl > 0 ? "RESTING" : "WAITING";
        }
        s.entryPrice = null;
        s.entryTime  = null;

        console.log(`🏄 [${name}] EXITING ${s.symbol} ${exitReason} | P&L: ${pnl>=0?"+":""}$${pnl.toFixed(3)}`);
        await tg(`🏄 <b>SURFER ${name} — WAVE COMPLETE</b>\n` +
          `${exitReason}\n` +
          `Token: <b>${s.symbol}</b> | Entry: $${s.entryPrice?.toFixed(8) || "?"}\n` +
          `Exit: $${price.toFixed(8)} | Move: ${(pct*100)>=0?"+":""}${(pct*100).toFixed(2)}%\n` +
          `P&L this wave: ${pnl>=0?"🟢 +$":"🔴 -$"}${Math.abs(pnl).toFixed(3)}\n` +
          `Lifetime: ${s.pnlUsd>=0?"+$":"-$"}${Math.abs(s.pnlUsd).toFixed(3)} over ${s.trades} waves`);

        // Score this wave against prediction model
        if (s.waveEntry) scoreCompletedWave(s.symbol, Math.min(s.entryPrice || price, price), Math.max(s.entryPrice || price, price));

        // Capital STAYS with surfer and compounds across waves
        const grossEth   = s.ethAlloc * (1 + pct);
