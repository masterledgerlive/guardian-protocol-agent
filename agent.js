...
const WATCHLIST = [
  ...
  },
  {
    ...
    stars:    3,
    status:   "thinking",
    reason:   "DeFi lending protocol on Base. Interesting fundamentals but watching fee structure and liquidity depth before committing capital.",
    entryPlan:"Wait for 4P/4T wave pattern to form. Min $50k liquidity confirmed.",
    addedDate:"2026-03-12",
    redFlags: ["Newer protocol, less battle-tested"],
    greenFlags:["Real yield", "Base native", "Growing TVL"],
  },
  {
    symbol:   "CBBTC",
    ...
    reason:   "Coinbase wrapped BTC on Base. High trust, Coinbase backing. Too expensive per token for current wallet size — wait until ETH balance grows to $200+ tradeable.",
    entryPlan:"Enter when tradeable ETH > $200. High conviction hold candidate.",
    addedDate:"2026-03-12",
    redFlags: ["Price per token high — needs larger capital"],
    greenFlags:["Coinbase issued", "BTC exposure on Base", "Maximum trust", "Will appreciate with BTC"],
  },
];
...
// Fetches OHLC candle data from GeckoTerminal for a token on Base
// and returns an array of close prices oldest → newest
async function fetchHistoricalCandles(tokenAddress, days = 90) {
  try {
    // GeckoTerminal free API — daily candles for Base chain token
    const network = "base";
    const url = `https://api.geckoterminal.com/api/v2/networks/${network}/tokens/${tokenAddress}/ohlcv/day?limit=${Math.min(days, 365)}&currency=usd`;
    ...
    // Return array of closes, oldest first
    return candles.reverse().map(c => c[4]).filter(p => p > 0);
  } catch (e) {
    return null;
  }
}
...
// This replaces the 2-4 day "learning period" with instant knowledge from real history
async function loadHistoricalData(days = 90) {
  console.log(`📅 Loading ${days}-day historical candle data for all tokens...`);
  ...
  for (const token of DEFAULT_TOKENS) {
    try {
      ...
      if (!candles || candles.length < 10) {
        console.log(`   ⚠️  ${token.symbol}: no candle data available — will learn live`);
        skipped++;
        continue;
      }

      // Seed the history readings with candle closes so indicators work
      if (!history[token.symbol]) history[token.symbol] = { readings: [], lastPrice: null };
      // Only seed if we have fewer live readings than candles (avoid overwriting fresh data)
      if (history[token.symbol].readings.length < candles.length) {
        const now = Date.now();
        const dayMs = 86_400_000;
        // Inject candles as spaced readings going back in time
        const syntheticReadings = candles.map((price, i) => ({
          price,
          time: now - (candles.length - i) * dayMs,
          synthetic: true,
        }));
        // Merge: keep any live readings, prepend synthetic ones
        ...
        history[token.symbol].lastPrice = candles[candles.length - 1];
      }

      // Store candle summary for /history command
      if (!history[token.symbol].candles) history[token.symbol].candles = {};
      const highs  = candles.map((_, i) => i); // placeholder — we'll compute properly
      const prices = candles;
      const period7  = prices.slice(-7);
      const period30 = prices.slice(-30);
      const period90 = prices.slice(-90);
      history[token.symbol].candles = {
        days90: { high: Math.max(...prices),   low: Math.min(...prices),   start: prices[0],              end: prices[prices.length-1] },
        days30: { high: Math.max(...period30), low: Math.min(...period30), start: period30[0],            end: period30[period30.length-1] },
        days7:  { high: Math.max(...period7),  low: Math.min(...period7),  start: period7[0],             end: period7[period7.length-1] },
        count:  candles.length,
        updatedAt: new Date().toISOString(),
      };

      loaded++;
      const c = history[token.symbol].candles;
      console.log(`   ✅ ${token.symbol}: ${candles.length} days | 90d H:$${c.days90.high.toFixed(6)} L:$${c.days90.low.toFixed(6)} | 7d H:$${c.days7.high.toFixed(6)} L:$${c.days7.low.toFixed(6)}`);
    } catch (e) {
      console.log(`   ⚠️  ${token.symbol}: candle load failed — ${e.message}`);
      skipped++;
    }
    await sleep(300); // gentle rate limiting — don't hammer API
  }
...
async function processToken(cdp, token, bal) {
  try {
    ...
    const shouldSell   = atMaxPeak && sellable > 1;  // sell at MAX peak regardless of entry — entry only needed for P&L display
    ...
    console.log(`  │ $${price.toFixed(8)} | 🪙 ${balance>=1?Math.floor(balance):balance.toFixed(4)} ($${(balance*price).toFixed(2)}) | ETH=$${ethUsd.toFixed(0)}`);
    console.log(`  │ MAX:$${maxPeak?.toFixed(8)||"?"}(${pctToSell}%) MIN:$${minTrgh?.toFixed(8)||"?"}(+${pctToBuy}%) P:${peakCnt} T:${trghCnt}`);
    console.log(`  │ 💓 ${ind.detail || "building indicators..."}`);
    if (arm.armed) console.log(`  │ ✅ ARMED ${(arm.net*100).toFixed(2)}% [${arm.priority}] (w/ price impact)`);
    else           console.log(`  │ ⏳ ${arm.reason}`);  // console only — no esc needed
    if (rd) console.log(`  │ 🏇 [${rd.bar}] ${rd.racePct}% | P&L: ${rd.pnlSign}$${rd.pnlUsd}`);
    if (drawdownHaltActive) console.log(`  │ 🛑 DRAWDOWN HALT ACTIVE — buys suspended`);
    console.log(`  └──────────────────────────────────────────────`);
      ...
      if (text.startsWith("/buy ")) {
        ...
        if (targets.length === 0) {
          ...
          if (!c) {
            ...
            const pctFromHigh90 = ((currentPrice - c.days90.high) / c.days90.high * 100).toFixed(1);
            const pctFromLow90  = ((currentPrice - c.days90.low)  / c.days90.low  * 100).toFixed(1);
            const pctFromHigh30 = ((currentPrice - c.days30.high) / c.days30.high * 100).toFixed(1);
            const pctFromLow30  = ((currentPrice - c.days30.low)  / c.days30.low  * 100).toFixed(1);
            const change90 = ((c.days90.end - c.days90.start) / c.days90.start * 100).toFixed(1);
            const change30 = ((c.days30.end - c.days30.start) / c.days30.start * 100).toFixed(1);
            const change7  = ((c.days7.end  - c.days7.start)  / c.days7.start  * 100).toFixed(1);

            await tg(
              `📊 <b>${t.symbol} — HISTORY REPORT</b>\n` +
              `🕐 Updated: ${new Date(c.updatedAt||Date.now()).toLocaleString()}\n` +
              `━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +

              `<b>📍 WHERE IT IS NOW</b>\n` +
              `   Price: $${currentPrice.toFixed(8)}\n` +
              `   vs 90d high: ${pctFromHigh90}%\n` +
              `   vs 90d low:  +${pctFromLow90}%\n\n` +

              `<b>📅 7-DAY WINDOW</b>\n` +
              `   High: $${c.days7.high.toFixed(8)}\n` +
              `   Low:  $${c.days7.low.toFixed(8)}\n` +
              `   Range: ${(((c.days7.high-c.days7.low)/c.days7.low)*100).toFixed(1)}%\n` +
              `   Change: ${change7 >= 0 ? "+" : ""}${change7}%\n\n` +

              `<b>📅 30-DAY WINDOW</b>\n` +
              `   High: $${c.days30.high.toFixed(8)}\n` +
              `   Low:  $${c.days30.low.toFixed(8)}\n` +
              `   Range: ${(((c.days30.high-c.days30.low)/c.days30.low)*100).toFixed(1)}%\n` +
              `   Change: ${change30 >= 0 ? "+" : ""}${change30}%\n\n` +

              `<b>📅 90-DAY WINDOW</b>\n` +
              `   High: $${c.days90.high.toFixed(8)}\n` +
              `   Low:  $${c.days90.low.toFixed(8)}\n` +
              `   Range: ${(((c.days90.high-c.days90.low)/c.days90.low)*100).toFixed(1)}%\n` +
              `   Change: ${change90 >= 0 ? "+" : ""}${change90}%\n\n` +

              `<b>🌊 WAVE LEVELS (live-learned)</b>\n` +
              `   Peaks:   ${ws.peaks.length > 0 ? ws.peaks.map(p => "$"+p.toFixed(6)).join(" → ") : "still building"}\n` +
              `   Troughs: ${ws.troughs.length > 0 ? ws.troughs.map(t => "$"+t.toFixed(6)).join(" → ") : "still building"}\n\n` +

              `<b>💓 INDICATORS NOW</b>\n` +
              `   ${ind.detail || "building..."}\n\n` +

              `<b>🎯 ARM STATUS</b>\n` +
              `   ${arm.armed ? "✅ ARMED ["+arm.priority+"] "+((arm.net||0)*100).toFixed(2)+"% net margin" : "⏳ "+esc(arm.reason)}\n\n` +

              `<b>📖 HOW TO READ THIS</b>\n` +
              `   Range = how wide the price swings — bigger = more opportunity\n` +
              `   Bot buys near the LOW, sells near the HIGH\n` +
              `   If current price is near the 90d low → potential buy setup forming\n` +
              `   If current price is near the 90d high → near sell territory`
            );
          }
        } else {
