...
async function main() {
  ...
  // After every restart, scan the real wallet for token balances.
  // For any token with real balance but no entry price recorded,
  // recover the entry price from the ledger (last BUY of that token).
  // This is the ground truth — the blockchain never lies.
  try {
    console.log("🔍 Scanning wallet for untracked positions...");
    const ethPriceNow = await getLiveEthPrice();

    // Read ledger for last known BUY prices
    const lf = await githubGetFromBranch("ledger.json", STATE_BRANCH);
    const ledgerTrades = lf?.content?.trades || [];

    // Build map of last BUY price per symbol from ledger
    const lastBuyFromLedger = {};
    const lastBuyEthFromLedger = {};
    for (const t of ledgerTrades) {
      if (t.type === "BUY" && t.symbol && t.price > 0) {
        lastBuyFromLedger[t.symbol]    = t.price;
        lastBuyEthFromLedger[t.symbol] = t.ethSpent || 0;
      }
    }

    // Check each token balance directly on-chain
    let recovered = 0;
    for (const token of tokens) {
      try {
        ...
        const price    = history[token.symbol]?.lastPrice || 0;
        const valueUsd = bal * price;

        // If we have a real balance worth more than $0.30 but no entry recorded
        if (bal > 0.001 && valueUsd > 0.30 && !token.entryPrice) {
          const ledgerEntry = lastBuyFromLedger[token.symbol];
          if (ledgerEntry) {
            token.entryPrice       = ledgerEntry;
            token.totalInvestedEth = lastBuyEthFromLedger[token.symbol] || (valueUsd / ethPriceNow);
            token.entryTime        = Date.now();
            recovered++;
            console.log(`   ✅ RECOVERED ${token.symbol}: entry=$${ledgerEntry.toFixed(6)} bal=${bal.toFixed(4)} value=$${valueUsd.toFixed(2)}`);
          } else {
            // No ledger entry — use current price as entry (conservative)
            token.entryPrice       = price;
            token.totalInvestedEth = valueUsd / ethPriceNow;
            token.entryTime        = Date.now();
            recovered++;
            console.log(`   ⚠️  RECOVERED ${token.symbol} (no ledger): using current $${price.toFixed(6)} as entry, val=$${valueUsd.toFixed(2)}`);
          }
        }

        // Ghost position — entry price set but zero balance
        if (token.entryPrice && bal < 0.001) {
          console.log(`   👻 Cleared ghost: ${token.symbol} — no balance but entry was $${token.entryPrice.toFixed(6)}`);
          token.entryPrice       = null;
          token.totalInvestedEth = 0;
          token.entryTime        = null;
        }
    ...
    if (recovered > 0) {
      console.log(`✅ Position recovery: ${recovered} position(s) restored from on-chain data`);
    } else {
      console.log(`✅ Position recovery: wallet matches recorded state`);
    }
  } catch (e) {
    console.log(`⚠️  Position recovery error (non-critical): ${e.message}`);
  }
