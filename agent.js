...
// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v15.2
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  for (let i = 0; i < RPC_URLS.length; i++) {
    try {
      ...
      if (e.message?.includes("429") || e.message?.includes("rate limit") ||
          e.message?.includes("over rate") || e.message?.includes("rpc timeout")) {
        nextRpc(); await sleep(300); // rotate RPC and retry quickly
      } else throw e;
    }
  }
...
async function estimateGasCostEth() {
  ...
  try {
    const gasPrice = await rpcCall(c => c.getGasPrice());
    cachedGasCostEth   = Number(gasPrice * BigInt(220_000)) / 1e18;
...
async function executeSell(cdp, token, sellPct, reason, price, isProtective = false) {
  try {
    ...
    // Get real on-chain quote — this is what the pool will actually pay.
    // If we can't get a quote, fall back to cached price with wider 70% slippage
    // instead of 85%, giving more room so the tx doesn't revert.
    const onChainWethOut = await getOnChainSellQuote(token.address, amtToSell, token.feeTier);
    let minWeth;
    if (onChainWethOut && onChainWethOut > 0n) {
      // Use 92% of real quote — tight but realistic slippage for execution
      minWeth = BigInt(Math.floor(Number(onChainWethOut) * 0.92));
      const quotedPrice = (Number(onChainWethOut) / 1e18 * ethUsd) / (sellable * sellPct);
      console.log(`   📊 On-chain quote: ${(Number(onChainWethOut)/1e18).toFixed(6)} WETH | implied price $${quotedPrice.toFixed(6)}`);
    } else {
      // Fallback: cached price with wide slippage so tx doesn't revert
      minWeth = BigInt(Math.floor(procEth * 0.70 * 1e18));
      console.log(`   ⚠️  Using cached price fallback with 70% slippage floor`);
    }
...
async function processToken(cdp, token, bal) {
  try {
    ...
    // ── DECISIONS ──────────────────────────────────────────────────────────
    // atMaxPeak: widen to 1.0% tolerance — 0.5% was too tight, price often peaked
    // just below the recorded high and the bot missed the window entirely.
    const atMaxPeak    = maxPeak && price >= maxPeak * 0.990;
    ...
    // profitableSell: catch a wave turning over — sell when in ≥1.5% net profit
    // AND MACD just crossed bearish (wave peaked, now declining). Stops the bot
    // from holding through a full reversal waiting for a stale recorded high.
    const pnlPctNow = entry ? (price - entry) / entry : 0;
    const decliningFromOverbought = ind.rsi !== null && ind.rsi < 60
                                 && ind.macd?.crossDown === true;
    const profitableSell = entry && pnlPctNow >= 0.015 && decliningFromOverbought
                        && netIfSellNow > breakEvenBuffer && sellable > 1;
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════");
  console.log("⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v15.2");
  ...
  console.log("      v15.2: on-chain quote before every sell — fixes execution reverted on stale price");
  ...
  await tg(
    `⚔️💓 <b>GUARDIAN v15.2 — HEARTBEAT EDITION ONLINE</b>\n\n` +
