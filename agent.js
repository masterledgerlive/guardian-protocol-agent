  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      try {
        ...
        // ── ONE-TIME WITHDRAWAL: send USD-equivalent ETH to wife's Coinbase ──
        // Usage: /withdrawusd 1    (sends $1 worth of ETH)
        // Usage: /withdrawusd 50   (sends $50 worth of ETH)
        // Destination: 0xd539E9d118fFdb7a56c759Be81983CbD38fF544f (wife's Base ETH address)
        ...
        if (!usdAmount || isNaN(usdAmount) || usdAmount <= 0) {
          ...
          await tg(`⏳ <b>WITHDRAWAL INITIATED</b>\nConverting $${usdAmount} USD → ETH → sending...\nDestination: 0xd539E9d118fFdb7a56c759Be81983CbD38fF544f`);
          try {
            // Get live ETH price
            ...
            console.log(`💸 WITHDRAWAL: $${usdAmount} = ${ethAmount.toFixed(8)} ETH @ $${ethPrice.toFixed(2)} = ${weiAmount} wei`);
            // Get the CDP account
            const account = await cdpClient.evm.getAccount({
              address: WALLET_ADDRESS
            });
            // Send transaction — must use network: "base" (CDP SDK v2 style, not chainId)
            ...
            await tg(
              ...
              `💰 Sent: $${usdAmount} USD\n` +
              `🔢 Amount: ${ethAmount.toFixed(8)} ETH\n` +
              `💲 ETH Price: $${ethPrice.toFixed(2)}\n` +
              `📬 To: 0xd539E9d118fFdb7a56c759Be81983CbD38fF544f\n` +
              `🔗 TX: <code>${tx.transactionHash}</code>\n\n` +
              `View on BaseScan: https://basescan.org/tx/${tx.transactionHash}`
            );
            console.log(`✅ Withdrawal sent! TX: ${tx.transactionHash}`);
          } catch (withdrawErr) {
            await tg(`❌ <b>WITHDRAWAL FAILED</b>\n${withdrawErr.message}`);
            console.log(`❌ Withdrawal error: ${withdrawErr.message}`);
          }
        }
