...
async function getTokenDecimals(address) {
  if (tokenDecimalsCache[address]) return tokenDecimalsCache[address];
  try {
    const dec = await rpcCall(c => c.readContract({ address, abi: ERC20_ABI, functionName: "decimals", args: [] }));
    tokenDecimalsCache[address] = Number(dec);
    return Number(dec);
  } catch { return 18; } // default to 18
}

async function getTokenBalance(address) {
  try {
    const raw      = await rpcCall(c => c.readContract({ address, abi: ERC20_ABI, functionName: "balanceOf", args: [WALLET_ADDRESS] }));
