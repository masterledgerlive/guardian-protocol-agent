...
// ── 🧠 IKN MEMORY ENGINE — added when memory-engine.js is pushed to repo ──────
// import { buildCliffNote, buildFullSummary, inscribeMemory, queueCliffNoteAsBTP,
//   getRecentCliffNotes, searchMemory, getMemoryListMessage, parseRememberCommand,
//   serializeRegistry, deserializeRegistry } from "./memory-engine.js";
import {
  storeKey,
  showKey,
...
async function loadFromGitHub() {
  ...
  try {
    ...
    if (mf?.content) {
      // deserializeRegistry(mf.content); // memory-engine.js not yet loaded
  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      // ── 🧠 MEMORY COMMANDS ──────────────────────────────────────────────────
        ...
        await tg("🧠 IKN Memory coming soon — push memory-engine.js to repo to activate");
        ...
        await tg("🧠 /remember — push memory-engine.js to repo to activate");
        ...
        await tg("🧠 /savesession — push memory-engine.js to repo to activate");

      } else if (text && text.startsWith("/recall ")) {
        // Search memory by topic or date
        await tg("🧠 /recall — push memory-engine.js to repo to activate");

      } else if (text === "/context") {
        // Pull recent memories and show what would be auto-injected
        await tg("🧠 /context — push memory-engine.js to repo to activate");
