...
// The 1% skim is divided equally three ways on every profitable sell:
//   0.33% → lottery piggy  (locked forever — your personal savings)
...
// ── PREDICTION FUND ───────────────────────────────────────────────────────────
// Trades earlier than the main bot using raw prediction signals.
// Enters on pre-buy (before confirmed trough), exits on pre-sell (before peak).
// Never draws from main ETH balance — only its own pool.
const PRED_FUND_MIN_CONF   = 55;     // confidence threshold (lower than main bot = earlier entry)
const PRED_FUND_MAX_ALLOC  = 0.50;   // max 50% of fund per trade
const PRED_FUND_MIN_ETH    = 0.0001; // minimum fund size to trade
const PRED_FUND_COOLDOWN   = 90_000; // 90s cooldown between pred-fund trades per token
...
let predFund       = 0;        // ETH balance of the prediction fund pool
let predFundPnl    = 0;        // cumulative net ETH profit/loss
let predFundTrades = 0;        // completed round-trips
const predFundPos  = {};       // open positions: { [symbol]: { entryPrice, ethIn, tokens, entryTime, conf } }
const predFundCool = {};       // per-token cooldown timestamps
// ── AGENTIC CAPITAL state ─────────────────────────────────────────────────────
let agentCapital   = 0;        // ETH reserved for the agentic research layer (Phase 3)
let agentPnl       = 0;        // cumulative net ETH P&L (Phase 3)
...
async function executeSell(cdp, token, sellPct, reason, price, isProtective = false) {
  try {
    ...
    if (received > 0 && netUsd > 0) {
      skim        = received * PIGGY_SKIM_PCT;       // total 1%
      skimLottery = skim * SKIM_LOTTERY_SHARE;       // 0.33% → piggy (locked forever)
      skimPred    = skim * SKIM_PRED_SHARE;          // 0.33% → prediction fund
      skimAgent   = skim * SKIM_AGENT_SHARE;         // 0.33% → agentic capital
    ...
    await tg(
      ...
      `🐷 Piggy:       ${skimLottery.toFixed(6)} ETH (0.33% → locked)\n` +
      `🧠 Pred fund:   ${skimPred.toFixed(6)} ETH (0.33% → pool: $${(predFund*ethUsd).toFixed(3)})\n` +
      `🤖 Agent cap:   ${skimAgent.toFixed(6)} ETH (0.33% → pool: $${(agentCapital*ethUsd).toFixed(3)})\n` +
...
async function processToken(cdp, token, bal) {
  try {
    ...
    // ── PREDICTION FUND TICK ─────────────────────────────────────────────────
    // Runs every cycle — manages pred fund entry/exit completely independently.
...
// ═══════════════════════════════════════════════════════════════════════════════
// 🧠 PREDICTION FUND TRADING LAYER
// Separate pool funded by 0.33% of every profitable sell.
// Trades on raw prediction signals — earlier entry/exit than the main bot.
// Enters on pre-buy signal (before confirmed trough).
// Exits on pre-sell signal (before confirmed peak), DI outlier, or main peak hit.
// Position sizing is proportional to pool size — starts tiny, grows from its own wins.
// NEVER touches the main ETH balance. All P&L tracked and persisted separately.
// ═══════════════════════════════════════════════════════════════════════════════
async function runPredFundTick(cdp, token, price, ethUsd, pred, ind) {
  try {
    const sym = token.symbol;
    const pos = predFundPos[sym];
    const cooldownOk = !predFundCool[sym] || (Date.now() - predFundCool[sym]) > PRED_FUND_COOLDOWN;

    // ── EXIT ─────────────────────────────────────────────────────────────────
    // Exit when: prediction flips to pre-sell, DI says unprecedented, or main peak hit
    const shouldExit = pos && (
      (pred.ready && pred.action === "pre-sell" && pred.confidence >= PRED_CONFIDENCE_SELL) ||
      (pred.diScore > DI_OUTLIER_THRESHOLD) ||
      (getMaxPeak(sym) && price >= getMaxPeak(sym) * 0.995)
    );
    if (shouldExit) {
      const grossEth = (pos.tokens * price) / ethUsd;
      const netEth   = grossEth * (1 - (token.poolFeePct || 0.006));
      const pnlEth   = netEth - pos.ethIn;
      const pnlUsd   = pnlEth * ethUsd;
      const holdMins = ((Date.now() - pos.entryTime) / 60000).toFixed(1);
      predFund += netEth;
      predFundPnl += pnlEth;
      predFundTrades++;
      ...
      const win = pnlEth >= 0;
      console.log(`  🧠⚡ PRED FUND EXIT [${sym}] ${win ? "WIN" : "LOSS"} ${pnlEth>=0?"+":""}${pnlEth.toFixed(6)} ETH ($${pnlUsd>=0?"+":""}${pnlUsd.toFixed(3)}) | held ${holdMins}m | fund: ${predFund.toFixed(6)} ETH`);
      await tg(
        `🧠⚡ <b>PRED FUND EXIT — ${sym}</b>
` +
        `${win ? "📈 PROFIT" : "📉 LOSS"}: ${pnlEth>=0?"+":""}$${pnlUsd.toFixed(4)}
` +
        `⏱ Held: ${holdMins} min | Entry conf: ${pos.conf}%
` +
        `🧠 Fund: ${predFund.toFixed(6)} ETH ($${(predFund*ethUsd).toFixed(3)})
` +
        `📊 Cumulative: ${predFundPnl>=0?"+":""}$${(predFundPnl*ethUsd).toFixed(3)} | ${predFundTrades} trades`
      );
      return;
    }

    // ── ENTRY ─────────────────────────────────────────────────────────────────
    // Enter when: pre-buy signal, confidence OK, no open position, cooldown clear
    const canEnter = !pos &&
      pred.ready &&
      pred.action === "pre-buy" &&
    ...
    if (canEnter) {
      ...
      predFundPos[sym] = {
        ...
        phase:      pred.cyclePhase,
      };
      predFundCool[sym] = Date.now();
      console.log(`  🧠✅ PRED FUND ENTRY [${sym}] ${allocEth.toFixed(6)} ETH @ $${price.toFixed(8)} | conf:${pred.confidence}% φ${pred.cyclePhase?.toFixed(0)}° | fund left: ${predFund.toFixed(6)} ETH`);
      await tg(
        `🧠✅ <b>PRED FUND ENTRY — ${sym}</b>
` +
        `💰 Allocated: ${allocEth.toFixed(6)} ETH ($${(allocEth*ethUsd).toFixed(3)})
` +
        `📉 Entry: $${price.toFixed(8)} | Conf: ${pred.confidence}% φ${pred.cyclePhase?.toFixed(0)}°
` +
        `⏳ ${pred.barsToTurn ? pred.barsToTurn + " bars to predicted turn" : "turn imminent"}
` +
        `🧠 Fund remaining: ${predFund.toFixed(6)} ETH ($${(predFund*ethUsd).toFixed(3)})`
      );
    }
  } catch (e) {
...
async function loadFromGitHub() {
  ...
  const pfOpenCount = Object.keys(predFundPos).length;
  console.log(`✅ ${tokens.length} tokens | Positions: ${positions || "none"} | Piggy: ${piggyBank.toFixed(6)} ETH | Trades: ${tradeCount}`);
  console.log(`🧠 PredFund: ${predFund.toFixed(6)} ETH | AgentCap: ${agentCapital.toFixed(6)} ETH | PF open positions: ${pfOpenCount}`);
}
  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      try {
        ...
        const pfWinRate = predFundTrades > 0 ? "tracking" : "no trades yet";
        await tg(
          `🐷🧠🤖 <b>THE THREE POOLS</b>\n` +
          `━━━━━━━━━━━━━━━━━━━━\n\n` +
          `🐷 <b>Piggy Bank</b> (locked forever)\n` +
          ...
          `   Open positions: ${Object.keys(predFundPos).join(", ")||"none"}\n\n` +
