...
const MIN_TOSHI_TO_TRADE  = BigInt("1000000000000000000"); // 1 TOSHI minimum
...
async function executeBuy(cdp, ethAmount, reason) {
  ...
  if (ethAmount > tradeableEth) {
    ...
    console.log(`⚠️  Adjusted to ${ethAmount.toFixed(6)} ETH`);
  }
  if (ethAmount < 0.000010) {
    console.log(`⚠️  Buy too small — skipping`);
    return;
  }
  ...
  try {
    ...
    await sendAlert(`⚠️ <b>GUARDIAN — BUY FAILED</b>\n${e.message}`);
  }
}

async function executeSell(cdp, sellPercent, reason, currentPrice) {
  if (!isInProfit(currentPrice)) {
    console.log(`🛑 SELL BLOCKED — not in profit yet`);
    return;
  }

  const toshiBalance = await getToshiBalance();
  if (toshiBalance < MIN_TOSHI_TO_TRADE) {
    console.log(`⚠️  TOSHI balance too small to sell`);
    return;
  }
  ...
  try {
    ...
    if (allowance < amountToSell) {
      ...
      await sendAlert(`🔑 <b>GUARDIAN — TOSHI APPROVED</b>\n🔗 <a href="https://basescan.org/tx/${approveTx}">Basescan</a>`);
    ...
    if (profit > 0) {
      ...
      console.log(`🐷 PIGGY BANK +${skim.toFixed(6)} ETH (total: ${totalSkimmed.toFixed(6)} ETH)`);
    }
    ...
    await sendAlert(`⚠️ <b>GUARDIAN — SELL FAILED</b>\n${e.message}`);
  }
}

async function main() {
  console.log("═══════════════════════════════════════════");
  console.log("⚔️   GUARDIAN SMART GRID BOT v4 — LIVE");
  console.log("═══════════════════════════════════════════");
  ...
  const hasToshi = toshiBalance >= MIN_TOSHI_TO_TRADE;
  const toshiUsd = price ? (Number(toshiBalance) / 1e18 * price).toFixed(2) : "?";
  ...
  console.log(`🪙 TOSHI held    : ${hasToshi ? "YES — $" + toshiUsd : "NONE"}\n`);

  await sendAlert(
    `⚔️ <b>GUARDIAN SMART GRID BOT v4 — STARTED</b>\n\n` +
    `👛 Wallet: <code>${WALLET_ADDRESS}</code>\n` +
    `💰 ETH: ${ethBalance.toFixed(6)} | Tradeable: ${tradeableEth.toFixed(6)}\n` +
    `🪙 TOSHI: ${hasToshi ? "$" + toshiUsd : "none"}\n` +
    `⛽ Gas reserve: ${GAS_RESERVE} ETH locked\n` +
    `💲 TOSHI price: $${price?.toFixed(8)}\n\n` +
    `${hasToshi ? "✅ TOSHI already held — skipping initial buy" : "🚀 No TOSHI found — doing initial buy"}\n` +
    `Bot watching every 15 seconds!`
  );

  // ── ONLY buy if no TOSHI already held ───────────
  if (!hasToshi) {
    console.log(`🚀 No TOSHI found — doing initial 45% buy...`);
    const initialEth = tradeableEth * 0.45;
    await executeBuy(cdp, initialEth, "INITIAL POSITION — 45% tradeable ETH into TOSHI");
  } else {
    console.log(`✅ TOSHI already held ($${toshiUsd}) — skipping initial buy, watching market...`);
  }
  ...
  while (true) {
    try {
      ...
      console.log(`💰 ETH: ${ethBalance.toFixed(6)} | Tradeable: ${tradeableEth.toFixed(6)} | P&L: ${pnl}%`);
      console.log(`🐷 Piggy: ${piggyBank.toFixed(6)} ETH | ⛽ Gas: ${GAS_RESERVE} ETH`);
