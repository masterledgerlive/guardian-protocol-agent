  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      // ── 🌟 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        // VAULT_VITA_ANTHROPIC_KEY is the main key — falls back for legacy installs
        const vitaApiKey = process.env.VAULT_VITA_ANTHROPIC_KEY || process.env.VITA_ANTHROPIC_KEY || process.env.ANTHROPIC_API_KEY;
        if (!vitaApiKey) {
          await tg(
      ...
      //        /vitalearn [paste any text after the command]
        ...
        const vitaKey = process.env.VAULT_VITA_ANTHROPIC_KEY || process.env.VITA_ANTHROPIC_KEY || process.env.ANTHROPIC_API_KEY;
        if (!vitaKey) { await tg("❌ VITA needs VAULT_VITA_ANTHROPIC_KEY in Railway"); }
        else {
...
VERIFY: c=299792458, Nobel=1921, born=1879-03-14, died=1955-04-18, LIGO detection=2015-09-14`,
        ...
        if (!vitaKey) {
          await tg("🌟 VITA needs VAULT_VITA_ANTHROPIC_KEY in Railway to answer");
      ...
      // ── 💓 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        const apiKey  = process.env.VAULT_VITA_ANTHROPIC_KEY || process.env.VITA_ANTHROPIC_KEY || process.env.ANTHROPIC_API_KEY;
        if (!apiKey) {
          await tg(
            "❌ <b>VAULT_VITA_ANTHROPIC_KEY not set</b>\n\n" +
            "Add it via vault:\n" +
            "<code>/newvault VAULT_VITA_ANTHROPIC_KEY</code>\n" +
            "Then reply with your API key.\n\n" +
            "Or add directly to Railway as VAULT_VITA_ANTHROPIC_KEY"
          );
        } else {
