  ...
  try {
    ...
    for (const update of data.result) {
      ...
      if (text.startsWith("/buy ")) {
        ...
        const weth2 = cachedWethBalance;
        const eth2  = cachedEthBalance;
        await sendAlert(
          "<b>GUARDIAN v10.0 STATUS</b>\n" +
          new Date().toLocaleTimeString() + "\n\n" +
          "ETH: " + eth2.toFixed(6) + " (~$" + (eth2 * ETH_USD).toFixed(2) + ")\n" +
          "WETH: " + weth2.toFixed(6) + " (~$" + (weth2 * ETH_USD).toFixed(2) + ") <- buys\n" +
          "Tradeable: " + getTradeableEth(eth2).toFixed(6) + " ETH\n" +
          "Piggy: " + piggyBank.toFixed(6) + " ETH LOCKED\n" +
          "Trades: " + tradeCount + "\n" +
          "-------------------------" +
          lines +
          "\nBasescan: https://basescan.org/address/" + WALLET_ADDRESS
        );
        ...
        await sendAlert(
          "GUARDIAN BALANCES\n\n" +
          "ETH:      " + eth2.toFixed(6) + " (~$" + (eth2 * ETH_USD).toFixed(2) + ")\n" +
          "WETH:     " + weth2.toFixed(6) + " (~$" + (weth2 * ETH_USD).toFixed(2) + ") <- bot buys\n" +
          "Combined: " + (eth2 + weth2).toFixed(6) + " ETH (~$" + ((eth2 + weth2) * ETH_USD).toFixed(2) + ")\n" +
          "Tradeable: " + getTradeableEth(eth2).toFixed(6) + " ETH\n" +
        ...
        for (const t of tokens) {
          ...
          const cp       = hl ? getCyclePosition(t.symbol, p) : null;
          const sellT    = hl ? hl.low + hl.range * SELL_ZONE : null;
          msg += (parseFloat(pnlPct) >= 0 ? "[+]" : "[-]") + " " + t.symbol + "\n";
          ...
          msg += "Holding: " + Math.floor(bal) + " | Lottery: " + lottery + " kept\n\n";
        }
