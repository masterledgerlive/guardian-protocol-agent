        ...
        if (!vitaApiKey) {
          ...
          try {
            ...
            const sessionCtx = [
              ...
              "PIGGY:" + piggyBank.toFixed(6) + "ETH",
              "RECENT_TRADES:" + tradeLog.slice(-5).map(t => t.type+t.symbol+"#"+t.tradeNum).join(","),
              ...
              "WHAT_WAS_BUILT_THIS_SESSION:vault-loader,vault-unlock,keystore,memory-engine,vita-memory,log-formatter — blockchain key vault system, stage1 telegram unlock, personal keystore, autonomous AI memory on Base blockchain",
              "KEY_DECISIONS:GITHUB_REPO+TELEGRAM_BOT_TOKEN moved to Base blockchain, VITA has own API key separate from DA, memory auto-files to vita-registry.json after each vitasave",
              "WHAT_WAS_PROVED:blockchain key retrieval at boot works, BTP inscription works, VITA autonomous memory save+file works",
              "VISION:IKN universal agent memory layer, every trade carries memory, strand algorithm, hive mind growing on chain",
              "NEXT:CDP keys migration, VITA own key vault, L1 spec, strand retrieval protocol, child key system",
              // Include any manually queued notes from /vitanote
              ...(global._vitaNotes && global._vitaNotes.length > 0
                ? ["QUEUED_NOTES:" + global._vitaNotes.map(n => n.note).join(" | ")]
                : []),
            ].join("\n");
...
VERIFY: c=299792458, Nobel=1921, born=1879-03-14, died=1955-04-18, LIGO detection=2015-09-14`,
      ...
      // ── 💓 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        await tg(
          ...
          `/vitasave — compress + inscribe session on Base\n` +
