...
// ═══════════════════════════════════════════════════════════════════════
// ⚔️  GUARDIAN CASCADE SYSTEM v11.7 — ACTIVE EDITION
// ═══════════════════════════════════════════════════════════════════════
...
const buyTimers     = {};
const waveState     = {};
const cyclePrices   = {};  // { symbol: { prev: price, prevTime: ms } } — for cross-cycle momentum
...
function getMomentum(symbol) {
  ...
  if (!cp || !cp.prev) return { direction: "neutral", pct: 0 };
  const pct = (cp.current - cp.prev) / cp.prev * 100;
  return {
    direction: pct > 0.1 ? "up" : pct < -0.1 ? "down" : "neutral",
    pct,
  };
}
...
function updateCyclePrice(symbol, price) {
  if (!cyclePrices[symbol]) {
    ...
    // Shift current -> prev each cycle (every ~30s)
    const age = Date.now() - cyclePrices[symbol].prevTime;
    if (age > 20000) {  // at least 20s between shifts
...
function isMomentumSlowing(symbol) {
  return false; // disabled — needs multi-cycle data, not same-cycle stream
}
...
// ── ENCODE ────────────────────────────────────────────────────────────────
// amountOutMinimum: set to expected * 0.85 (15% max slippage) to prevent bad fills
function encodeSwap(tokenIn, tokenOut, amountIn, recipient, fee = 3000, amountOutMinimum = 0n) {
  const p = (v, isAddr = false) => (isAddr ? v.slice(2) : BigInt(v).toString(16)).padStart(64, "0");
  return "0x04e45aaf" + p(tokenIn,true) + p(tokenOut,true) + p(fee) + p(recipient,true) + p(amountIn) + p(amountOutMinimum) + p(0);
}

// Calculate minimum WETH out for a sell: tokens * price / ETH_USD * 0.85, in wei
function minWethOut(tokenAmount, priceUsd) {
  const expectedEth = (tokenAmount * priceUsd) / ETH_USD;
  const minEth      = expectedEth * 0.85; // 15% max slippage
  return BigInt(Math.floor(minEth * 1e18));
}

// Calculate minimum tokens out for a buy: ethIn / price * ETH_USD * 0.85, in token units (1e18)
function minTokensOut(ethIn, priceUsd) {
  const expectedTokens = (ethIn * ETH_USD) / priceUsd;
  const minTokens      = expectedTokens * 0.85;
  return BigInt(Math.floor(minTokens * 1e18));
}
...
async function executeBuy(cdp, token, tradeableEth, reason, price, forcedEth = 0) {
  try {
    ...
    if (useWeth) {
      ...
      const minOut = minTokensOut(ethToSpend, price);
      const { transactionHash } = await cdp.evm.sendTransaction({ address: WALLET_ADDRESS, network: "base", transaction: { to: SWAP_ROUTER, data: encodeSwap(WETH_ADDRESS, token.address, amountIn, WALLET_ADDRESS, token.feeTier, minOut) } });
      txHash = transactionHash;
    } else {
      const minOut = minTokensOut(ethToSpend, price);
      const { transactionHash } = await cdp.evm.sendTransaction({ address: WALLET_ADDRESS, network: "base", transaction: { to: SWAP_ROUTER, value: amountIn, data: encodeSwap(WETH_ADDRESS, token.address, amountIn, WALLET_ADDRESS, token.feeTier, minOut) } });
      txHash = transactionHash;
    }
...
async function executeSell(cdp, token, sellPct, reason, price, isProtective = false) {
  try {
    ...
    // If total balance is effectively zero (dust / rounding), clear position
    if (totalBal < 0.01) {
      console.log(`   ⚠️  ${token.symbol}: dust balance ${totalBal} — clearing position`);
    ...
    if (sellable < 1) {
      console.log(`   ⏳ ${token.symbol}: only lottery token remains — clearing position state`);
    ...
    // Log if selling below entry (peak sell - taking small loss at market top rather than bigger loss later)
    if (!isProtective && token.entryPrice && price < token.entryPrice) {
      const lossPct = ((price - token.entryPrice) / token.entryPrice * 100).toFixed(2);
      console.log(`   ⚠️  ${token.symbol}: selling at peak — ${lossPct}% vs entry (market peaked, exiting)`);
    }
    ...
    const minWeth = minWethOut(sellable * sellPct, price);
    console.log(`      🛡️ Min WETH out: ${(Number(minWeth)/1e18).toFixed(6)} (85% of expected $${(sellable*sellPct*price).toFixed(4)})`);
...
async function processToken(cdp, token, ethBal) {
  try {
    ...
    const balDisp = balance >= 1 ? Math.floor(balance).toString() : balance.toFixed(6);
    console.log(`  │ 🪙 ${balDisp} ($${(balance*price).toFixed(2)}) | mom:${mom.direction}(${mom.pct.toFixed(2)}%) prev:$${cyclePrices[token.symbol]?.prev?.toFixed(8)||"?"}`);
...
async function main() {
  console.log("═══════════════════════════════════════════════════════");
  console.log("⚔️   GUARDIAN CASCADE SYSTEM v11.7 — ACTIVE EDITION");
  console.log("═══════════════════════════════════════════════════════\n");
