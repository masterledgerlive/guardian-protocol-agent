        ...
        if (!vitaApiKey) {
          ...
          try {
            ...
            try {
              ...
              // Load existing registry with SHA
              let registry = {};
              let regSha   = null;
              try {
                const rf = await githubGet("vita-registry.json");
                if (rf?.content && typeof rf.content === "object") {
                  registry = rf.content;
                  regSha   = rf.sha || null;
                }
              } catch (e) {
                console.log("📁 VITA: creating new registry");
              }
              ...
              // Save with correct SHA
              const savedSha = await githubSave("vita-registry.json", registry, regSha);
              if (!savedSha) throw new Error("githubSave returned null — SHA conflict or API error");
              console.log("📁 VITA: registry saved — " + Object.keys(registry).length + " entries");

              await tg(
                "📁 <b>FILED AS:</b> <code>" + regKey + "</code>\n" +
                "Registry: " + Object.keys(registry).length + " session(s) total\n\n" +
                "Pull anytime:\n" +
                "<code>/vita " + fileLabel.split("-").slice(0,3).join(" ") + "</code>"
              );
            } catch (fileErr) {
