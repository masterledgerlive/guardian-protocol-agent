...
// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v14.4
// ═══════════════════════════════════════════════════════════════════════════════
...
function getArmStatus(symbol, gasCostEth, tradeEth) {
  ...
  if (net < minNM) {
    ...
    if (grossPct < feePct * 1.5) {
      return { armed: false, reason: `⚠️ DEAD WAVE: range ${(grossPct*100).toFixed(2)}% < fees ${(feePct*100).toFixed(1)}% — needs bigger move` };
    }
...
async function processToken(cdp, token, bal) {
  try {
    ...
    // Fires async, cached 90s — never adds latency to the main loop.
    // If top traders are buying: adds buy confirmation. If selling: blocks buys.
    let topTraderSig = null;
    fetchTopTraderSignal(token.address).then(sig => {
    ...
    if (arm.armed) lines.push(`  │ ✅ ARMED ${(arm.net*100).toFixed(2)}% [${arm.priority}] (w/ price impact)`);
      ...
      if (maxPeak && minTrgh && maxPeak > minTrgh) {
        ...
        if (grossRange < minNeeded && peakCnt >= 2 && trghCnt >= 2) {
          lines.push(`  │ ⚠️  DEAD WAVE: ${(grossRange*100).toFixed(2)}% range < ${(minNeeded*100).toFixed(1)}% needed to clear fees — needs wider price action`);
        }
      }
    }
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════");
  console.log("⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v14.4");
  ...
  console.log("      v14.4: wider sell trigger | smart money signal | dead-wave detector");
  ...
  await tg(
    `⚔️💓 <b>GUARDIAN v14.4 — HEARTBEAT EDITION ONLINE</b>\n\n` +
