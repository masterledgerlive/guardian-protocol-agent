...
async function processSurfers(cdp, bal, ethUsd) {
  ...
  for (const [name, s] of Object.entries(surfers)) {
    ...
    if (s.status === "WAITING") {
      // Enter when at/near min trough + armed + good indicator score
      // OR when 4-wave projection says trough is imminent
      const atTrough    = minT && price <= minT * (1 + TROUGH_ENTRY_BAND); // use wider band
      const projTrough  = getNextProjectedTrough(s.symbol);
      const projImminent = projTrough
        && projTrough.etaMs - Date.now() < 3 * 60_000 // within 3 min
        && price <= projTrough.projPrice * 1.04;
      // Enhanced entry: wave prediction + traditional checks + spot call detection
      const entrySigSurf = getSurferEntrySignal(s.symbol, price, ethUsd);
      const goodEntry = (arm.armed && arm.net >= 0.02 && ind.score >= 40 && (atTrough || projImminent))
                     || (entrySigSurf.shouldEnter && entrySigSurf.confidence >= 60);
      if (goodEntry) {
        s.entryPrice = price;
        ...
        s.trades++;
        console.log(`🏄 [${name}] PADDLING IN → ${s.symbol} @ $${price.toFixed(8)} | alloc $${(s.ethAlloc*ethUsd).toFixed(2)}`);
        await tg(`🏄 <b>SURFER ${name} — PADDLING IN</b>\n` +
          `Token: <b>${s.symbol}</b> @ $${price.toFixed(8)}\n` +
          `Capital: $${(s.ethAlloc*ethUsd).toFixed(2)} | Net margin: ${(arm.net*100).toFixed(2)}%\n` +
          `RSI: ${ind.rsi?.toFixed(1) || "?"} | Score: ${ind.score}%`);
      }
      ...
      if (atPeak || (waveLost && !kahuna?.active) || stopLoss) {
        ...
        // After exit, find next best wave
        s.symbol = null;

      } else {
        // Still riding — log progress periodically
        // (handled by main processToken display, no extra action needed)
      }
...
function computeWavePrediction(symbol, currentPrice, ethUsd) {
  ...
  const waves = [
    { wave: 1, low: clip(w1Low), high: clip(w1High), goDown: goingDown, amp: amp * buyAmp, predicted: true },
    { wave: 2, low: clip(w2Low), high: clip(w2High), goDown: !goingDown, amp: amp * buyAmp, predicted: true },
    { wave: 3, low: clip(w3Low), high: clip(w3High), goDown: goingDown,  amp: amp * buyAmp, predicted: true },
    { wave: 4, low: clip(w4Low), high: clip(w4High), goDown: !goingDown, amp: amp * buyAmp, predicted: true },
  ];
...
function getSurferEntrySignal(symbol, currentPrice, ethUsd) {
  ...
  const nearEntry = currentPrice <= pred.entryTarget * 1.02;
  if (nearEntry) { confidence += 30; reasons.push("at predicted entry zone"); }
  ...
  const recentMove = recentReadings.length >= 2
    ...
    : 0;
  const spotBuy = recentMove >= 0.005; // 0.5% move in last 5 readings = instant wave forming
...
async function processToken(cdp, token, bal) {
  try {
    ...
    const atMinTrough     = minTrgh && price <= minTrgh * (1 + TROUGH_ENTRY_BAND);
  ...
  if (pf?.content) {
    ...
    for (const t of tokens) {
      if (pos.entries?.[t.symbol] != null) {
        ...
        // SANITY CHECK: if saved entry is > 50x current live price, it's corrupt
        // (was stored as USD value instead of token price — classic state bug)
        // Also reject entries that are 0 or negative.
        const isSane = savedEntry > 0 &&
          (currentPrice === 0 || (savedEntry <= currentPrice * 50 && savedEntry >= currentPrice * 0.02));
        if (isSane || currentPrice === 0) {
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════");
  console.log("⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v18.0 — CHAIN-FIRST / SURGICAL FIX");
  ...
  // ── EMERGENCY POSITION INJECTION ─────────────────────────────────────────
  // DA has confirmed holdings: MIGGLES $9.01, TOBY $2.37, MOG $0.57
  // Hardcode these so /exit works immediately while ledger system is fixed
  // Entry price set to current market price (unknown true entry — use 0 cost basis)
  try {
    const ethPriceEmergency = await getLiveEthPrice();
    const emergencyPositions = [
      { symbol: "MIGGLES", valueUsd: 9.01  },
      { symbol: "TOBY",    valueUsd: 2.37  },
      { symbol: "MOG",     valueUsd: 0.57  },
    ];
    for (const ep of emergencyPositions) {
      const token = tokens.find(t => t.symbol === ep.symbol);
      if (!token) continue;
      if (token.entryPrice) continue; // already set — don't overwrite
      // Read actual balance from chain
      const bal = await getTokenBalance(token.address);
      const estimatedPrice = bal > 0 ? (ep.valueUsd / bal) : 0.000001;
      token.entryPrice       = estimatedPrice > 0 ? estimatedPrice : 0.000001;
      token.totalInvestedEth = ep.valueUsd / ethPriceEmergency;
      token.entryTime        = Date.now();
      tokenBalanceCache[token.symbol] = bal;
      console.log(`🚨 EMERGENCY INJECT ${ep.symbol}: bal=${bal.toFixed(4)} entry=$${token.entryPrice.toFixed(8)} val≈$${ep.valueUsd}`);
    }
    await tg(
      "🚨 <b>EMERGENCY POSITIONS LOADED</b>\n━━━━━━━━━━━━━━━━━━━━\n" +
      "MIGGLES: ~$9.01\n" +
      "TOBY: ~$2.37\n" +
      "MOG: ~$0.57\n\n" +
      "Run now to exit:\n" +
      "<code>/exit MIGGLES</code>\n" +
      "<code>/exit TOBY</code>\n" +
      "<code>/exit MOG</code>"
    );
  } catch (e) {
    console.log("⚠️ Emergency inject failed: " + e.message);
  }
