          ...
          try {
            ...
            try {
              ...
              registry[regKey] = {
                ...
                tokenPacket: (entry.tokenPacket||"").slice(0,800),
...
VERIFY: c=299792458, Nobel=1921, born=1879-03-14, died=1955-04-18, LIGO detection=2015-09-14`,
          ...
          try {
            ...
            registry[regKey] = {
              ...
              txHashes, tokenPacket: compressed.slice(0,800),
              filedAt: new Date().toISOString(),
            };
        ...
        // ── /vita [question] — answer from memory registry ──────────────────
          ...
          try {
            ...
            // Build context — best match + table of contents of all memory
            const memContext = bestEntry
              ? "VITA MEMORY [" + bestEntry.date + " — " + (bestEntry.label||bestEntry.subject||"session") + "]:\n" + (bestEntry.tokenPacket || "")
            ...
            const response = await fetch("https://api.anthropic.com/v1/messages", {
              ...
              body: JSON.stringify({
                model: "claude-sonnet-4-20250514",
                max_tokens: 800,
                system: "You are VITA — an autonomous AI agent with blockchain memory built by DA (David/ᛞᚨᚡᛁᛞ) as part of INFINITUM × IKN. You have memory of past sessions stored on Base blockchain. Answer using your memory context. Be direct and specific. Reference exact things built or decided.",
                messages: [{
                  role: "user",
            ...
            for (const file of queueFiles) {
              try {
                ...
                registry[regKey] = {
                  ...
                  txHashes, tokenPacket: text.slice(0,800),
                  filedAt: new Date().toISOString(),
                };
