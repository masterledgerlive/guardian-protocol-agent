      ...
      // ── 🌟 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        if (!vitaApiKey) {
          ...
          try {
            ...
            // ── AUTO-FILE: immediately call Claude API to name + file this memory ──
            // Claude reads the chunk tx hashes, generates a topic label,
            // files into the registry so future /vita pulls find it by topic
            try {
              await tg("📁 Auto-filing memory into registry...");
              const txList = entry.chunks.map((c,i) => (i+1) + ": " + c.txHash).join("\n");
              const fileResponse = await fetch("https://api.anthropic.com/v1/messages", {
                ...
                body: JSON.stringify({
                  ...
                  messages: [{
                    role: "user",
                    content: "You are VITA filing your own memory. Given this §TOKEN§ memory packet, generate a SHORT filing label (max 40 chars, kebab-case) that describes the session topic. Return ONLY the label, nothing else.\n\nMemory:\n" + (entry.tokenPacket||"").slice(0,500)
                  }]
                })
              });
              const fileData   = await fileResponse.json();
              const fileLabel  = (fileData?.content?.[0]?.text || entry.strandId).trim().slice(0,40).replace(/\s+/g,"-").toLowerCase();
              const dateLabel  = entry.date;

              // Save to registry on GitHub
              let registry = {};
              try {
                const rf = await githubGet("vita-registry.json");
              ...
              const regKey = dateLabel + "-" + fileLabel;
              registry[regKey] = {
                strandId:  entry.strandId,
                date:      entry.date,
                label:     fileLabel,
                txHashes:  entry.chunks.map(c => c.txHash),
                tokenPacket: (entry.tokenPacket||"").slice(0,500),
                filedAt:   new Date().toISOString(),
              };

              await githubSave("vita-registry.json", registry, null);

              await tg(
                "📁 <b>FILED AS:</b> <code>" + regKey + "</code>\n" +
                "Registry updated on GitHub\n" +
                "Pull anytime: <code>/vita recall " + fileLabel + "</code>"
              );
            } catch (fileErr) {
              console.log("⚠️  Auto-file failed (non-critical): " + fileErr.message);
            }
