...
// ═══════════════════════════════════════════════════════════════
// GUARDIAN CASCADE SYSTEM v8.0 — 8 TOKENS + CHEAPER FEES
//
// HOW IT WORKS:
...
const MIN_TRADE_USD  = 0.10;     // trade must be worth at least $0.10
...
const DEFAULT_TOKENS = [
  ...
  { symbol: "TOSHI",   address: "0xAC1Bd2486aAf3B5C0fc3Fd868558b082a531B2B4", status: "active", lotteryTickets: 0, entryPrice: null, totalInvested: 0 },
  ...
  { symbol: "CLANKER", address: "0x1bc0c42215582d5a085795f4badbac3ff36d1bcb", status: "active", lotteryTickets: 0,     entryPrice: null, totalInvested: 0 },
  { symbol: "SEAM",    address: "0x1C7a460413dD4e964f96D8dFC56E7223cE88CD85", status: "active", lotteryTickets: 0,     entryPrice: null, totalInvested: 0 },
];
...
async function loadFromGitHub() {
  ...
  // Load saved positions (entry prices, totalInvested, piggy bank, tradeCount)
  const pf = await githubGet("positions.json");
  if (pf && pf.content) {
    positionsSha = pf.sha;
    const pos    = pf.content;
    // Restore piggy bank and trade count
    ...
    // Restore entry prices per token
    if (pos.entries) {
    ...
    console.log(`✅ Positions restored: ${Object.keys(pos.entries||{}).filter(k=>pos.entries[k]).join(", ") || "none"}`);
...
async function saveToGitHub() {
  ...
  // Save positions — entry prices, piggy bank, trade count survive restarts
  const positions = {
...
// ── FEE TIER per token (CLANKER uses 1%, others use 0.3%) ─────────────────
function getFeeTier(tokenAddress) {
  // CLANKER only has liquidity in the 1% pool
  if (tokenAddress.toLowerCase() === "0x1bc0c42215582d5a085795f4badbac3ff36d1bcb") return 10000;
  return 3000; // 0.3% — cheaper for all other tokens
}
...
async function executeSell(cdp, token, sellPct, reason, price, hl) {
  ...
  const sellable = totalBal;

  if (sellable < 1) {
    console.log(`   ⏳ ${token.symbol}: no tokens to sell`);
    return;
  }
  ...
  console.log(`      Selling ALL ${sellable.toFixed(0)} tokens — no lottery lock`);
  ...
  try {
    ...
    if (profit > 0) {
      const skim    = profit * 0.01;  // 1% to piggy bank
      piggyBank    += skim;
      totalSkimmed += skim;
    }

    // Piggy bank grows toward TOSHI ATH goal ($1000)
    // No lottery lock — sell everything, piggy bank handles the dream
    ...
    await sendAlert(
      ...
      `🐷 Piggy bank: ${piggyBank.toFixed(6)} ETH (growing toward TOSHI $1k goal)\n` +
      `📊 ${reason}\n` +
...
async function processToken(cdp, token, ethBalance) {
  ...
  const sellableLine = sellable > 1 ? `${sellable.toFixed(0)} sellable` : `none`;
...
async function sendReport(ethBalance) {
  ...
  await sendAlert(
    `📊 <b>GUARDIAN CASCADE v8.0 — 30 MIN REPORT</b>\n\n` +
    `💰 ETH: ${ethBalance.toFixed(6)} (~$${(ethBalance*ETH_USD).toFixed(2)})\n` +
    `♻️ Tradeable: ${getTradeableEth(ethBalance).toFixed(6)} ETH\n` +
    `🐷 Piggy bank: ${piggyBank.toFixed(6)} ETH (${(piggyBank*ETH_USD).toFixed(2)} USD)\n` +
    `🎯 Goal: enough TOSHI at ATH = $1000 target\n` +
...
// Message your bot any time to get instant info:
//   /status   — full portfolio snapshot right now
//   /eth      — ETH balance + tradeable amount
//   /prices   — current price + cycle position for all tokens
//   /piggy    — piggy bank balance + progress
//   /trades   — total trades since bot started
...
`;
        for (const t of tokens) {
          ...
          const bal = history[t.symbol] ? (await getTokenBalance(t.address)) : 0;
          const usd = (bal * p).toFixed(2);
          const inSell = hl ? getCyclePosition(t.symbol, p) >= (SELL_ZONE*100) : false;
          const inBuy  = hl ? getCyclePosition(t.symbol, p) <= (BUY_ZONE*100) : false;
          const zone   = inSell ? "🔴 SELL" : inBuy ? "🟢 BUY" : "⬜ WATCH";
          const sellTrig = hl ? hl.low + (hl.range * SELL_ZONE) : null;
          const pctNeeded = sellTrig ? ((sellTrig - p) / p * 100) : null;
          msg += `<b>${t.symbol}</b> ${zone} | Cycle: ${cp}%
`;
          msg += `💲 $${p.toFixed(8)} | 🪙 $${usd}
`;
          if (pctNeeded && pctNeeded > 0) msg += `📈 Need +${pctNeeded.toFixed(2)}% to sell
`;
          msg += `
`;
        }
...
` +
          `Total: ${ethBalance.toFixed(6)} ETH (~$${(ethBalance*ETH_USD).toFixed(2)})
` +
          `Tradeable: ${getTradeableEth(ethBalance).toFixed(6)} ETH
` +
          `Gas reserve: ${GAS_RESERVE} ETH (locked)
` +
          `Sell reserve: 0.001 ETH (locked)
` +
          `🐷 Piggy: ${piggyBank.toFixed(6)} ETH

` +
          `🔗 <a href="https://basescan.org/address/${WALLET_ADDRESS}">View on Basescan</a>`
        );
...
`; continue; }
          ...
          const inSell = hl ? getCyclePosition(t.symbol, p) >= (SELL_ZONE*100) : false;
          const inBuy  = hl ? getCyclePosition(t.symbol, p) <= (BUY_ZONE*100) : false;
          const zone   = inSell ? "🔴" : inBuy ? "🟢" : "⬜";
          msg += `${zone} <b>${t.symbol}</b> $${p.toFixed(8)} | Cycle: ${cp}%
`;
          if (hl) msg += `   LOW: $${hl.low.toFixed(8)} | HIGH: $${hl.high.toFixed(8)}
`;
        }
        await sendAlert(msg);

      } else if (text === "/piggy" || text === "piggy") {
        const goal    = 0.5; // 0.5 ETH goal for piggy bank
...
` +
        ...
        await sendAlert(
          ...
          `/eth       — ETH balance breakdown\n` +
          `/prices    — live prices + cycle positions\n` +
          `/positions — open positions + P&L\n` +
          `/piggy     — piggy bank progress\n` +
          `/trades    — trade count + stats\n` +
          `/help      — this menu\n\n` +
          `Just type any command and I'll respond instantly!`
        );
      }
    }
  } catch (e) {
    // Silent fail — don't crash the bot over a command error
  }
}
...
async function main() {
  console.log("═══════════════════════════════════════════════════");
  console.log("⚔️   GUARDIAN CASCADE SYSTEM v8.0 — 8 TOKENS + CHEAPER FEES");
  console.log("═══════════════════════════════════════════════════\n");
  ...
  await sendAlert(
    `⚔️ <b>GUARDIAN CASCADE v8.0 — STARTED</b>\n\n` +
  ...
  let cachedEth = await getEthBalance();
