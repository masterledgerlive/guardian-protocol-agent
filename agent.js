...
async function btpInscribe(cdp, tradeLabel) {
  try {
    ...
    // ── Read receipts ────────────────────────────────────────────────────────
    const isTradeBTP = name.startsWith("BUY#") || name.startsWith("SELL#");

    if (isTradeBTP) {
      // Parse the receipt fields for a clean Telegram display
      const fields = {};
      full.replace(/^\[TRADE:[^\]]+\]/, "").split("|").forEach(f => {
        const [k, ...v] = f.split(":");
        if (k) fields[k.trim()] = v.join(":").trim();
      });
      const isBuy  = name.startsWith("BUY#");
      const isWin  = (fields.RESULT||"").includes("PROFIT");
      const pnl    = fields.PNL || "";

      await tg(
        (isBuy ? "🟢" : (isWin ? "✅" : "🔴")) + " <b>TRADE RECEIPT ON BASE</b>\n" +
        "━━━━━━━━━━━━━━━━━━━━\n" +
        (isBuy ? [
          `📊 <b>${fields.SYM}</b> [${fields.TIER||"?"}] — BUY #${name.slice(4)}`,
          `💰 Entry: <b>$${fields.ENTRY||"?"}</b>`,
          `📥 Invested: ${fields.IN||"?"}`,
          `🎯 Target: ${fields.TARGET||"?"} | Wave ext: ${fields.WAVE_EXT||"?"}`,
          `🛑 Stop: ${fields.STOP||"?"}`,
          `📈 Net margin: ${fields.NET_MARGIN||"?"}`,
          `💱 ETH: $${fields.ETH_PRICE||"?"}`,
        ] : [
          `📊 <b>${fields.SYM}</b> — SELL #${name.slice(5)} ${isWin ? "✅ PROFIT" : "❌ LOSS"}`,
          `💰 Exit: <b>$${fields.EXIT||"?"}</b> | Entry was: $${fields.ENTRY||"?"}`,
          `📤 Received: ${fields.OUT||"?"}`,
          `${isWin ? "🟢" : "🔴"} P&L: <b>${pnl}</b>`,
          `🐷 Piggy: ${fields.PIGGY||"?"}ETH`,
          `📋 Reason: ${fields.REASON||"?"}`,
          `💱 ETH: $${fields.ETH_PRICE||"?"}`,
        ]).join("\n") + "\n" +
        `🔗 <a href="https://basescan.org/tx/${transactionHash}">View receipt on BaseScan ↗</a>\n` +
        `<i>Input Data → UTF-8 to read full data</i>`
      );
    } else if (isDefault) {
...
async function executeBuy(cdp, token, bal, reason, price, forcedEth = 0, isCascade = false) {
  try {
    ...
    // 📜 On-chain trade receipt — complete traffic report inscribed on Base
    if (BTP_INSCRIPTIONS_ENABLED) {
      try {
        ...
        const stopLoss   = minTr ? (minTr * 0.97).toFixed(8) : "?";
        const fib100     = maxPk ? maxPk.toFixed(8) : "?";
        const fib161     = maxPk ? (minTr + (maxPk - minTr) * 1.618).toFixed(8) : "?";
        const netPct     = (armStatus.net * 100).toFixed(2);
        const usdIn      = (ethToSpend * ethUsd).toFixed(2);
        const tier       = tierLabel;
        const receipt    = [
          `[TRADE:BUY:#${tradeCount}]`,
          `SYM:${token.symbol}`,
          `ENTRY:$${price.toFixed(8)}`,
          `IN:${ethToSpend.toFixed(6)}ETH($${usdIn})`,
          `TIER:${tier}`,
          `NET_MARGIN:${netPct}%`,
          `TARGET:$${fib100}(+${targetPct}%)`,
          `WAVE_EXT:$${fib161}(161.8%)`,
          `STOP:$${stopLoss}`,
          `MIN:$${minTr?.toFixed(8)||"?"}`,
          `MAX:$${maxPk?.toFixed(8)||"?"}`,
          `ETH_PRICE:$${ethUsd.toFixed(2)}`,
          `TX:${txHash}`,
          `VITA:ᛞᚨᚡᛁᛞ|The truth is the chain`,
        ].join("|");
        btpEnqueue(`BUY#${tradeCount}`, receipt);
...
async function executeSell(cdp, token, sellPct, reason, price, isProtective = false) {
  try {
    ...
    // 📜 On-chain trade receipt — complete sell traffic report inscribed on Base
    if (BTP_INSCRIPTIONS_ENABLED) {
      try {
        const entryP   = token.entryPrice || price;
        const pnlPct   = entryP > 0 ? ((price - entryP) / entryP * 100).toFixed(2) : "?";
        const pnlUsd   = (received * ethUsd - (token.totalInvestedEth||0) * sellPct * ethUsd).toFixed(3);
        const win      = parseFloat(pnlUsd) >= 0;
        const receipt  = [
          `[TRADE:SELL:#${tradeCount}]`,
          `SYM:${token.symbol}`,
          `EXIT:$${price.toFixed(8)}`,
          `ENTRY:$${entryP.toFixed(8)}`,
          `PNL:${win ? "+" : ""}${pnlUsd}USD(${win ? "+" : ""}${pnlPct}%)`,
          `OUT:${received.toFixed(6)}ETH($${(received*ethUsd).toFixed(2)})`,
          `SELL_PCT:${(sellPct*100).toFixed(0)}%`,
          `REASON:${reason.slice(0,30)}`,
          `PIGGY:${piggyBank.toFixed(6)}ETH`,
          `ETH_PRICE:$${ethUsd.toFixed(2)}`,
          `TX:${transactionHash}`,
          `RESULT:${win ? "PROFIT✓" : "LOSS✗"}`,
          `VITA:ᛞᚨᚡᛁᛞ|The chain is alive`,
        ].join("|");
        btpEnqueue(`SELL#${tradeCount}`, receipt);
