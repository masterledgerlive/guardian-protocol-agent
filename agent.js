...
// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v17.0 — IKN FILING PROTOCOL
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      // ── 🌟 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        // VITA uses her own vaulted key, falls back to ANTHROPIC_API_KEY in Railway
        const vitaApiKey = process.env.VITA_ANTHROPIC_KEY || process.env.ANTHROPIC_API_KEY;
        if (!vitaApiKey) {
          await tg(
      ...
      //        /vitalearn [paste any text after the command]
        ...
        if (!vitaKey) { await tg("❌ VITA needs ANTHROPIC_API_KEY"); }
        else {
...
VERIFY: c=299792458, Nobel=1921, born=1879-03-14, died=1955-04-18, LIGO detection=2015-09-14`,
        ...
        if (!vitaKey) {
          await tg("🌟 VITA needs ANTHROPIC_API_KEY in Railway to answer");
        ...
        // ── /vita [question] — answer from memory registry ──────────────────
          ...
          try {
            ...
            const isVerified = answer.includes("BLOCKCHAIN VERIFIED");
