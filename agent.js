...
// If a position has moved less than X% over Y minutes, sell a partial slice
// and immediately cascade those proceeds into the best-armed available token.
// Prevents capital being locked indefinitely in sideways positions.
const STALE_MOVE_PCT       = 0.005;  // 0.5% — "no meaningful movement"
const STALE_WINDOW_MS      = 25 * 60_000; // 25 minutes of no movement = stale
const STALE_SELL_PCT       = 0.40;   // sell 40% of stale position
const STALE_MIN_USD        = 2.00;   // only cascade if proceeds ≥ $2
const STALE_COOLDOWN_MS    = 45 * 60_000; // 45min before same token can stale-cascade again
...
const DEFAULT_TOKENS = [
  ...
  { symbol: "BALD",    address: "0x27D2DECb4bFC9C76F0309b8E88dec3a601Fe25a8", feeTier: 3000,  poolFeePct: 0.006, minNetMargin: MIN_NET_MARGIN,
    score: { liquidity:6, waveQuality:7, fundamentals:5, coinbaseFit:6, community:7, total:31 },
    notes: "BALD - the original Base meme. Named after Brian Armstrong. Launched with Base chain 2023. Historic cultural significance." },

  { symbol: "NORMIE",  address: "0x7F12d13B34F5F4f0a9449c16Bcd42f0da47AF200", feeTier: 10000, poolFeePct: 0.010, minNetMargin: 0.010,
    score: { liquidity:5, waveQuality:6, fundamentals:4, coinbaseFit:6, community:7, total:28 },
    notes: "NORMIE - mission to onboard 1B normies to Base. 93K+ holders. Scout tier." },
  ...
  { symbol: "ROOST",   address: "0x2105B0af56f34C13f05D6bCcC15ba884c06F7F74", feeTier: 10000, poolFeePct: 0.010, minNetMargin: 0.010,
    score: { liquidity:5, waveQuality:6, fundamentals:5, coinbaseFit:6, community:7, total:29 },
    notes: "Roost - Base culture meme, Farcaster native. Scout tier." },

  { symbol: "ENJOY",   address: "0xa6b280b42cb0b7c4a4f789eC6cCC3A7609A1Bc39", feeTier: 10000, poolFeePct: 0.010, minNetMargin: 0.010,
    score: { liquidity:5, waveQuality:6, fundamentals:4, coinbaseFit:6, community:7, total:28 },
    notes: "ENJOY - Farcaster tipping currency. Real utility as social reward token. Scout tier." },

  { symbol: "TALENT",  address: "0x4B4B1aA1b8879778d5477e4B7A62c0B0B05fA5B6", feeTier: 3000,  poolFeePct: 0.006, minNetMargin: MIN_NET_MARGIN,
    score: { liquidity:6, waveQuality:6, fundamentals:7, coinbaseFit:7, community:7, total:33 },
    notes: "Talent Protocol - onchain reputation infrastructure on Base. Coinbase aligned. Scout tier." },
];
...
async function loadHistoricalData(days = 90) {
  ...
  for (const token of allTokens) {
    // Skip watchlist placeholders (e.g. FIBER_NODE_PLAY has address "PENDING")
    if (!token.address || token.address === "PENDING" || !token.address.startsWith("0x")) {
      console.log(`   ⏭️  ${token.symbol}: skipping (no valid address)`);
      continue;
    }
    ...
    try {
      ...
      try {
        candles = await fetchCandlesGeckoTerminal(token.address, days);
        if (candles && candles.length >= 7) {
          console.log(`   📡 ${token.symbol}: GeckoTerminal ✅ (${candles.length} candles)`);
        } else {
          gtError = `GeckoTerminal: ${candles ? `only ${candles.length} candles` : "no data / pool not found"}`;
        }
      } catch (e) {
        gtError = `GeckoTerminal: ${e.message}`;
      }

      // ── Fallback to DexScreener if GeckoTerminal failed ──────────────────
      if (!candles || candles.length < 7) {
        await sleep(300); // small gap to avoid rate-limit cascade
        try {
          candles = await fetchCandlesDexScreener(token.address, days);
          if (candles && candles.length >= 7) {
            console.log(`   📡 ${token.symbol}: DexScreener  ✅ (${candles.length} candles) [GT failed: ${gtError}]`);
          } else {
            dsError = `DexScreener: ${candles ? `only ${candles.length} candles` : "no data / pair not found"}`;
          }
        } catch (e) {
          dsError = `DexScreener: ${e.message}`;
        }
      }

      // ── Both sources failed — log detail and skip ─────────────────────────
      if (!candles || candles.length < 7) {
...
async function processToken(cdp, token, bal) {
  try {
    const hasPosition = !!(token.entryPrice);
    const price = await getTokenPrice(token.address, hasPosition);
    ...
    // If RSI ≥ 75 (hard overbought) + BB upper touch + in profit + near peak (within 3%)
    // → sell now. The wave is at its top; waiting for the confirmed drop means selling
    // the decline, not the peak. This is the "missing the 5% round trip" fix.
    ...
    const earlySellSignal = entry
      ...
      && bbData   !== null && price >= bbData.upper * 0.998   // at or above BB upper
      && nearPeak
      && netIfSellNow > breakEvenBuffer * 2  // must be solidly in profit (2x buffer)
      && sellable > 1;
    ...
    const shouldSell = (atMaxPeak || predSell || earlySellSignal) && sellable > 1 && netIfSellNow > breakEvenBuffer;
    ...
    if (entry && balance > 1) {
      ...
      if (!ref) {
        ...
        } else if (elapsed > STALE_WINDOW_MS && coolOk && !shouldSell && !stopLossHit) {
          const staleSellUsd = sellable * price * STALE_SELL_PCT;
          if (staleSellUsd >= STALE_MIN_USD) {
            console.log(`  ⏰ [${token.symbol}] STALE CASCADE — no movement in ${(elapsed/60000).toFixed(0)}min, selling ${(STALE_SELL_PCT*100).toFixed(0)}%`);
            await tg(`⏰ <b>STALE CASCADE — ${token.symbol}</b>\nNo movement for ${(elapsed/60000).toFixed(0)} min (${(moved*100).toFixed(2)}% move)\nSelling ${(STALE_SELL_PCT*100).toFixed(0)}% to redeploy capital\n💰 ~$${staleSellUsd.toFixed(2)} freed up`);
  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      try {
        ...
        await tg(
          `🏄 <b>GUARDIAN PROTOCOL — SURF EDITION</b>\n\n` +
          `<b>Strategy:</b>\n` +
          ...
          `<b>Medal System:</b>\n` +
          `🥇 Gold = +5%+ wave  🥈 Silver = +2.5%+  🥉 Bronze = profit\n` +
          `🦈 Wipeout = loss    〰️🏄〰️ = riding the wave\n\n` +
          `<b>Commands:</b>\n` +
          `/wake (or /gm) — full morning briefing\n` +
          `/bank — complete money statement\n` +
          `/leaderboard — 🏆 wave scoreboard all tokens\n` +
          `/surf — 🌊 current riding positions surf report\n` +
          ...
          `<b>Active tokens (29):</b> AERO BRETT VIRTUAL MORPHO CBBTC DEGEN WELL SEAM AIXBT TOSHI KITE KEYCAT DOGINME XCN SKI PRIME LUNA HIGHER MOCHI ZORA BNKR TYBG MIGGLES BALD NORMIE BENJI ROOST ENJOY TALENT`
        );
      }
      } catch (cmdErr) {
...
async function main() {
  ...
  const BATCH2_SYMBOLS = ["ZORA","BNKR","TYBG","MIGGLES","BALD","NORMIE","BENJI","ROOST","ENJOY","TALENT"];
