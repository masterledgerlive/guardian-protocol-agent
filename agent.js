...
// Processes ikn-strand queue entries with DDC/IKN-X filing + trust stamping
import { bootReader as iknBootReader } from "./ikn-boot-reader.js";
import { processIKNEntry }             from "./ikn-queue-processor.js";
...
// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v18.0 — CHAIN-FIRST / SURGICAL FIX
// ═══════════════════════════════════════════════════════════════════════════════
...
//
// v17.0 IKN FILING PROTOCOL:
//   1. IKN boot reader — reads vita-registry.json at startup, arms Claude context
//   2. IKN queue processor — ikn-strand entries get DDC/IKN-X filing + trust stamp
//   3. CONFIRMED/SUGGESTED/DISPUTED trust levels in registry
//   4. Hive correction protocol — mis-files flagged, never deleted
//   5. Filing stamp on every session handoff
//
...
const MIN_NET_MARGIN       = 0.025;  // 2.5% minimum — covers fees + inscription + buffer

// ── BTP INSCRIPTION TOGGLE ────────────────────────────────────────────────────
// When true: every trade inscribes VITA message on Base (costs extra gas)
// When false: trades only — no inscription overhead
// Toggle via /btpon and /btpoff in Telegram
// Rule: if BTP is ON, its gas cost is included in the profitability gate
let BTP_INSCRIPTIONS_ENABLED = true;
// v18: BTP auto-suspend at very low capital. Gas cost of inscription (~$0.04)
// is too large relative to trade size when wallet < $5 tradeable.
// Auto-off below threshold, auto-on when recovered.
...
const BTP_GAS_UNITS = 50_000; // conservative estimate for inscription tx
...
// savedEarningsUsd is the counting total that Telegram /piggy must match to dust USD.
function syncTokenPiggy(token, balance, priceUsd) {
  ...
  const dustMark = (Number(priceUsd) > 0 && Number(token.piggyReserve) > 0)
    ? Number(token.piggyReserve) * Number(priceUsd)
    : (prior.dustUsd || 0);
  const saved = effectiveSavedEarningsUsd({
    savedEarningsUsd: prior.savedEarningsUsd || token.savedEarningsUsd || 0,
    dustUsd: dustMark,
    piggyMinUsd: piggyBankMinUsd(process.env, piggyOptsFromToken(token)),
  });
  const opts = piggyOptsFromToken(token, { savedEarningsUsd: saved });
  token.piggyReserve = ratchetPiggyReserve(token.piggyReserve, balance, priceUsd, process.env, opts);
  token.savedEarningsUsd = saved;
  prior.dustReserve = token.piggyReserve;
  prior.dustUsd = (Number(priceUsd) > 0) ? token.piggyReserve * Number(priceUsd) : (prior.dustUsd || 0);
  prior.savedEarningsUsd = Math.max(saved, prior.dustUsd || 0);
...
// ══════════════════════════════════════════════════════════════════════════════
// 🏄 SURFER SYSTEM — Named Capital Pools, One Wave Each
// ──────────────────────────────────────────────────────────────────────────────
// Each "Surfer" is a named slice of capital (ETH) that rides ONE token wave.
// Philosophy: a surfer waits for the right wave, paddles in at the lowest trough,
// rides to the peak, exits, rests, and reassigns to the next best incoming wave.
// Unlike the main bot (which manages multiple tokens), each Surfer is focused:
//   - One token at a time
//   - Enters at confirmed MIN trough with high indicator score
//   - Exits at confirmed MAX peak or when wave loses pressure
//   - Never chases — waits for the NEXT clean wave
//   - Reports all P&L under its name (saved to VITA memory)
//
// COMMANDS:
//   /sendsurferout 9          — create surfer with $9 (auto-named)
//   /sendsurferout 9 KAHUNA   — create surfer named KAHUNA with $9
//   /surfers                  — list all surfers, wave, P&L
//   /surfer KAHUNA            — detailed status of one surfer
//   /surferout KAHUNA         — manually retire a surfer (sell + return to pool)
//   /surferstats              — lifetime stats across all retired surfers
// ══════════════════════════════════════════════════════════════════════════════

// Surfer name pool — auto-assigned in order when no name given
const SURFER_NAMES = [
  "KAHUNA", "BARREL", "GROM", "MAVERICKS", "PIPELINE", "NAZARE",
  "TEAHUPOO", "JAWS", "GHOST", "DHARMA", "RINCON", "SUPERBANK",
  "ULUWATU", "SNAPPER", "PADANG", "KIRRA", "HOSSEGOR", "TOFINO",
];
let surferNameIdx  = 0;
const surfers      = {};  // { name: { ethAlloc, symbol, entryPrice, entryTime, status, pnlEth, pnlUsd, trades, retiredAt } }
const surferHistory = []; // retired surfer records for /surferstats

function nextSurferName() {
  const name = SURFER_NAMES[surferNameIdx % SURFER_NAMES.length] +
               (surferNameIdx >= SURFER_NAMES.length ? `-${Math.floor(surferNameIdx/SURFER_NAMES.length)+1}` : "");
  surferNameIdx++;
  return name;
}

// Create a new surfer with a given ETH allocation
// Finds the best current wave to assign it to (highest-scoring armed token)
function createSurfer(ethAlloc, name, ethUsd) {
  const surferName = name || nextSurferName();
  if (surfers[surferName]) return { error: `Surfer ${surferName} already exists` };

  // Find best wave to assign — top armed token by indicator score + net margin
  const candidates = tokens
    .filter(t => !t.frozen && !t.disabled)
    .map(t => {
      const arm = getArmStatus(t.symbol, 0, ethAlloc);
      const ind = getIndicatorScore(t.symbol);
      if (!arm.armed) return null;
      return { symbol: t.symbol, score: (arm.net * 100) + ind.score, net: arm.net };
    })
    .filter(Boolean)
    .sort((a, b) => b.score - a.score);

  const wave = candidates[0]?.symbol || null;

  surfers[surferName] = {
    ethAlloc,
    usdAlloc: ethAlloc * ethUsd,
    symbol:     wave,
    entryPrice: null,
    entryTime:  null,
    status:     wave ? "WAITING" : "IDLE",    // WAITING → RIDING → RESTING → WAITING
    pnlEth:     0,
    pnlUsd:     0,
    trades:     0,
    totalIn:    0,
  ...
  console.log(`🏄 NEW SURFER [${surferName}] ${ethAlloc.toFixed(6)} ETH ($${(ethAlloc*ethUsd).toFixed(2)}) → wave: ${wave || "IDLE (no arms available)"}`);
  return { name: surferName, ethAlloc, symbol: wave, status: surfers[surferName].status };
}

// Get surfer status line for display
function surferStatusLine(name, ethUsd) {
  const s = surfers[name];
  if (!s) return `❓ ${name}: not found`;
  const price = s.symbol ? (history[s.symbol]?.lastPrice || 0) : 0;
  const nowUsd = s.entryPrice && price ? (s.ethAlloc / (s.entryPrice / price)) * price * ethUsd : s.usdAlloc;
  const pnlStr = s.pnlUsd >= 0 ? `+$${s.pnlUsd.toFixed(3)}` : `-$${Math.abs(s.pnlUsd).toFixed(3)}`;

  const statusEmoji = { WAITING: "⏳", RIDING: "🏄", RESTING: "😴", IDLE: "💤", RETIRED: "🏆" }[s.status] || "?";
  return `${statusEmoji} <b>${name}</b> — $${(s.ethAlloc*ethUsd).toFixed(2)} | ${s.symbol || "IDLE"} | P&L: ${pnlStr} | ${s.trades} rides`;
}

// Process surfer decisions each main loop cycle
// Called once per cycle after token prices are refreshed
async function processSurfers(cdp, bal, ethUsd) {
  if (Object.keys(surfers).length === 0) return;

  const gasCost = await estimateGasCostEth();

  for (const [name, s] of Object.entries(surfers)) {
    if (s.status === "IDLE" || s.status === "RETIRED") continue;

    // If no wave assigned yet, try to find one
    if (!s.symbol) {
      const candidates = tokens
        .filter(t => !t.frozen && !t.disabled)
        .map(t => {
          const arm = getArmStatus(t.symbol, gasCost, s.ethAlloc);
          if (!arm.armed) return null;
          const ind = getIndicatorScore(t.symbol);
          return { symbol: t.symbol, score: (arm.net * 100) + ind.score };
        })
        .filter(Boolean)
        .sort((a, b) => b.score - a.score);
      if (candidates.length > 0) {
        s.symbol = candidates[0].symbol;
        s.status = "WAITING";
        console.log(`🏄 [${name}] Found wave: ${s.symbol}`);
      }
      continue;
    }

    const price = history[s.symbol]?.lastPrice || 0;
    if (!price) continue;

    const arm  = getArmStatus(s.symbol, gasCost, s.ethAlloc);
    const ind  = getIndicatorScore(s.symbol);
    const minT = getMinTrough(s.symbol);
    const maxP = getMaxPeak(s.symbol);

    if (s.status === "WAITING") {
      // Use 4-wave prediction model for entry — spot call or predicted trough
      computeWavePrediction(s.symbol, price, ethUsd);
      const sig = getSurferEntrySignal(s.symbol, price, ethUsd);
      const calEvents = getCalendarPressure();
      const calNote = calEvents.length ? calEvents.map(e => e.note).join(", ") : "";

      if (sig.shouldEnter && sig.confidence >= 55) {
        s.entryPrice  = price;
        s.entryTime   = Date.now();
        s.status      = "RIDING";
        s.totalIn    += s.ethAlloc;
        s.waveTarget  = sig.targetPrice;
        s.waveEntry   = sig.wave1;
        s.waveNum     = (s.waveNum || 0) + 1;
        s.trades++;
        const entryType = sig.spotBuy ? "🚀 SPOT CALL" : "🎯 PREDICTED TROUGH";
        console.log(`🏄 [${name}] ${entryType} → ${s.symbol} @ $${price.toFixed(8)} conf:${sig.confidence}% $${(s.ethAlloc*ethUsd).toFixed(2)}`);
        await tg(`🏄 <b>SURFER ${name} — ${entryType}</b>\n` +
          `Token: <b>${s.symbol}</b> @ $${price.toFixed(8)}\n` +
          `Capital: $${(s.ethAlloc*ethUsd).toFixed(2)} | Confidence: ${sig.confidence}%\n` +
          `Target: $${sig.targetPrice?.toFixed(8) || "?"} | Wave #${s.waveNum}\n` +
          `Signal: ${sig.reasons}` +
          (calNote ? `\nCalendar: ${calNote}` : "") +
          (sig.dowContext ? `\nDay bias: ${sig.dowContext.note}` : ""));
      } else {
        if (!s._lastWaitLog || Date.now() - s._lastWaitLog > 120_000) {
          console.log(`🏄 [${name}] WAITING ${s.symbol} conf:${sig.confidence}% | ${sig.reasons}`);
          s._lastWaitLog = Date.now();
        }
      }

        } else if (s.status === "RIDING") {
      if (!s.entryPrice) { s.status = "WAITING"; continue; }
      const pct    = (price - s.entryPrice) / s.entryPrice;
      const atPeak = maxP && price >= maxP * 0.99;
      const waveLost = ind.macd?.crossDown && ind.rsi !== null && ind.rsi > 55 && pct > 0.01;
      const stopLoss = pct < -STOP_LOSS_PCT;
      const kahuna   = getKahunaSignal(s.symbol);

      // Ride kahuna all the way — don't exit early on a big wave
      if (atPeak || (waveLost && !kahuna?.active) || stopLoss) {
        // Score this wave against prediction model before clearing entry
        if (s.waveEntry) scoreCompletedWave(s.symbol, Math.min(s.entryPrice || price, price), Math.max(s.entryPrice || price, price));

        const exitReason = stopLoss ? "🛑 STOP LOSS" : atPeak ? "🎯 PEAK EXIT" : "📉 WAVE LOST MOMENTUM";
        const grossEth   = s.ethAlloc * (1 + pct);
        const feeEth2    = grossEth * (tokens.find(t=>t.symbol===s.symbol)?.poolFeePct || 0.006) * 2;
        const netEth2    = grossEth - feeEth2 - gasCost;
        const pnl2       = (netEth2 - s.ethAlloc) * ethUsd;
        const newEthAlloc = Math.max(netEth2, s.ethAlloc * 0.5); // floor at 50%
        s.pnlEth   += (netEth2 - s.ethAlloc);
        s.pnlUsd   += pnl2;
        s.totalOut += netEth2;
        s.lastWave  = s.symbol;
        s.ethAlloc  = newEthAlloc; // compound — capital grows with wins
        s.status    = pnl2 > 0 ? "RESTING" : "WAITING";
        s.entryPrice = null;
        s.entryTime  = null;
        s.waveEntry  = null;
        s.waveTarget = null;

        const predScore2 = predictionScores[s.symbol];
        const accStr2 = predScore2 ? `Acc: ${predScore2.hits}/${predScore2.count}` : "";
        console.log(`🏄 [${name}] EXITING ${s.symbol} ${exitReason} | P&L: ${pnl2>=0?"+":""}$${pnl2.toFixed(3)} | Cap: $${(newEthAlloc*ethUsd).toFixed(2)} ${accStr2}`);
        await tg(`🏄 <b>SURFER ${name} — WAVE ${s.waveNum||"?"} COMPLETE</b>\n` +
          `${exitReason}\n` +
          `Token: <b>${s.symbol}</b> | Entry: $${s.entryPrice?.toFixed(8) || "?"}\n` +
          `Exit: $${price.toFixed(8)} | Move: ${(pct*100)>=0?"+":""}${(pct*100).toFixed(2)}%\n` +
          `P&L this wave: ${pnl2>=0?"🟢 +$":"🔴 -$"}${Math.abs(pnl2).toFixed(3)}\n` +
          `Capital (compounding): $${(newEthAlloc*ethUsd).toFixed(2)}\n` +
          `Lifetime: ${s.pnlUsd>=0?"+$":"-$"}${Math.abs(s.pnlUsd).toFixed(3)} over ${s.trades} waves | ${accStr2}`);

        // Auto-find next best wave — highest prediction confidence
        const nextWave = tokens
          .filter(t => !t.frozen && !t.disabled)
          .map(t => {
            computeWavePrediction(t.symbol, history[t.symbol]?.lastPrice || 0, ethUsd);
            const sig = getSurferEntrySignal(t.symbol, history[t.symbol]?.lastPrice || 0, ethUsd);
            return { symbol: t.symbol, confidence: sig.confidence };
          })
          .filter(t => t.confidence > 0)
          .sort((a, b) => b.confidence - a.confidence);

        s.symbol = nextWave[0]?.symbol || null;
        s._lastWaitLog = null;

      } else {
        // Still riding — show progress in console every minute
        if (!s._lastRideLog || Date.now() - s._lastRideLog > 60_000) {
          const pctNow = s.entryPrice ? ((price - s.entryPrice) / s.entryPrice * 100) : 0;
          const toTarget = s.waveTarget ? ((s.waveTarget - price) / price * 100) : 0;
          console.log(`🏄 [${name}] RIDING ${s.symbol} | ${pctNow>=0?"+":""}${pctNow.toFixed(2)}% | target ${toTarget.toFixed(2)}% away`);
          s._lastRideLog = Date.now();
        }
      }

    } else if (s.status === "RESTING") {
      // Rest 2 full cycles after a win before looking for next wave
      if (!s.restUntil) { s.restUntil = Date.now() + 30_000; }
      if (Date.now() > s.restUntil) {
        s.restUntil = null;
        s.status    = "WAITING";
        s.symbol    = null; // will reassign next cycle
        console.log(`🏄 [${name}] Rested — scanning for next wave`);
      }
    }
  }
}

// Retire a surfer — sell position (if riding) and return ETH to main pool
async function retireSurfer(cdp, name, ethUsd) {
  const s = surfers[name];
  if (!s) return `❓ Surfer ${name} not found`;

  const record = { ...s, retiredAt: new Date().toISOString(), name };
  surferHistory.push(record);
  delete surfers[name];

  const msg = `🏆 <b>SURFER ${name} RETIRED</b>\n` +
    `Capital returned: $${(record.ethAlloc*ethUsd).toFixed(2)}\n` +
    `Lifetime P&L: ${record.pnlUsd>=0?"+$":"-$"}${Math.abs(record.pnlUsd).toFixed(3)}\n` +
    `Total waves ridden: ${record.trades}`;
  console.log(`🏄 RETIRED surfer [${name}]`);
  return msg;
}
...
// ══════════════════════════════════════════════════════════════════════════════
// 🔮 WAVE PREDICTION ENGINE v18.0 — 4-Wave Forward Projection
// ──────────────────────────────────────────────────────────────────────────────
// Each token gets a 4-wave prediction every cycle:
//   Wave 1: next trough → peak (immediate)
//   Wave 2: wave 1 peak → next trough (the reload zone)
//   Wave 3: wave 2 trough → next peak
//   Wave 4: wave 3 peak → next trough (the far horizon)
//
// INPUTS that feed the model:
//   - Historical OHLC candles (daily highs/lows per timeframe)
//   - Live RSI, MACD, BB squeeze state
//   - Day-of-week bias (Monday open / Friday close / Tuesday Trump patterns)
//   - Calendar pressure (tax dates, month-end, quarter-end)
//   - Wave amplitude history (avg peak-to-trough % for THIS token)
//   - BTC/ETH trend (macro context from CBBTC price direction)
//
// SCORING: Each completed wave gets a prediction score:
//   actual_high vs predicted_high, actual_low vs predicted_low
//   Score 0-100: 100 = perfect, 70+ = good, <50 = recalibrate model
// ══════════════════════════════════════════════════════════════════════════════

const wavePredictions  = {};  // { [symbol]: { waves: [W1,W2,W3,W4], score, updatedAt } }
const predictionScores = {};  // { [symbol]: { hits, misses, avgError } }

// ── Day-of-week bias constants ────────────────────────────────────────────────
// Based on empirical crypto market patterns:
// Monday: opening pressure — often gap up or down from weekend, then reverses
// Friday: end-of-week — sell pressure builds as people de-risk for weekend
// Tuesday: historically volatile after Monday settlement
// Wednesday/Thursday: trend continuation days — most reliable waves
const DOW_BIAS = {
  0: { name: "Sunday",    buyMod: -0.05, sellMod: -0.03, note: "Weekend illiquidity — thin book" },
  1: { name: "Monday",    buyMod: +0.08, sellMod: -0.06, note: "Monday open — gap fill + reversal. Often best buy day." },
  2: { name: "Tuesday",   buyMod: +0.03, sellMod: +0.04, note: "Trump/news volatility. Check sentiment before entry." },
  3: { name: "Wednesday", buyMod: +0.05, sellMod: +0.05, note: "Mid-week trend — clean waves, reliable signal." },
  4: { name: "Thursday",  buyMod: +0.04, sellMod: +0.06, note: "Pre-Friday positioning — good exit day." },
  5: { name: "Friday",    buyMod: -0.04, sellMod: +0.08, note: "Weekend de-risk — sell high, don't buy late." },
  6: { name: "Saturday",  buyMod: -0.02, sellMod: -0.04, note: "Weekend — lower volume, wider spreads." },
};

// ── Calendar pressure events ──────────────────────────────────────────────────
// Days that force Americans to spend/sell → create predictable price pressure
function getCalendarPressure() {
  const now  = new Date();
  const month = now.getMonth() + 1; // 1-12
  const day   = now.getDate();
  const events = [];

  // Tax day pressure (mid-April US) → sell pressure on crypto to pay taxes
  if (month === 4 && day >= 10 && day <= 20) {
    events.push({ type: "TAX_SELL", severity: 0.7, note: "US tax week — forced crypto selling likely" });
  }
  // Quarter end (March 31, June 30, Sep 30, Dec 31) → institutional rebalancing
  if ((month === 3 || month === 6 || month === 9 || month === 12) && day >= 28) {
    events.push({ type: "QUARTER_END", severity: 0.5, note: "Quarter-end rebalancing — expect volatility" });
  }
  // Month end (last 3 days) → sell pressure for rent/bills
  if (day >= 28) {
    events.push({ type: "MONTH_END", severity: 0.3, note: "Month-end bill pressure — mild sell bias" });
  }
  // Month start (1st-3rd) → fresh capital inflow
  if (day <= 3) {
    events.push({ type: "MONTH_START", severity: 0.3, note: "Fresh monthly capital — buy bias" });
  }
  // First trading day after US holiday — gap fills
  // (simplified: track common US holidays)
  return events;
}

// ── Compute avg wave amplitude for a token from historical peaks/troughs ──────
function getAvgWaveAmplitude(symbol) {
  const ws = waveStats[symbol];
  const h  = history[symbol];
  if (!ws || !h) return 0.05; // default 5% if no data

  // Use historical peak/trough pairs to compute avg move
  const peaks   = ws.peaks   || [];
  const troughs = ws.troughs || [];
  if (peaks.length < 2 || troughs.length < 2) {
    // Fall back to candle range
    const candles = h.candles;
    if (candles?.range90) {
      const range90 = (candles.range90.high - candles.range90.low) / candles.range90.low;
      return range90 * 0.3; // estimate single wave as ~30% of 90d range
    }
    return 0.05;
  }

  // Compute peak-to-prior-trough amplitudes
  const amplitudes = [];
  for (let i = 1; i < Math.min(peaks.length, troughs.length); i++) {
    const amp = (peaks[i] - troughs[i - 1]) / troughs[i - 1];
    if (amp > 0 && amp < 5) amplitudes.push(amp); // filter outliers
  }
  if (!amplitudes.length) return 0.05;
  return amplitudes.reduce((s, a) => s + a, 0) / amplitudes.length;
}

// ── Main prediction function ──────────────────────────────────────────────────
function computeWavePrediction(symbol, currentPrice, ethUsd) {
  const h     = history[symbol];
  const ws    = waveStats[symbol];
  const ind   = getIndicatorScore(symbol);
  if (!h || !currentPrice) return null;

  const amp        = getAvgWaveAmplitude(symbol);
  const minTrough  = getMinTrough(symbol)  || currentPrice * (1 - amp);
  ...
  let buyAmp  = amp;
  let sellAmp = amp;
  buyAmp  *= (1 + dowBias.buyMod);
  sellAmp *= (1 + dowBias.sellMod);
  for (const ev of calPress) {
    if (ev.type === "TAX_SELL" || ev.type === "MONTH_END") {
      sellAmp *= (1 + ev.severity * 0.3);
      buyAmp  *= (1 - ev.severity * 0.2);
    } else if (ev.type === "MONTH_START") {
      buyAmp  *= (1 + ev.severity * 0.3);
    }
  }

  // RSI/MACD adjustment — if oversold now, next trough may be shallower
  let troughBias = 1.0;
  let peakBias   = 1.0;
  if (ind.rsi !== null) {
    if (ind.rsi < 30)  { troughBias = 0.7; peakBias = 1.15; } // oversold → shallower next trough, higher peak
    if (ind.rsi > 70)  { troughBias = 1.2; peakBias = 0.85; } // overbought → deeper next trough, lower peak
    if (ind.rsi > 80)  { troughBias = 1.4; peakBias = 0.75; } // very overbought → expect big drop
  }
  if (ind.macd?.crossUp)   peakBias   *= 1.1;
  if (ind.macd?.crossDown) troughBias *= 1.2;

  // BB squeeze — coiled spring, expect breakout
  const bbSqueeze = ind.bb?.squeeze;
  if (bbSqueeze) {
    peakBias   *= 1.2;
    troughBias *= 0.9;
  }

  // Determine wave direction from current position in range
  const rangePos = maxPeak > minTrough ? (currentPrice - minTrough) / (maxPeak - minTrough) : 0.5;

  // Where is the NEXT trough predicted?
  // If we're currently high in the range (rangePos > 0.6) → predict drop first
  // If we're low in the range (rangePos < 0.4) → predict rise first
  const goingDown = rangePos > 0.55 || (ind.rsi !== null && ind.rsi > 65);

  // Wave projections:
  let w1Low, w1High, w2Low, w2High, w3Low, w3High, w4Low, w4High;

  if (goingDown) {
    // Current → trough first → peak → trough → peak
    w1Low  = currentPrice * (1 - buyAmp * troughBias);      // next trough
    w1High = w1Low * (1 + sellAmp * peakBias);               // peak after that trough
    w2Low  = w1High * (1 - buyAmp * troughBias * 0.9);       // second trough (slightly shallower)
    w2High = w2Low * (1 + sellAmp * peakBias * 1.05);        // second peak (slightly higher)
    w3Low  = w2High * (1 - buyAmp * troughBias * 0.85);
    w3High = w3Low  * (1 + sellAmp * peakBias * 1.1);
    w4Low  = w3High * (1 - buyAmp * troughBias * 0.8);
    w4High = w4Low  * (1 + sellAmp * peakBias * 1.15);
  } else {
    // Currently at/near trough → peak first → trough → peak → trough
    w1High = currentPrice * (1 + sellAmp * peakBias);
    w1Low  = w1High * (1 - buyAmp * troughBias);
    w2High = w1Low  * (1 + sellAmp * peakBias * 1.05);
    w2Low  = w2High * (1 - buyAmp * troughBias * 0.9);
    w3High = w2Low  * (1 + sellAmp * peakBias * 1.1);
    w3Low  = w3High * (1 - buyAmp * troughBias * 0.85);
    w4High = w3Low  * (1 + sellAmp * peakBias * 1.15);
    w4Low  = w4High * (1 - buyAmp * troughBias * 0.8);
  }

  // Clip to known all-time range from candle data (don't predict outside history)
  const candles = h.candles;
  const allTimeHigh = candles?.range90?.high || currentPrice * 3;
  const allTimeLow  = candles?.range90?.low  || currentPrice * 0.1;

  const clip = (v) => Math.max(allTimeLow * 0.5, Math.min(allTimeHigh * 2, v));

  // Apply macro altcoin bias to amplitude — BTC trend affects how big waves will be
  const macroBias = (macroSignal?.altcoinBias || 1.0);
  const waves = [
    { wave: 1, low: clip(w1Low), high: clip(w1High), goDown: goingDown, amp: amp * buyAmp * macroBias, predicted: true, macroBias },
    { wave: 2, low: clip(w2Low), high: clip(w2High), goDown: !goingDown, amp: amp * buyAmp * macroBias, predicted: true, macroBias },
    { wave: 3, low: clip(w3Low), high: clip(w3High), goDown: goingDown,  amp: amp * buyAmp * macroBias, predicted: true, macroBias },
    { wave: 4, low: clip(w4Low), high: clip(w4High), goDown: !goingDown, amp: amp * buyAmp * macroBias, predicted: true, macroBias },
  ];
  ...
  const prediction = {
    ...
    goingDown,
    waves,
    entryTarget: goingDown ? clip(w1Low) : currentPrice * 1.002, // entry = predicted trough OR right now if going up
    exitTarget:  goingDown ? clip(w1High) : clip(w1High),
    dowContext:  dowBias,
    calEvents:   calPress,
    amp,
    rsi:         ind.rsi,
  ...
  wavePredictions[symbol] = prediction;
  return prediction;
}

// ── Score a completed wave against its prediction ──────────────────────────────
function scoreCompletedWave(symbol, actualLow, actualHigh) {
  const pred = wavePredictions[symbol];
  if (!pred?.waves?.length) return;

  const w = pred.waves[0]; // score the first predicted wave
  const lowErr  = Math.abs((actualLow  - w.low)  / w.low);
  const highErr = Math.abs((actualHigh - w.high) / w.high);
  const avgErr  = (lowErr + highErr) / 2;
  const score   = Math.round(Math.max(0, 100 - avgErr * 500)); // 500x penalty for % error

  if (!predictionScores[symbol]) predictionScores[symbol] = { hits: 0, misses: 0, totalErr: 0, count: 0 };
  const ps = predictionScores[symbol];
  ps.count++;
  ps.totalErr += avgErr;
  ps.avgErr    = ps.totalErr / ps.count;
  if (score >= 60) ps.hits++; else ps.misses++;

  console.log(`🔮 [${symbol}] Wave score: ${score}/100 | low err ${(lowErr*100).toFixed(1)}% | high err ${(highErr*100).toFixed(1)}% | accuracy ${ps.hits}/${ps.count}`);
  // Shift to next predicted wave now that W1 is done
  pred.waves.shift();
}

// ── Get best entry window for a token ─────────────────────────────────────────
// "Right before the wave" — not at the tip, but at the predicted trough entry
// Returns: { shouldEnter, confidence, entryPrice, targetPrice, reason }
function getSurferEntrySignal(symbol, currentPrice, ethUsd) {
  const pred = wavePredictions[symbol];
  if (!pred) return { shouldEnter: false, confidence: 0, reason: "no prediction" };

  const w1    = pred.waves?.[0];
  if (!w1) return { shouldEnter: false, confidence: 0, reason: "waves consumed" };

  const ind   = getIndicatorScore(symbol);
  const arm   = getArmStatus(symbol, 0, 0.001);
  let confidence = 0;
  const reasons = [];

  // Is price at/near predicted trough entry?
  // FIX v18: if recent 5-tick move is DOWNWARD (price falling), the trough is
  // probably NOT here yet — adjust entry target lower before scoring confidence.
  // This prevents buying into a rapid drop that hasn't bottomed.
  const recentTicks = (history[symbol]?.readings || []).slice(-5).map(r => r.price);
  const recentFallPct = recentTicks.length >= 3
    ? (recentTicks[recentTicks.length-1] - recentTicks[0]) / recentTicks[0]
    : 0;
  const fallingFast = recentFallPct < -0.008; // falling > 0.8% in last 5 ticks = still going down
  const adjustedEntryTarget = fallingFast
    ? pred.entryTarget * (1 - Math.abs(recentFallPct) * 2)  // push predicted entry lower
    : pred.entryTarget;

  const nearEntry = currentPrice <= adjustedEntryTarget * 1.02;
  if (nearEntry && !fallingFast) { confidence += 30; reasons.push("at predicted entry zone"); }
  if (nearEntry && fallingFast)  { confidence -= 15; reasons.push(`⚠️ still falling ${(recentFallPct*100).toFixed(2)}% — wait for lower low`); }

  // RSI oversold?
  if (ind.rsi !== null && ind.rsi < 40) { confidence += 20; reasons.push(`RSI oversold ${ind.rsi.toFixed(0)}`); }
  if (ind.rsi !== null && ind.rsi < 30) { confidence += 10; reasons.push("extreme oversold"); }

  // MACD turning up?
  if (ind.macd?.crossUp) { confidence += 20; reasons.push("MACD cross UP"); }

  // BB squeeze releasing?
  if (ind.bb?.squeeze && ind.macd?.crossUp) { confidence += 15; reasons.push("BB squeeze release"); }

  // Day-of-week Monday bonus
  const dow = new Date().getDay();
  if (dow === 1) { confidence += 10; reasons.push("Monday open bias"); }
  if (dow === 3 || dow === 4) { confidence += 5; reasons.push("mid-week clean signal"); }
  if (dow === 5) { confidence -= 10; reasons.push("Friday — exit bias, not entry"); }

  // Armed with real margin?
  if (arm.armed) { confidence += 15; reasons.push(`armed ${(arm.net*100).toFixed(1)}% margin`); }

  // Price going UP rapidly (spot buy opportunity — instant wave)?
  const recentReadings = (history[symbol]?.readings || []).slice(-5).map(r => r.price);
  const recentMove = recentReadings.length >= 2
    ? (recentReadings[recentReadings.length-1] - recentReadings[0]) / recentReadings[0]
    : 0;
  const spotBuy     = recentMove >= 0.005; // 0.5% move UP in last 5 ticks = instant wave forming
  const rapidlyFalling = recentMove < -0.010; // >1% drop = don't enter, wait for bottom

  if (spotBuy)       { confidence += 25; reasons.push(`🚀 SPOT CALL +${(recentMove*100).toFixed(2)}% in last 5 ticks`); }
  if (rapidlyFalling){ confidence -= 30; reasons.push(`⬇️ FALLING FAST ${(recentMove*100).toFixed(2)}% — wait for new low`); }

  return {
    shouldEnter: confidence >= 55,
    confidence: Math.min(100, confidence),
    entryPrice:  currentPrice,
    targetPrice: pred.exitTarget,
    wave1:       w1,
    reasons:     reasons.join(" | "),
    spotBuy,
    dowContext:  pred.dowContext,
  };
}
...
const DEFAULT_TOKENS = [
  ...
  { symbol: "GAME",    address: "0x1C4CcA7C5DB003824208aDDA61Bd749e55F463a3", feeTier: 10000, poolFeePct: 0.010, minNetMargin: 0.010,
    frozen: true, frozenReason: "CUT class — ghost Uni V3 WETH. Liquid book is Uni V2 GAME/VIRTUAL 0xD418dfE7670c21F682E041F34250c114DB5D7789 (~$2.14M); Uni V3 GAME/WETH feeTier 3000 0x70fbffe3… liquidity()=0 / ghost (STF buys hit the wrong book). Catalog fee 10000 quotes the live 1% book first. Not a hitch seat. Exits-only (always-plus leftover sells + piggy dust still apply).",
    score: { liquidity:7, waveQuality:7, fundamentals:8, coinbaseFit:8, community:7, total:37 },
    notes: "GAME by Virtuals — CUT class (ghost Uni V3). FROZEN exits-only — thin Uni V3 WETH vs liquid Uni V2 GAME/VIRTUAL." },
...
// ── RPC ROTATION ──────────────────────────────────────────────────────────────
// Env first: BASE_RPC || RPC_URL || BASE_RPC_URL. Official Base first; meowrpc/drpc last (429).
...
// Reads (price checks, balance queries) use normal RPC — only swaps use MEV protection
// Flashbots Protect Base: https://base.flashbots.net
// If this RPC fails, falls back to CDP's own routing (already somewhat protected)
const MEV_PROTECTED_RPC = "https://base.flashbots.net";
let rpcIndex = 0;
...
function raceWithTimeout(promise, ms = 6000, label = "rpc timeout 6s") {
  ...
  // Timeout winning must not leave simulate()/RPC as an unhandled rejection.
  work.catch(() => {});
...
async function estimateGasCostEth() {
  ...
  try {
    const gasPrice = await rpcCall(c => c.getGasPrice());
    // FIX: Use 500_000 gas units — BTP calldata on swaps requires 435k+
    // Previous 300_000 underestimated causing buy failures
    cachedGasCostEth   = Number(gasPrice * BigInt(500_000)) / 1e18;
...
function updatePortfolioPeak(currentUsd) {
  // FIX v18: If portfolio is near-zero, reset the peak so a ghost halt from
  // a previous session doesn't permanently block buys after a refund.
  // Any time wallet is < $1 total, we treat it as a clean slate.
  if (currentUsd < 1.00) {
    if (portfolioPeakUsd > 0 && drawdownHaltActive) {
      console.log(`🔄 Portfolio near-zero ($${currentUsd.toFixed(2)}) — resetting drawdown peak. Ghost halt cleared.`);
      tg(`🔄 <b>DRAWDOWN PEAK RESET</b>\nPortfolio < $1 — ghost halt cleared.\nBot ready for new capital.`).catch(()=>{});
    }
    portfolioPeakUsd   = 0;
...
const STRAND_SIZE  = 5;
let strandTrades   = [];  // accumulates trade data for current strand
let strandNum      = 1;   // current strand number
...
async function appendToLedger(entry) {
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      const sha    = lf?.sha || null;

      ledger.trades.push(entry);
      ledger.lastUpdated = new Date().toISOString();
      ledger.totalTrades = ledger.trades.length;

      const sells = ledger.trades.filter(t => t.type === "SELL");
      const wins  = sells.filter(t => t.netUsd > 0);
      ledger.stats = {
        totalTrades:  ledger.trades.length,
        sells:        sells.length,
        wins:         wins.length,
        winRate:      sells.length > 0 ? ((wins.length/sells.length)*100).toFixed(1)+"%" : "n/a",
        totalNetUsd:  sells.reduce((s,t) => s + (t.netUsd||0), 0).toFixed(2),
        totalSkimEth: sells.reduce((s,t) => s + (t.skimEth||0), 0).toFixed(6),
      };

      await githubSaveToState("ledger.json", ledger, sha);
      console.log(`📖 Ledger: trade #${ledger.totalTrades} recorded permanently (${entry.type} ${entry.symbol})`);
      return; // success
    } catch (e) {
      console.log(`⚠️  Ledger append attempt ${attempt}/3 failed: ${e.message}`);
...
// Macro BTC/ETH trend signal — fetches weekly candles from Binance
// Used to boost/dampen all token predictions when macro is strongly trending
const macroSignal = { btcTrend: 0, ethTrend: 0, altcoinBias: 0, updatedAt: 0 };
async function refreshMacroSignal() {
  try {
    const [btcRes, ethRes] = await Promise.allSettled([
      fetch("https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1w&limit=12", { signal: AbortSignal.timeout(5000) }),
      fetch("https://api.binance.com/api/v3/klines?symbol=ETHUSDT&interval=1w&limit=12", { signal: AbortSignal.timeout(5000) }),
    ]);
    const parseTrend = async (res) => {
      if (res.status !== "fulfilled" || !res.value.ok) return 0;
      const raw = await res.value.json();
      if (!Array.isArray(raw) || raw.length < 4) return 0;
      const closes = raw.map(k => parseFloat(k[4]));
      const recent = closes.slice(-4);
      const older  = closes.slice(-8, -4);
      const recentAvg = recent.reduce((s,v)=>s+v,0)/recent.length;
      const olderAvg  = older.reduce((s,v)=>s+v,0)/older.length;
      return (recentAvg - olderAvg) / olderAvg; // positive = uptrend, negative = downtrend
    };
    macroSignal.btcTrend = await parseTrend(btcRes);
    macroSignal.ethTrend = await parseTrend(ethRes);
    // Altcoin bias: strong BTC uptrend = alts follow; extreme BTC pump = alts lag
    const btcAbs = Math.abs(macroSignal.btcTrend);
    if (macroSignal.btcTrend > 0.10)       macroSignal.altcoinBias =  0.8; // BTC pumping hard — alts may lag
    else if (macroSignal.btcTrend > 0.03)  macroSignal.altcoinBias =  1.2; // BTC gentle rise — alts follow
    else if (macroSignal.btcTrend < -0.10) macroSignal.altcoinBias =  0.5; // BTC crashing — alts crash harder
    else if (macroSignal.btcTrend < -0.03) macroSignal.altcoinBias =  0.8; // BTC weak — alts weak
    else                                   macroSignal.altcoinBias =  1.0; // neutral
    macroSignal.updatedAt = Date.now();
    console.log(`📡 Macro: BTC ${macroSignal.btcTrend>=0?"+":""}${(macroSignal.btcTrend*100).toFixed(1)}% | ETH ${macroSignal.ethTrend>=0?"+":""}${(macroSignal.ethTrend*100).toFixed(1)}% | AltBias: ${macroSignal.altcoinBias.toFixed(2)}x`);
  } catch (e) {
    console.log(`  ⚠️  Macro signal refresh failed: ${e.message}`);
  }
}

// Master fetcher — tries all sources in priority order, returns best result
// Priority: GeckoTerminal (Base-native) → DexScreener → Binance (for mainstream tokens)
async function fetchHistoricalCandles(tokenAddress, days = 90, symbol = null) {
  // Try GeckoTerminal first (most reliable OHLC on Base)
...
// ══════════════════════════════════════════════════════════════════════════════
// 🔮 FOUR-WAVE LOOKAHEAD — Projects next 4 peak/trough cycles
// ──────────────────────────────────────────────────────────────────────────────
// Uses recent wave history (peaks + troughs) to project:
//   - Timing: average bars between peaks/troughs
//   - Price: average amplitude, projected from current price
//   - Confidence: how consistent the last N waves were
// Scores each projected wave on whether we hit our predicted mark (for learning)
// This feeds into buy/sell decisions — "wave 2 trough is ~20min away at ~$X"
// ══════════════════════════════════════════════════════════════════════════════

const waveProjections = {}; // { [symbol]: { waves: [WaveProjection], lastUpdated, score } }

function buildFourWaveProjection(symbol) {
  const ws = waveStats[symbol];
  if (!ws) return null;

  const peaks   = waveState[symbol]?.peaks   || [];
  const troughs = waveState[symbol]?.troughs || [];
  const readings = history[symbol]?.readings || [];
  if (peaks.length < 2 || troughs.length < 2 || readings.length < 10) return null;

  const price = readings[readings.length - 1]?.price || 0;
  if (!price) return null;

  // Compute average wave metrics from last 4 peaks/troughs
  const recentPeaks   = peaks.slice(-4);
  const recentTroughs = troughs.slice(-4);
  const maxPk  = Math.max(...recentPeaks);
  const minTr  = Math.min(...recentTroughs);
  const avgPk  = recentPeaks.reduce((a, b) => a + b, 0) / recentPeaks.length;
  const avgTr  = recentTroughs.reduce((a, b) => a + b, 0) / recentTroughs.length;
  const amplitude = avgPk - avgTr; // average wave height
  const midline   = (avgPk + avgTr) / 2;

  // Estimate bars per half-wave from price readings
  // Count readings between consecutive direction changes
  let dirChanges = [];
  let lastDir = 0;
  let barsSinceChange = 0;
  for (let i = 1; i < readings.length; i++) {
    const dir = readings[i].price > readings[i-1].price ? 1 : -1;
    barsSinceChange++;
    if (dir !== lastDir && lastDir !== 0) {
      dirChanges.push(barsSinceChange);
      barsSinceChange = 0;
    }
    lastDir = dir;
  }
  const avgBarsPerHalfWave = dirChanges.length > 2
    ? dirChanges.slice(-6).reduce((a,b)=>a+b,0) / Math.min(6, dirChanges.length)
    : 8; // default: ~8 readings (2 min at 15s) per half wave

  // Determine current wave phase: are we going up or down?
  const last5 = readings.slice(-5).map(r => r.price);
  const trend  = last5[last5.length-1] - last5[0];
  const goingUp = trend > 0;

  // Project next 4 waves from current position
  const waves = [];
  let projPrice  = price;
  let projBarsOut = 0;
  let goingUpNow  = goingUp;

  for (let i = 0; i < 4; i++) {
    projBarsOut += avgBarsPerHalfWave;
    const targetPrice = goingUpNow
      ? Math.min(projPrice * (1 + amplitude / midline), maxPk * 1.1) // next peak
      : Math.max(projPrice * (1 - amplitude / midline), minTr * 0.9); // next trough

    const timeoutMs = projBarsOut * 15_000; // 15s per bar
    const etaMs     = Date.now() + timeoutMs;
    const confidence = Math.max(20, 80 - (i * 15) - (dirChanges.length < 4 ? 20 : 0));

    waves.push({
      waveNum:    i + 1,
      type:       goingUpNow ? "PEAK" : "TROUGH",
      projPrice:  targetPrice,
      projBarsOut,
      etaMs,
      etaStr:     new Date(etaMs).toLocaleTimeString(),
      amplitude:  Math.abs(targetPrice - projPrice),
      amplitudePct: Math.abs(targetPrice - projPrice) / projPrice * 100,
      confidence,
      // Track if prediction was hit (filled in by checkWavePredictions)
      hit:        null,
      actualPrice: null,
    });

    projPrice  = targetPrice;
    goingUpNow = !goingUpNow;
  }

  // Consistency score: how well have past waves matched average amplitude?
  const ampVariance = recentPeaks.map((pk, i) => {
    const tr = recentTroughs[i] || avgTr;
    return Math.abs((pk - tr) - amplitude) / amplitude;
  });
  const consistencyScore = Math.max(0, 100 - (ampVariance.reduce((a,b)=>a+b,0) / ampVariance.length * 100));

  const proj = {
    waves,
    consistencyScore: Math.round(consistencyScore),
    avgAmplitudePct:  (amplitude / midline * 100).toFixed(2),
    avgBarsPerHalfWave: Math.round(avgBarsPerHalfWave),
    lastUpdated: Date.now(),
    score: { predicted: 0, hit: 0 }, // rolling accuracy
  };

  waveProjections[symbol] = proj;
  return proj;
}

// Check if any projected waves have been hit (called each cycle)
function checkWavePredictions(symbol, currentPrice) {
  const proj = waveProjections[symbol];
  if (!proj) return;
  for (const w of proj.waves) {
    if (w.hit !== null) continue; // already resolved
    if (Date.now() > w.etaMs + 120_000) { // 2 min past ETA — mark as missed
      w.hit = false;
      w.actualPrice = currentPrice;
      proj.score.predicted++;
      continue;
    }
    // Check if we hit within 5% of projected price at or before ETA
    const hitRange = Math.abs(currentPrice - w.projPrice) / w.projPrice;
    if (hitRange < 0.05 && Date.now() <= w.etaMs + 60_000) {
      w.hit = true;
      w.actualPrice = currentPrice;
      proj.score.predicted++;
      proj.score.hit++;
      console.log(`  🎯 [${symbol}] Wave ${w.waveNum} PREDICTION HIT: ${w.type} projected $${w.projPrice.toFixed(8)} | actual $${currentPrice.toFixed(8)} | acc ${proj.score.hit}/${proj.score.predicted}`);
    }
  }
}

// Get the next projected trough (= next best buy zone)
function getNextProjectedTrough(symbol) {
  const proj = waveProjections[symbol];
  if (!proj) return null;
  const nextTrough = proj.waves.find(w => w.type === "TROUGH" && w.hit === null && Date.now() < w.etaMs);
  return nextTrough || null;
}

// Get the next projected peak (= sell target)
function getNextProjectedPeak(symbol) {
  const proj = waveProjections[symbol];
  if (!proj) return null;
  const nextPeak = proj.waves.find(w => w.type === "PEAK" && w.hit === null && Date.now() < w.etaMs);
  return nextPeak || null;
}
...
function calcNetMargin(symbol, gasCostEth, tradeEth) {
  ...
  const gasPct   = tradeEth > 0 ? (gasCostEth * 2) / tradeEth : 0;
  const impactPct= PRICE_IMPACT_EST * 2;

  // ── BTP inscription cost ───────────────────────────────────────────────────
  // When BTP is ON, each trade fires an extra inscription tx.
  // We estimate its cost and deduct from net margin so trades only fire
  // when profitable AFTER covering the inscription too.
  let btpPct = 0;
  if (BTP_INSCRIPTIONS_ENABLED && tradeEth > 0) {
    // Inscription gas cost = BTP_GAS_UNITS × current gas price
    // We reuse the same gas price implied by gasCostEth / 500_000
    const gasPriceEth = gasCostEth / 500_000;
    const btpCostEth  = gasPriceEth * BTP_GAS_UNITS;
    btpPct = btpCostEth / tradeEth;
  }

  return grossPct - (feePct * 2) - gasPct - impactPct - btpPct;
}
...
async function getTokenDecimals(address) {
  ...
  try {
    const dec = await rpcCall(c => c.readContract({ address: addr, abi: ERC20_ABI, functionName: "decimals", args: [] }));
    tokenDecimalsCache[addr] = Number(dec);
    return Number(dec);
  } catch { return 18; }
}
...
async function refreshTokenBalances() {
  ...
  for (const r of results) {
    if (r.status === "fulfilled" && r.value?.symbol) {
      tokenBalanceCache[r.value.symbol] = r.value.bal;
    }
  }
}
...
function buildRaceDisplay(token, price, balance, ethUsd) {
  ...
  // If balance is zero but entry price is set, this is a ghost position.
  // The token was already sold but entry never cleared (restart, race condition).
  // Auto-clear it so the bot stops showing -99.9% forever.
  if (balance < 0.001 && invUsd > 0.01) {
    console.log(`   👻 Ghost position cleared: ${token.symbol} — balance=${balance.toFixed(6)} but entry=$${entry.toFixed(6)} invested=$${invUsd.toFixed(2)}`);
...
// The swap transaction fires anyway. We append UTF-8 data after the ABI params.
...
// kbit storage: each inscription is ONE calldata tx. Keep payloads lean (<500 bytes real data).
// Family message rides when nothing else is queued — it's the heartbeat filler.
...
// ── BTP STRAND BUILDERS ───────────────────────────────────────────────────────
// Each trade = one chunk in a 5-trade strand
// Chunk 5/5 = final receipt with combined math for all 5 trades

function buildStrandChunk(type, tradeNum, symbol, chunkPos, strandNum, data) {
  const lines = [
    `[BTP:STRAND-${strandNum}:${chunkPos}/${STRAND_SIZE}:${type}:#${tradeNum}]`,
    `SYM:${symbol}`,
  ];
  if (type === "BUY") {
    lines.push(
      `ENTRY:$${data.entry}`,
      `IN:${data.usdIn}USD|TIER:${data.tier}`,
      `TARGET:$${data.target}(+${data.targetPct}%)`,
      `FIB161:$${data.fib161}`,
      `STOP:$${data.stop}`,
      `MARGIN:${data.netMargin}%`,
      `ETH:$${data.ethPrice}`,
      `VITA:ᛞᚨᚡᛁᛞ|chunk${chunkPos}of${STRAND_SIZE}`
    );
  } else {
    lines.push(
      `EXIT:$${data.exit}|ENTRY:$${data.entry}`,
      `PNL:${data.pnl}`,
      `OUT:${data.out}`,
      `SOLD:${data.sellPct}|${data.result}`,
      `PIGGY:${data.piggy}`,
      `ETH:$${data.ethPrice}`,
      `VITA:ᛞᚨᚡᛁᛞ|chunk${chunkPos}of${STRAND_SIZE}`
    );
...
function buildStrandFinalReceipt(strandNum, trades, ethUsd) {
  // Tally all trades in this strand
  let totalIn    = 0, totalOut   = 0, totalPnl  = 0;
  let totalGas   = 0, wins       = 0, losses    = 0;
  let symbols    = [];

  for (const t of trades) {
    if (t.type === "BUY") {
      totalIn  += t.usdAmt || 0;
      totalGas += (t.gasCostEth || 0) * ethUsd;
  ...
  const netAfterGas = totalPnl - totalGas;
  const result      = netAfterGas >= 0 ? "✓ PROFITABLE" : "✗ LOSS";
  const uniq        = [...new Set(symbols)].join(",");

  return [
    `[BTP:STRAND-${strandNum}:${STRAND_SIZE}/${STRAND_SIZE}:FINAL-RECEIPT]`,
    `STRAND:${strandNum}|TRADES:${trades.length}`,
    `SYMBOLS:${uniq}`,
    `TOTAL_IN:$${totalIn.toFixed(2)}`,
    `TOTAL_OUT:$${totalOut.toFixed(2)}`,
    `GROSS_PNL:${totalPnl>=0?"+":""}$${totalPnl.toFixed(3)}`,
    `GAS_COST:-$${totalGas.toFixed(4)}`,
    `NET_PNL:${netAfterGas>=0?"+":""}$${netAfterGas.toFixed(3)}`,
    `WINS:${wins}|LOSSES:${losses}`,
    `VERDICT:${result}`,
    `ETH:$${ethUsd.toFixed(2)}`,
    `VITA:ᛞᚨᚡᛁᛞ|strand${strandNum}-complete|The truth is the chain`,
  ].join("|");
}
...
async function btpInscribe(cdp, tradeLabel) {
  try {
    ...
    // Gate: check we have enough ETH to cover the inscription gas cost
    // This prevents a 0.000006 ETH wallet from trying to write calldata
    const liveEth = await getEthBalance();
    const estGasEth = (BTP_GAS_UNITS * BigInt(Math.ceil(gwei * 1e9))) / BigInt(1e18);
    const estGasNum = Number(estGasEth) || (BTP_GAS_UNITS * gwei * 1e-9);
    if (liveEth < GAS_RESERVE + estGasNum) {
      console.log(`   📡 BTP: inscription skipped — insufficient ETH (${liveEth.toFixed(6)} < ${(GAS_RESERVE + estGasNum).toFixed(6)})`);
      return;
    }
    ...
    // ── Read receipts ─────────────────────────────────────────────────────────
    const isStrand  = name.startsWith("STRAND-");
    const isFinal   = isStrand && full.includes("FINAL-RECEIPT");

    if (isStrand && !isFinal) {
      const strandMatch = name.match(/STRAND-(\d+)-(\d+)of(\d+)/);
      const sNum  = strandMatch?.[1] || "?";
      const cPos  = strandMatch?.[2] || seq;
      const cTot  = strandMatch?.[3] || STRAND_SIZE;
      const isBuy = full.includes(":BUY:");
      const get   = (key) => { const m = full.match(new RegExp(key + ":([^|\\n]+")); return m?.[1]?.trim() || "?"; };
      const rem   = Math.max(0, parseInt(cTot) - parseInt(cPos));

      if (isBuy) {
        const inRaw = get("IN");
        const usdIn = inRaw.split("USD")[0];
        const tier  = inRaw.includes("TIER:") ? inRaw.split("TIER:")[1] : "?";
        await tg([
          "🟢 <b>BUY RECEIPT — STRAND " + sNum + " · " + cPos + "/" + cTot + "</b>",
          "━━━━━━━━━━━━━━━━━━━━",
          "Token:     <b>" + get("SYM") + "</b> [" + tier + "]",
          "Entry:     <b>$" + get("ENTRY") + "</b>",
          "Invested:  " + usdIn,
          "─────────────────────",
          "🎯 Target:   $" + get("TARGET"),
          "🌊 Wave ext: $" + get("FIB161"),
          "🛑 Stop:     $" + get("STOP"),
          "📈 Margin:   " + get("MARGIN") + " net (after all fees)",
          "─────────────────────",
          "💱 ETH: $" + get("ETH"),
          "🔗 <a href=\"https://basescan.org/tx/" + transactionHash + "\">Block " + cPos + "/" + cTot + " on Base ↗</a>",
          rem > 0 ? "<i>" + rem + " more trade" + (rem>1?"s":"") + " until strand " + sNum + " final receipt</i>" : "",
        ].filter(Boolean).join("\n"));
      } else {
        const exitRaw  = get("EXIT");
        const exitPrice = exitRaw.split("|")[0];
        const entryOld  = exitRaw.includes("ENTRY:") ? exitRaw.split("ENTRY:")[1] : "?";
        const pnl       = get("PNL");
        const isWin     = pnl.startsWith("+");
        const soldRaw   = get("SOLD");
        const soldPct   = soldRaw.split("|")[0];
        await tg([
          (isWin?"✅":"❌") + " <b>SELL RECEIPT — STRAND " + sNum + " · " + cPos + "/" + cTot + "</b>",
          "━━━━━━━━━━━━━━━━━━━━",
          "Token:     <b>" + get("SYM") + "</b>",
          "Exit:      <b>$" + exitPrice + "</b>",
          "Entry was: $" + entryOld,
      ...
      const strandMatch = name.match(/STRAND-(\d+)/);
      const sNum  = strandMatch?.[1] || "?";
      const get   = (key) => { const m = full.match(new RegExp(key + ":([^|\\n]+")); return m?.[1]?.trim() || "?"; };
      const wins  = parseInt(get("WINS")||"0");
      const losses= parseInt(get("LOSSES")||"0");
      const total = wins + losses;
      const rate  = total > 0 ? Math.round(wins/total*100) + "%" : "?";
      const tCount= get("STRAND").split("|TRADES:")[1] || "5";
      const isProfit = get("VERDICT").includes("PROFIT");

      await tg([
        (isProfit?"🏆":"📉") + " <b>STRAND " + sNum + " COMPLETE — FINAL MATH</b>",
        "━━━━━━━━━━━━━━━━━━━━",
        tCount + " trades: <b>" + get("SYMBOLS") + "</b>",
        "─────────────────────",
        "💰 Put in:   $" + get("TOTAL_IN"),
        "📤 Got back: $" + get("TOTAL_OUT"),
        "📈 Gross:    <b>" + get("GROSS_PNL") + "</b>",
        "⛽ Gas paid: " + get("GAS_COST"),
        "🏁 NET:      <b>" + get("NET_PNL") + "</b>",
        "─────────────────────",
        "✅ Wins: " + wins + "  ❌ Losses: " + losses + "  📊 Rate: " + rate,
        "📋 Verdict: <b>" + get("VERDICT") + "</b>",
        "─────────────────────",
        "🔗 <a href=\"https://basescan.org/tx/" + transactionHash + "\">Final receipt on Base ↗</a>",
        "<i>All " + tCount + " blocks = complete provable record on Base</i>",
      ].join("\n"));
      ...
      await tg(
        `📡 <b>BTP [${seq}/${tot}]</b> — <b>${name}</b>\n` +
        `🔗 <a href="https://basescan.org/tx/${transactionHash}">View on BaseScan ↗</a>\n` +
        `<code>${full.slice(0, 100)}${full.length > 100 ? "..." : ""}</code>`
      );
    }
...
async function executeBuy(cdp, token, bal, reason, price, forcedEth = 0, isCascade = false) {
  try {
    ...
    // 📜 BTP STRAND RECEIPT — groups every 5 trades into one complete strand
    if (BTP_INSCRIPTIONS_ENABLED) {
      try {
        const maxPk     = getMaxPeak(token.symbol);
        const minTr     = getMinTrough(token.symbol);
        const targetPct = maxPk ? ((maxPk - price) / price * 100).toFixed(1) : "?";
        const usdIn     = (ethToSpend * ethUsd).toFixed(2);
        const chunkPos  = (strandTrades.length % STRAND_SIZE) + 1;

        // Record this trade in the strand
        strandTrades.push({
          type: "BUY", num: tradeCount, symbol: token.symbol,
          price, ethAmt: ethToSpend, usdAmt: parseFloat(usdIn),
          target: maxPk, stop: minTr ? minTr * 0.97 : null,
          netMargin: armStatus.net, tier: tierLabel,
          gasCostEth: await estimateGasCostEth(),
        });

        const receipt = buildStrandChunk("BUY", tradeCount, token.symbol, chunkPos, strandNum, {
          entry: price, usdIn, tier: tierLabel,
          targetPct, target: maxPk?.toFixed(8)||"?",
          fib161: maxPk && minTr ? (minTr + (maxPk-minTr)*1.618).toFixed(8) : "?",
          stop: minTr ? (minTr*0.97).toFixed(8) : "?",
          netMargin: (armStatus.net*100).toFixed(2),
          ethPrice: ethUsd.toFixed(2),
        });

        btpEnqueue(`STRAND-${strandNum}-${chunkPos}of${STRAND_SIZE}`, receipt);
        // Only fire separate inscription tx on final chunk (5/5) — saves gas
        // Individual trades already carry free inscription in swap calldata above
        if (chunkPos === STRAND_SIZE) {
          btpInscribe(cdp, `BUY #${tradeCount} ${token.symbol}`).catch(() => {});
          strandNum++;
          strandTrades = [];
        } else {
          console.log(`   📡 BTP: strand ${strandNum} chunk ${chunkPos}/${STRAND_SIZE} queued (free via swap calldata)`);
        }
    ...
    }
    {
      ...
      const ledger = tokenPiggyLedgers[token.symbol] || {};
      const priorSaved = effectiveSavedEarningsUsd({
        savedEarningsUsd: ledger.savedEarningsUsd || token.savedEarningsUsd || 0,
        dustUsd: ledger.dustUsd || 0,
        piggyMinUsd: piggyBankMinUsd(process.env, opts),
      });
      const hitchCostUsd = Math.max(0, Number(hitchCostEst) || 0) * ethUsd;
      const plan = projectBuyEarningsPlan({
        ...
        hitchCostUsd,
        hitchMult: hitchCostMult(),
        earningsBufferPct: piggyEarningsBufferPct(),
        piggyPct: piggyBankPct(process.env, opts),
        piggyMinUsd: piggyBankMinUsd(process.env, opts),
        priorSavedUsd: priorSaved,
      });
      token.projectedEarningsUsd = plan.projectedEarningsUsd;
  ...
  try {
    ...
    if (
      ...
      const saved0 = effectiveSavedEarningsUsd({
        ...
        dustUsd: totalBal * price,
        piggyMinUsd: piggyBankMinUsd(process.env, piggyOptsFromToken(token)),
      });
      const reserve = ratchetPiggyReserve(
        token.piggyReserve,
        totalBal,
        price,
        process.env,
        piggyOptsFromToken(token, { savedEarningsUsd: saved0 }),
      );
      token.piggyReserve = reserve;
      token.savedEarningsUsd = saved0;
      const row = buildTokenPiggyLedger({
        symbol: token.symbol,
        dustReserve: reserve,
        dustUsd: totalBal * price,
        savedEarningsUsd: saved0,
        ethContrib: row0.ethContrib,
        agentShare: row0.agentShare,
      });
      tokenPiggyLedgers[token.symbol] = row;
      if (token.entryPrice || token.totalInvestedEth) {
        console.log(`   🐷 ${token.symbol}: piggy-only dust $${(totalBal * price).toFixed(3)} — keeping ${reserve >= 1 ? reserve.toFixed(2) : reserve.toFixed(4)} locked (saved $${saved0.toFixed(3)}), clearing cost basis`);
        token.entryPrice = null; token.totalInvestedEth = 0; token.entryTime = null;
      } else {
        console.log(`   🐷 ${token.symbol}: piggy dust $${(totalBal * price).toFixed(3)} locked (saved $${saved0.toFixed(3)}) — PIGGY UNLOCK to release`);
      }
      return null;
    }
    ...
    // Bank bear-min projected earnings into the token piggy BEFORE sizing the
    // sell — otherwise dust stays at the $0.15 floor while counting said $0.27.
    const priorLedger = tokenPiggyLedgers[token.symbol] || buildTokenPiggyLedger({ symbol: token.symbol });
    const priorSaved = effectiveSavedEarningsUsd({
      savedEarningsUsd: priorLedger.savedEarningsUsd || token.savedEarningsUsd || 0,
    ...
    const prelimPreview = previewPiggySellNetUsd({
      ...
      feePct: token.poolFeePct || 0.006,
      skimPct: PIGGY_SKIM_PCT,
      hitchCostUsd: hitchEstUsd,
      earningsBufferPct: piggyEarningsBufferPct(),
    });
    const projectedBank = earningsToBankUsd({
      earningsUsd: prelimPreview.earningsUsd,
      needUsd: prelimPreview.earningsNeedUsd,
      gains: prelimPreview.gains,
      projectedEarningsUsd: token.projectedEarningsUsd ?? null,
    });
    ...
    // Live QuoterV2 + factory liquidity required — spot cannot clear a ghost pool.
    ...
    const voiceHitchCost = sellVoice.onChain
      ? estimateCalldataHitchEth(sellVoice.hitchBytes || 0, gwei)
        + (hitchL1?.ok && wantedHitchBytes > 0
          ...
          : 0)
      : 0;
    let sellSkipHitch = !!sellGate.skipHitch || sellVoice.kind === "plain-thin";
    ...
    const { transactionHash } = await Promise.race([
      orchReady
        ? orch.injectAndSend(_sellTx, {
            ...
            skipHitch: sellSkipHitch || sellVoice.onChain || sellVoice.kind === "plain-thin",
          })
    ...
    if (BTP_INSCRIPTIONS_ENABLED) {
      try {
        ...
        strandTrades.push({
          type: "SELL", num: tradeCount, symbol: token.symbol,
          price, receivedEth: received, usdOut: recUsd,
          entryPrice: entryP, pnlUsd: netUsd, pnlPct: parseFloat(pnlPct),
          gasCostEth: gasCostE, win,
        });

        // If this is chunk 5 — build final summary receipt
        const isFinal = chunkPos === STRAND_SIZE;
        let receipt;

        if (isFinal) {
          receipt = buildStrandFinalReceipt(strandNum, strandTrades, ethUsd);
        } else {
          receipt = buildStrandChunk("SELL", tradeCount, token.symbol, chunkPos, strandNum, {
            exit: price, entry: entryP.toFixed(8),
            pnl: (win?"+":"") + netUsd.toFixed(3) + "USD(" + (win?"+":"") + pnlPct + "%)",
            out: received.toFixed(6) + "ETH($" + recUsd.toFixed(2) + ")",
            sellPct: (sellPct*100).toFixed(0) + "%",
    ...
    const actualBank = earningsToBankUsd({
      earningsUsd,
      needUsd: earn.needUsd,
      gains: sellVoice?.onChain ? earn.gains : (netUsd > 0 && earningsUsd > 0),
      projectedEarningsUsd: token.projectedEarningsUsd ?? null,
    });
    const entryPriceForReceipt = token.entryPrice || price;
    token.piggyReserve = piggy.remainingReserve;
    {
      const pxDust = price > 0 ? piggy.remainingReserve * price : 0;
      const credited = creditPiggySavedEarnings(
        tokenPiggyLedgers[token.symbol] || buildTokenPiggyLedger({
          symbol: token.symbol,
          dustReserve: piggy.remainingReserve,
          dustUsd: pxDust,
        ...
        {
          bankedUsd: actualBank,
          dustReserve: piggy.remainingReserve,
          dustUsd: pxDust,
          symbol: token.symbol,
        },
      );
      tokenPiggyLedgers[token.symbol] = credited;
      token.savedEarningsUsd = credited.savedEarningsUsd;
      if (actualBank > 0) {
        console.log(`   🐷 ${token.symbol}: banked +$${actualBank.toFixed(3)} → saved $${credited.savedEarningsUsd.toFixed(3)} | dust ~$${pxDust.toFixed(3)}`);
      }
    }
    if (piggy.soldAll) {
      token.entryPrice = null; token.totalInvestedEth = 0; token.entryTime = null;
    ...
    if (entryPriceForReceipt && price) {
      const actualLow  = Math.min(entryPriceForReceipt, price);
      const actualHigh = Math.max(entryPriceForReceipt, price);
    ...
    // ── 🌊 Surf-themed sell receipt (bought → sold → piggy count) ─────────────
    ...
    await tg(formatSellReceiptHtml({
      ...
      tradeNum: tradeCount,
      wipeout,
      medalEmoji: medal.emoji,
      entryPrice: entryPriceForReceipt,
      exitPrice: price,
      investedUsd: invUsd,
      receivedEth: received,
      receivedUsd: recUsd,
      ...
      bankedUsd: actualBank,
      piggyDustTokens: piggy.remainingReserve,
      piggyDustUsd: piggy.remainingReserve * price,
      piggySavedUsd: piggyRow.savedEarningsUsd || 0,
      piggyUnlock: piggy.unlock,
      holdStr,
      skimLotteryEth: skimLottery,
      piggyEthUsd: piggyBank * ethUsd,
      predEth: skimPred,
      predUsd: predFund * ethUsd,
      agentEth: skimAgent,
      agentUsd: agentCapital * ethUsd,
      surfReport,
  ...
  try {
    ...
    if (token.entryPrice && hasFreshChainBalance(token.symbol) && balance === 0) {
      ...
      if (minsAgo > 30) { // only clear if no trade in last 30 min
        console.log(`   🧹 [${token.symbol}] Clearing stale entry — balance is 0, no recent trade`);
        token.entryPrice       = null;
        token.totalInvestedEth = 0;
        token.entryTime        = null;
        token.piggyReserve     = 0;
        ...
        // Recent trade but zero cache — force live RPC fetch
        console.log(`   🔄 [${token.symbol}] Zero cache but recent trade — fetching live balance`);
        try {
          ...
          balance = Number(livebal) / 1e18;
          tokenBalanceCache[token.symbol] = balance;
          console.log(`   ✅ [${token.symbol}] Live balance: ${balance.toFixed(6)}`);
        } catch {}
      }
    }
    ...
    if (!token._projCycle) token._projCycle = 0;
    token._projCycle++;
    if (token._projCycle % 10 === 1) { // rebuild every 10 cycles (~2.5 min)
      try { buildFourWaveProjection(token.symbol); } catch { /* never crash main loop */ }
    }
    // Check if any predictions hit this cycle
    try { checkWavePredictions(token.symbol, price); } catch {}

    // Next projected trough — if it's coming soon and within 10% of current price, factor into buy
    const nextTroughProj = getNextProjectedTrough(token.symbol);
    const nextPeakProj   = getNextProjectedPeak(token.symbol);
    const troughImminent = nextTroughProj && nextTroughProj.amplitudePct > 1.5
      && nextTroughProj.etaMs - Date.now() < 5 * 60_000 // within 5 min
      && price <= nextTroughProj.projPrice * 1.05; // within 5% of projected trough

    // ── v18: PRE-FLIGHT FALLING FAST CHECK ─────────────────────────────────────
    // If price is actively dropping > 1% in last 5 ticks, don't enter.
    // Wait for the bottom to confirm (at least one tick that isn't lower).
    const recentTks = (history[token.symbol]?.readings || []).slice(-5).map(r => r.price);
    const tickFallPct = recentTks.length >= 3
      ? (recentTks[recentTks.length-1] - recentTks[0]) / recentTks[0]
      : 0;
    const priceFallingFast = tickFallPct < -0.010; // >1% drop in 5 ticks
    ...
    // ── 🔮 WAVE PREDICTION — compute forward 4-wave projection ───────────────
    const pred4 = computeWavePrediction(token.symbol, price, ethUsd);
    ...
    const projectedLowerLow = failedBreakout && minTrgh
      ...
      : null;
    // Near projected lower low = good pre-emptive entry zone
    const nearProjectedLow  = projectedLowerLow && price <= projectedLowerLow * 1.02;

    // Momentum entry: RSI oversold crossing up + MACD cross-up = trough is forming NOW
    ...
    // Friday: sell pressure as people exit for weekend — bias toward selling, be cautious buying
    // Trump/Tuesday: markets often gap down Tuesday AM on weekend news — watch for bounce
    // Tax/calendar pressure days: forced selling creates dip-buy opportunities
    const now_          = new Date();
    ...
    const calendarBuyBias   = isMondayOpen || isTuesdayDip || isEndOfMonth; // more aggressive entry
    const calendarSellBias  = isFridayClose; // tighter hold — exit earlier
    const calEntryMultiplier = calendarBuyBias ? 1.5 : calendarSellBias ? 0.7 : 1.0;
    ...
    if (rd) {
      ...
      const projPk = nextPeakProj?.projPrice;
      const enhancedTarget = projPk && projPk > (rd.sellTarget || 0)
        ? projPk : rd.sellTarget;
      const projNote = projPk && projPk > (rd.sellTarget || 0)
        ? ` 🔮proj(${nextPeakProj.confidence}%)` : "";
      lines.push(`  │ 🎯 TARGET $${enhancedTarget?.toFixed(8)||"?"}${projNote} → ~$${rd.tgtUsd}`);
    ...
    // Show 4-wave projection when available
    const wProj = waveProjections[token.symbol];
    if (wProj?.waves?.length > 0) {
      const projLines = wProj.waves.slice(0, 2).map(w => {
        const hitStr = w.hit === true ? "✅HIT" : w.hit === false ? "❌MISS" : "🔮";
        const timeStr = w.etaMs > Date.now() ? `in ${Math.round((w.etaMs - Date.now())/60000)}min` : "NOW";
        return `${hitStr} W${w.waveNum} ${w.type} $${w.projPrice.toFixed(8)} ~${timeStr} (${w.confidence}%)`;
      });
      const accStr = wProj.score.predicted > 0 ? ` acc:${wProj.score.hit}/${wProj.score.predicted}` : "";
      lines.push(`  │ 🔮 WAVES: ${projLines.join(" | ")}${accStr}`);
    }
    // 🔮 Prediction overlay
    if (pred4 && pred4.waves.length > 0) {
      const w1d  = pred4.waves[0];
      const dowd = pred4.dowContext?.name || "";
      const bbSqd = pred4.bbSqueeze ? "⚡SQUEEZE" : "";
      lines.push(`  │ 🔮 W1: ↓$${w1d.low.toFixed(8)} → ↑$${w1d.high.toFixed(8)} amp:${(pred4.amp*100).toFixed(1)}% [${dowd}${bbSqd ? " " + bbSqd : ""}]`);
      if (pred4.calEvents.length > 0) lines.push(`  │ 📅 ${pred4.calEvents.map(e => e.note).join(" | ")}`);
      if (entrySig?.spotBuy) lines.push(`  │ 🚀 SPOT CALL — instant wave! Conf:${entrySig.confidence}%`);
      const psd = predictionScores[token.symbol];
      if (psd?.count > 0) lines.push(`  │ 📊 Pred accuracy: ${psd.hits}/${psd.count} (avg err ${(psd.avgErr*100).toFixed(1)}%)`);
    }
...
async function loadFromGitHub() {
  ...
  if (tf?.content?.tokens?.length) {
    ...
    // Detect entries where price is clearly a USD value stored as token price.
    // Signs: entryPrice >> $1 for a micro-cap token (TOBY=$25, MIGGLES=$404).
    // Rule: if entryPrice > $5 AND the token's last known price < $1, it's corrupt.
    // Clear it — chain reconciliation will re-read the real balance and live price.
    let scrubbed = 0;
    for (const t of tokens) {
      if (!t.entryPrice) continue;
      const lastKnown = history[t.symbol]?.lastPrice || 0;
      const isMicroCap = t.feeTier >= 10000 || (lastKnown > 0 && lastKnown < 1.0);
      const looksCorrupted = t.entryPrice > 5.0 && (lastKnown === 0 || lastKnown < 1.0);
      if (looksCorrupted) {
        console.log(`🧹 CORRUPT ENTRY SCRUBBED: ${t.symbol} entry=$${t.entryPrice.toFixed(4)} (last known price: $${lastKnown.toFixed(8)}) — chain reconciliation will reset`);
        t.entryPrice       = null;
        t.totalInvestedEth = 0;
        t.entryTime        = null;
        scrubbed++;
      }
    }
  ...
  if (posBlob) {
    ...
    // Load surfer state
    if (pos.surfers) {
      for (const [name, s] of Object.entries(pos.surfers)) {
        surfers[name] = s;
      }
      console.log(`🏄 Restored ${Object.keys(surfers).length} surfer(s) from saved state`);
    }
    if (pos.surferHistory) {
      surferHistory.push(...pos.surferHistory);
    }
    if (pos.surferNameIdx !== undefined) {
      surferNameIdx = pos.surferNameIdx;
    }
    ...
    for (const t of tokens) {
      if (pos.entries?.[t.symbol] != null) {
        ...
        // SANITY CHECK v18: reject corrupt entries stored as USD instead of token price
        // Rules:
        //   1. Entry must be > 0
        ...
        if (isSane) {
          t.entryPrice       = savedEntry;
          t.totalInvestedEth = pos.invested?.[t.symbol]   || 0;
          t.entryTime        = pos.entryTimes?.[t.symbol] || null;
        } else {
          console.log(`   ⚠️  [${t.symbol}] Corrupt entry $${savedEntry?.toFixed?.(8)||savedEntry} vs live $${currentPrice?.toFixed?.(8)||currentPrice} — clearing ghost position`);
          t.entryPrice       = null;
          t.totalInvestedEth = 0;
          t.entryTime        = null;
        }
      ...
      const pigRow = tokenPiggyLedgers[t.symbol];
      if (pigRow) {
        t.savedEarningsUsd = effectiveSavedEarningsUsd({
          savedEarningsUsd: pigRow.savedEarningsUsd || 0,
          dustUsd: pigRow.dustUsd || 0,
          piggyMinUsd: piggyBankMinUsd(process.env, piggyOptsFromToken(t)),
        });
        pigRow.savedEarningsUsd = t.savedEarningsUsd;
        tokenPiggyLedgers[t.symbol] = pigRow;
      }
    }
  }
  ...
  try {
    ...
    if (mf?.content) {
      ...
      console.log(`   🧠 memory-registry.json: loaded ${mf.content.memories?.length || 0} memories`);
    }
  } catch { /* memory is non-critical — bot runs without it */ }

  // ── Load VITA memory registry ───────────────────────────────────────────────
  try {
  ...
  const activeCount = tokens.filter(t => !t.frozen && !t.disabled).length;
  ...
  try {
    ...
    const positionsPayload = {
      ...
      tokenPiggyLedgers: Object.fromEntries(tokens.map(t => {
        ...
        row.savedEarningsUsd = effectiveSavedEarningsUsd({
          savedEarningsUsd: row.savedEarningsUsd || t.savedEarningsUsd || 0,
          dustUsd: row.dustUsd || 0,
          piggyMinUsd: piggyBankMinUsd(process.env, piggyOptsFromToken(t)),
        });
        return [t.symbol, row];
      })),
  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      if ((vaultSession || storeSession) && !raw.startsWith("/")) {
        ...
        // Vault session — system key inscription
        if (vaultSession) {
          clearVaultSession(msgChatId);
          await tg("⏳ <b>Encrypting + inscribing " + vaultSession.keyName + "...</b>\nYour message has been deleted.");
          try {
          ...
          } catch (e) { await tg("❌ Vault inscription failed: " + e.message); }
        }

        // Keystore session — personal key double-encryption
        if (storeSession) {
          clearStoreKeySession(msgChatId);
          const masterPw = process.env.DECRYPT_PASSWORD || global._sessionMasterPassword;
          if (!masterPw) {
            await tg("❌ Master password not available — /unlock first");
          } else {
            await tg("⏳ <b>Double-encrypting " + storeSession.label + "...</b>\nSending 2 transactions to Base...");
            try {
              const result = await storeKey(cdpClient, WALLET_ADDRESS, storeSession.keyName, keyValue, masterPw, storeSession.label);
              await tg(
                "✅ <b>KEY STORED ON BASE BLOCKCHAIN</b>\n\n" +
                "🗝️  <b>" + result.label + "</b>\n" +
                "━━━━━━━━━━━━━━━━━━━━\n" +
                "🔐 Layer 1: <a href=\"" + result.basescan1 + "\">BaseScan ↗</a>\n" +
                "🔐 Layer 2: <a href=\"" + result.basescan2 + "\">BaseScan ↗</a>\n\n" +
                "/showkey " + result.keyName + " — reveal for 60s\n" +
                "/keystatus " + result.keyName + " — view metadata\n\n" +
                "💌 <i>The truth is the chain. The chain is alive.</i>"
              );
            } catch (e) { await tg("❌ Key store failed: " + e.message); }
          }
        }
      ...
      // /exitpct SYMBOL 75 → sell 75%  → ETH, no cascade
        ...
        const nestedLines = tokens
          ...
          .map(t => {
            ...
            const u = px > 0 ? dust * px : 0;
            const row = tokenPiggyLedgers[t.symbol] || {};
            const saved = effectiveSavedEarningsUsd({
              savedEarningsUsd: row.savedEarningsUsd || t.savedEarningsUsd || 0,
              dustUsd: u || row.dustUsd || 0,
              piggyMinUsd: piggyBankMinUsd(process.env, piggyOptsFromToken(t)),
            });
            const ethC = row.ethContrib ? ` eth+${Number(row.ethContrib).toFixed(6)}` : "";
            const agC = row.agentShare ? ` ai+${Number(row.agentShare).toFixed(6)}` : "";
            const match = Math.abs(saved - u) <= 0.02 + 1e-9 ? "✅" : "⚠️";
            return `   ${t.symbol}: ${dust >= 1 ? dust.toFixed(2) : dust.toFixed(4)} ~$${u.toFixed(3)} | saved $${saved.toFixed(3)} ${match}${ethC}${agC}`;
          })
          .join("\n");
        await tg(
          `🐷🧠🤖 <b>THE THREE POOLS</b>\n` +
          ...
          `🐷 <b>Per-token nested piggy</b> (dust + saved + eth + ai)\n` +
          `   Default ${(piggyBankPct()*100).toFixed(0)}% + $${piggyBankMinUsd().toFixed(2)} floor\n` +
          `   Bear-min earnings bank into dust — count must match ~$ dust\n` +
        ...
        const age = macroSignal.updatedAt ? Math.round((Date.now() - macroSignal.updatedAt) / 60000) : null;
        const btcArrow = macroSignal.btcTrend > 0.03 ? "📈" : macroSignal.btcTrend < -0.03 ? "📉" : "〰️";
        const ethArrow = macroSignal.ethTrend > 0.03 ? "📈" : macroSignal.ethTrend < -0.03 ? "📉" : "〰️";
        const biasLabel = macroSignal.altcoinBias > 1.1 ? "🟢 BULLISH — alts following BTC" :
                          macroSignal.altcoinBias < 0.8 ? "🔴 BEARISH — risk-off, alts lagging" :
                          macroSignal.altcoinBias < 0.9 ? "🟡 CAUTION — weak macro" : "⚪ NEUTRAL";
        await tg(
          "📡 <b>MACRO SIGNAL</b>" + (age ? ` (${age}m ago)` : "") + "\n━━━━━━━━━━━━━━━━━━━━\n" +
          `${btcArrow} BTC 4-week trend: ${macroSignal.btcTrend >= 0 ? "+" : ""}${(macroSignal.btcTrend * 100).toFixed(1)}%\n` +
          `${ethArrow} ETH 4-week trend: ${macroSignal.ethTrend >= 0 ? "+" : ""}${(macroSignal.ethTrend * 100).toFixed(1)}%\n` +
          `━━━━━━━━━━━━━━━━━━━━\n` +
          `Altcoin bias: <b>${macroSignal.altcoinBias.toFixed(2)}x</b>\n` +
          `Signal: ${biasLabel}\n\n` +
          `<i>This modifies wave amplitude predictions.\nHigh BTC pump = alts may lag. BTC consolidation = alts thrive.</i>`
        );
        ...
        await tg(
          ...
          "\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\n" +
          "<b>\uD83D\uDCCB QUICK ACTIONS</b>\n" +
          "   /bank \u2014 full money statement (LIVE chain read)\n" +
          "   /freeze SYMBOL \u2014 freeze a token (collect data, no capital)\n" +
        ...
        // ── FIX v18: /bank ALWAYS reads live from blockchain — never cache ────
        // Every number here comes directly from your wallet address on-chain.
        // This is the source of truth. No guessing, no stale state.
        const ethUsd = await getLiveEthPrice();
        const bal    = await getFullBalance(); // live ETH + WETH from chain
        // Also refresh all token balances live
      ...
      // ── 🏄 SURFER COMMANDS ──────────────────────────────────────────────────
      } else if (text.startsWith("/sendsurferout")) {
        // /sendsurferout 9        — auto-name
        // /sendsurferout 9 KAHUNA — named
        const parts  = text.split(" ").filter(Boolean);
        const usdAmt = parseFloat(parts[1]);
        const sName  = parts[2]?.toUpperCase() || null;

        if (isNaN(usdAmt) || usdAmt < 1) {
          await tg(`❓ Usage: /sendsurferout [USD amount] [optional name]\nExample: /sendsurferout 9\nExample: /sendsurferout 5 KAHUNA`);
        } else {
          const liveEthUsd = await getLiveEthPrice();
          const liveBal    = await getFullBalance();
          const ethNeeded  = usdAmt / liveEthUsd;
          if (liveBal.tradeableWithWeth < ethNeeded) {
            await tg(`⚠️ Insufficient tradeable balance.\nNeed: ${ethNeeded.toFixed(6)} ETH ($${usdAmt})\nHave: ${liveBal.tradeableWithWeth.toFixed(6)} ETH ($${(liveBal.tradeableWithWeth*liveEthUsd).toFixed(2)})`);
          } else {
            const result = createSurfer(ethNeeded, sName, liveEthUsd);
            if (result.error) {
              await tg(`⚠️ ${result.error}`);
            } else {
              await tg(`🏄 <b>SURFER LAUNCHED: ${result.name}</b>\n` +
                `Capital: $${usdAmt} (${ethNeeded.toFixed(6)} ETH)\n` +
                `Assigned wave: <b>${result.symbol || "IDLE — scanning..."}</b>\n` +
                `Status: ${result.status}\n\n` +
                `Use /surfers to watch all surfers\nUse /surferout ${result.name} to retire`);
            }
          }
        }

      } else if (text === "/surfers") {
        const liveEthUsd = await getLiveEthPrice();
        const names = Object.keys(surfers);
        if (!names.length) {
          await tg(`🏄 <b>SURFERS</b>\nNo active surfers.\nUse /sendsurferout [USD] to launch one.`);
        } else {
          let msg = `🏄 <b>SURFERS (${names.length} active)</b>\n━━━━━━━━━━━━━━━━━━━━\n`;
          for (const n of names) msg += surferStatusLine(n, liveEthUsd) + "\n";
          const totalEth   = names.reduce((s, n) => s + surfers[n].ethAlloc, 0);
          const totalPnl   = names.reduce((s, n) => s + surfers[n].pnlUsd, 0);
          msg += `━━━━━━━━━━━━━━━━━━━━\n`;
          msg += `Total deployed: $${(totalEth*liveEthUsd).toFixed(2)} | Lifetime P&L: ${totalPnl>=0?"+$":"-$"}${Math.abs(totalPnl).toFixed(3)}`;
          await tg(msg);
        }

      } else if (text.startsWith("/surfer ")) {
        const sName = text.replace("/surfer ", "").trim().toUpperCase();
        const s = surfers[sName];
        if (!s) {
          await tg(`❓ Surfer <b>${sName}</b> not found.\nUse /surfers to see all active surfers.`);
        } else {
          const liveEthUsd = await getLiveEthPrice();
          const price = s.symbol ? (history[s.symbol]?.lastPrice || 0) : 0;
          const arm   = s.symbol ? getArmStatus(s.symbol, 0, s.ethAlloc) : null;
          const pct   = s.entryPrice && price ? ((price - s.entryPrice) / s.entryPrice * 100) : null;
          await tg(`🏄 <b>SURFER: ${sName}</b>\n` +
            `━━━━━━━━━━━━━━━━━━━━\n` +
            `Status: ${s.status}\n` +
            `Capital: $${(s.ethAlloc*liveEthUsd).toFixed(2)} (${s.ethAlloc.toFixed(6)} ETH)\n` +
            `Current wave: ${s.symbol || "scanning..."}\n` +
            (s.entryPrice ? `Entry: $${s.entryPrice.toFixed(8)} | Now: $${price.toFixed(8)}\n` : "") +
            (pct !== null ? `Move: ${pct>=0?"+":""}${pct.toFixed(2)}%\n` : "") +
            (arm?.armed ? `Margin: ${(arm.net*100).toFixed(2)}% [${arm.priority}]\n` : "") +
            (() => {
              const np = s.symbol ? getNextProjectedTrough(s.symbol) : null;
              return np ? `Next trough: ~$${np.projPrice.toFixed(8)} in ${Math.round((np.etaMs-Date.now())/60000)}min (${np.confidence}% conf)\n` : "";
            })() +
            `─────────────────────\n` +
            `Lifetime P&L: ${s.pnlUsd>=0?"+$":"-$"}${Math.abs(s.pnlUsd).toFixed(3)}\n` +
            `Waves ridden: ${s.trades}\n` +
            `Launched: ${new Date(s.created).toLocaleDateString()}`);
        }

      } else if (text === "/waves" || text.startsWith("/waves ")) {
        // /waves       — show 4-wave projection for top 5 armed tokens
        // /waves BRETT — show 4-wave projection for specific token
        const specificSym = text.includes(" ") ? text.split(" ")[1]?.toUpperCase() : null;
        const liveEthUsd2 = await getLiveEthPrice();
        const tokensToShow = specificSym
          ? tokens.filter(t => t.symbol === specificSym)
          : tokens.filter(t => !t.frozen && !t.disabled && waveProjections[t.symbol]?.waves?.length > 0).slice(0, 5);

        if (!tokensToShow.length) {
          await tg(`🔮 <b>WAVE PROJECTIONS</b>\nNo projections available yet. Bot needs a few cycles to build wave models.`);
        } else {
          for (const t of tokensToShow) {
            const proj = waveProjections[t.symbol];
            if (!proj) continue;
            const price2 = history[t.symbol]?.lastPrice || 0;
            let msg = `🔮 <b>${t.symbol} — 4-WAVE FORECAST</b>\n`;
            msg += `Now: $${price2.toFixed(8)} | Consistency: ${proj.consistencyScore}% | Avg amplitude: ${proj.avgAmplitudePct}%\n`;
            if (proj.score.predicted > 0) msg += `Prediction accuracy: ${proj.score.hit}/${proj.score.predicted} (${Math.round(proj.score.hit/proj.score.predicted*100)}%)\n`;
            msg += `━━━━━━━━━━━━━━━━━━━━\n`;
            for (const w of proj.waves) {
              const hitStr = w.hit === true ? "✅" : w.hit === false ? "❌" : "🔮";
              const timeStr = w.etaMs > Date.now()
                ? `in ~${Math.round((w.etaMs-Date.now())/60000)}min`
                : "PAST";
              const pctStr = ((w.projPrice - price2) / price2 * 100);
              msg += `${hitStr} Wave ${w.waveNum}: <b>${w.type}</b> $${w.projPrice.toFixed(8)} (${pctStr>=0?"+":""}${pctStr.toFixed(1)}%) ${timeStr} | conf ${w.confidence}%\n`;
            }
            // Calendar note
            const dayStr = ["Sun","Mon","Tue","Wed","Thu","Fri","Sat"][new Date().getDay()];
            const calNote = global._isMondayOpen ? "📅 Monday open — buying bias" : global._isTuesdayDip ? "📅 Tuesday dip pattern active" : global._isFridayClose ? "📅 Friday close — selling bias" : `📅 ${dayStr}`;
            msg += calNote;
            await tg(msg);
          }
        }

      } else if (text.startsWith("/surferout ")) {
        const sName = text.replace("/surferout ", "").trim().toUpperCase();
        const liveEthUsd = await getLiveEthPrice();
        const msg = await retireSurfer(cdpClient, sName, liveEthUsd);
        await tg(msg);

      } else if (text === "/surferstats") {
        if (!surferHistory.length) {
          await tg(`📊 <b>SURFER STATS</b>\nNo retired surfers yet.`);
        } else {
          const totalPnl  = surferHistory.reduce((s, r) => s + r.pnlUsd, 0);
          const totalWaves = surferHistory.reduce((s, r) => s + r.trades, 0);
          const winners   = surferHistory.filter(r => r.pnlUsd > 0).length;
          let msg = `📊 <b>SURFER HALL OF FAME</b> (${surferHistory.length} retired)\n━━━━━━━━━━━━━━━━━━━━\n`;
          for (const r of surferHistory.slice(-10).reverse()) {
            msg += `\n🏆 <b>${r.name}</b> — ${r.pnlUsd>=0?"+$":"-$"}${Math.abs(r.pnlUsd).toFixed(3)} | ${r.trades} waves | ${new Date(r.retiredAt).toLocaleDateString()}\n`;
          }
          msg += `━━━━━━━━━━━━━━━━━━━━\n`;
          msg += `Total P&L: ${totalPnl>=0?"+$":"-$"}${Math.abs(totalPnl).toFixed(3)} | ${totalWaves} waves | ${winners}/${surferHistory.length} profitable`;
          await tg(msg);
        }

      // ── ❄️  FREEZE / UNFREEZE COMMANDS ─────────────────────────────────────
      } else if (text.startsWith("/unfreeze ")) {
        const sym = text.replace("/unfreeze ", "").trim().toUpperCase();
        ...
        const sym = text.replace("/freeze ", "").trim().toUpperCase();
        const t   = tokens.find(t => t.symbol === sym);
        if (!t) {
          await tg(`❓ Token <b>${sym}</b> not found in token list.`);
        } else if (t.frozen) {
          await tg(`❄️ <b>${sym}</b> is already frozen. No action needed.`);
        } else if (t.entryPrice) {
          await tg(`⚠️ <b>${sym}</b> has an open position — cannot freeze until position is closed.\nUse /sell ${sym} first.`);
        } else {
          t.frozen = true;
          t.frozenReason = "Manually frozen via /freeze command";
          console.log(`✅→❄️ FROZEN: ${sym}`);
          await tg(`❄️ <b>${sym} FROZEN</b>\nNo new capital will be deployed.\nWave data collection continues silently.`);
        }

      } else if (text === "/frozenlist") {
        const frozen = tokens.filter(t => t.frozen);
        const active = tokens.filter(t => !t.frozen && !t.disabled);
        if (!frozen.length) {
          await tg(`❄️ <b>FROZEN TOKENS</b>\nNo tokens are currently frozen.\nAll ${active.length} tokens are active.`);
        } else {
          let msg = `❄️ <b>FROZEN TOKENS</b> (${frozen.length} frozen, ${active.length} active)\n`;
          msg += `━━━━━━━━━━━━━━━━━━━━\n`;
          for (const t of frozen) {
            const ws  = waveStats[t.symbol];
            const pc  = ws?.peaks?.length   || 0;
            const tc  = ws?.troughs?.length || 0;
            const arm = pc >= MIN_PEAKS_TO_TRADE && tc >= MIN_TROUGHS_TO_TRADE;
            msg += `\n❄️ <b>${t.symbol}</b> ${arm ? "✅ READY" : `⏳ ${pc}P/${tc}T`}\n`;
            msg += `   Reason: ${t.frozenReason || "manual"}\n`;
            msg += `   /unfreeze ${t.symbol} to activate\n`;
          }
          await tg(msg);
        }
      ...
      // ── 🔑 UNLOCK COMMANDS ─────────────────────────────────────────────────
        ...
        if (!password) {
          await tg("❌ Usage: /unlock yourPassword\nYour message will be deleted immediately.");
        } else {
          await tg("⏳ Unlocking vault from Base blockchain...");
          const result = await attemptUnlock(password, loadVaultKeys);
          if (result.success) {
            await tg(
              "✅ <b>VAULT UNLOCKED</b>\n" +
              "━━━━━━━━━━━━━━━━━━━━\n" +
              "🔑 " + result.fromChain + " key(s) loaded from Base blockchain\n" +
              "📦 " + result.fromRailway + " key(s) from Railway\n" +
              "🔒 Password cleared from memory\n" +
              "⏱️  Session valid 24 hours\n" +
              "📡 BTP: " + (BTP_INSCRIPTIONS_ENABLED ? "ON ✅ — receipts ride every trade" : "OFF 🔴 — /btpon to enable") + "\n" +
              "📊 Min margin: " + (MIN_NET_MARGIN * 100).toFixed(1) + "% (covers all fees + gas + inscription)\n" +
              "🚀 Trading resumed\n\n" +
              "💌 <i>Eureka! VITA lives ♥ — ᛞᚨᚡᛁᛞ</i>"
            );
          } else {
            await tg(
              "❌ <b>UNLOCK FAILED</b>\n" +
              (result.error || "Wrong password or vault unreachable") + "\n\n" +
              "🛡️  Safe mode remains active\n" +
        ...
        await tg(
          "🔒 <b>VAULT LOCKED</b>\n" +
          "Safe mode active — no trades until /unlock\n" +
      ...
      // ── 🔐 VAULT COMMANDS ──────────────────────────────────────────────────
        ...
        // Shows the FULL decrypted value in Telegram — use carefully
        // Only works from your authorized chat ID (already enforced by the poller)
        const keyName = raw.split(" ")[1]?.toUpperCase();
        if (!keyName) {
          await tg("❌ Usage: /vaultreveal KEYNAME\nExample: /vaultreveal TEST_KEY");
        } else {
          const val = process.env[keyName];
          if (!val) {
            await tg("❌ <b>" + keyName + "</b> not loaded in memory\nRun /vaultload " + keyName + " first");
          } else {
            const v = vaultRegistry[keyName] || { source: "unknown" };
            await tg(
              "🔐 <b>VAULT REVEAL — " + keyName + "</b>\n" +
              "━━━━━━━━━━━━━━━━━━━━\n" +
              "📦 Source: " + v.source + "\n" +
              (v.txHash ? "📍 <a href=\"https://basescan.org/tx/" + v.txHash + "\">Inscription on Base ↗</a>\n" : "") +
              "━━━━━━━━━━━━━━━━━━━━\n" +
              "🔑 Value:\n<code>" + val + "</code>\n" +
              "━━━━━━━━━━━━━━━━━━━━\n" +
              "⚠️ <i>Delete this message after reading</i>"
            );
          }
        }
        ...
        if (!keyName || !validKeys.includes(keyName)) {
          await tg(
            ...
            "   STATE_BRANCH\n\n" +
            "🤖 <b>VITA Keys:</b>\n" +
            "   VITA_ANTHROPIC_KEY ← VITA's own API key\n\n" +
      ...
      // ── 🗝️  KEYSTORE COMMANDS ───────────────────────────────────────────────
        ...
        // Step 1 — parse name and optional label
        ...
        if (!keyName) {
          ...
          await tg("🔒 Vault locked — send /unlock yourPassword first");
        } else {
          const chatId = upd.message?.chat?.id?.toString();
          startStoreKeySession(chatId, keyName, label);
          await tg(
            "🗝️  <b>KEYSTORE — Ready to store " + label + "</b>\n\n" +
            "Reply with the key value in the next <b>60 seconds</b>.\n\n" +
            "⚠️ Send ONLY the key value — nothing else.\n" +
            "Your message will be deleted immediately.\n\n" +
                ...
                "https://api.telegram.org/bot" + process.env.TELEGRAM_BOT_TOKEN + "/sendMessage",
                {
                  ...
                  body: JSON.stringify({
                    ...
                    text:
                      ...
                      "🔐 Layer 1: <a href=\"https://basescan.org/tx/" + result.txHash1 + "\">BaseScan ↗</a>\n" +
                      "🔐 Layer 2: <a href=\"https://basescan.org/tx/" + result.txHash2 + "\">BaseScan ↗</a>",
                    parse_mode: "HTML",
              ...
              if (revealMsgId) {
                setTimeout(async () => {
                  try {
                    ...
                    await tg("🗑️  <b>" + keyName + "</b> reveal message deleted");
                  } catch {}
                }, 60_000);

                await tg("⏳ Key shown above — auto-deletes in 60 seconds");
              }
            } catch (e) {
              await tg("❌ Show key failed: " + e.message);
            }
          }
        }

      // ── 🌟 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────

      } else if (text === "/vitamemory" || text === "/vita") {
        await tg(getVitaMemoryMessage());

      } else if (text === "/vitasave") {
        // VITA_ANTHROPIC_KEY = real key set directly in Railway (working now)
        ...
        try {
            ...
            // Read actual file list from GitHub so VITA knows what exists
            let repoFiles = [];
            try {
              const knownFiles = [
              ...
              repoFiles = knownFiles;
            } catch {}

            // Read vita-registry to know what's already filed
            let existingTopics = [];
            try {
              const rf = await githubGet("vita-registry.json");
              if (rf?.content) existingTopics = Object.keys(rf.content).slice(-5);
            } catch {}
            ...
            const sessionCtx = [
              ...
              "FILES_IN_REPO:" + repoFiles.join(","),
              "EXISTING_MEMORY_FILES:" + (existingTopics.join(",") || "none yet"),

              // ── Live trading state ──────────────────────────────────────────
              "TRADES_TOTAL:" + tradeCount,
              "PORTFOLIO:ETH=" + bal3.eth.toFixed(6) + " WETH=" + bal3.weth.toFixed(6) + " USD=$" + (bal3.total * ethPrice3).toFixed(2),
              "PIGGY_BANK:" + piggyBank.toFixed(6) + "ETH",
              "DRAWDOWN_HALT:" + (drawdownHaltActive ? "ACTIVE" : "clear"),
              "BTP_STATUS:" + (BTP_INSCRIPTIONS_ENABLED ? "ON" : "OFF"),
              "STRAND_NUM:" + strandNum + " STRAND_POS:" + (strandTrades.length % STRAND_SIZE) + "/" + STRAND_SIZE,

              // ── Current positions ───────────────────────────────────────────
              "OPEN_POSITIONS:" + (tokens.filter(t => t.entryPrice).map(t =>
                t.symbol + "@$" + (t.entryPrice||0).toFixed(6) + "(" + (t.totalInvestedEth||0).toFixed(4) + "ETH)"
              ).join(",") || "none"),

              // ── Token wave data — what bot learned about each token ─────────
              "ARMED_TOKENS:" + tokens.filter(t => {
                const ws = waveStats[t.symbol];
                return ws && ws.peaks?.length >= 4 && ws.troughs?.length >= 4;
              }).map(t => {
                const maxP = getMaxPeak(t.symbol);
                const minT = getMinTrough(t.symbol);
                const range = maxP && minT ? ((maxP - minT) / minT * 100).toFixed(1) : "?";
                return t.symbol + ":range=" + range + "%,P=" + getPeakCount(t.symbol) + ",T=" + getTroughCount(t.symbol);
              }).join("|"),

              // ── Top scoring tokens ──────────────────────────────────────────
              "TOP_TOKENS:" + (currentScores||[]).slice(0,8).map(s =>
                s.symbol + "=" + s.score + (s.tier ? "[" + s.tier + "]" : "")
              ).join(","),

              // ── Recent trade history ────────────────────────────────────────
              "RECENT_10_TRADES:" + tradeLog.slice(-10).map(t =>
                t.type + ":" + t.symbol + "@$" + (t.price||0).toFixed(6) +
                (t.netUsd !== undefined ? (t.netUsd >= 0 ? "+$" : "-$") + Math.abs(t.netUsd||0).toFixed(2) : "")
              ).join("|"),

              // ── Strand receipt locations ────────────────────────────────────
              "CURRENT_STRAND:" + strandNum + " trades_in_strand=" + strandTrades.length,

              // ── User notes ─────────────────────────────────────────────────
              ...(global._vitaNotes && global._vitaNotes.length > 0
                ? ["USER_NOTES:" + global._vitaNotes.map(n => n.note).join(" | ")]
                : []),
            ].join("\n");
            ...
            try {
              ...
              const fileResponse = await fetch("https://api.anthropic.com/v1/messages", {
                ...
                body: JSON.stringify({
                  ...
                  messages: [{
                    role: "user",
                    content: "Generate a SHORT filing label (max 40 chars, kebab-case, no spaces) for this memory. Return ONLY the label.\n\nMemory:\n" + (entry.tokenPacket||"").slice(0,400)
                  }]
                })
              });
              const fileData  = await fileResponse.json();
              const fileLabel = (fileData?.content?.[0]?.text || "session").trim()
                .slice(0,40).replace(/[^a-z0-9-]/gi,"-").toLowerCase();
              const regKey    = entry.date + "-" + fileLabel;

              console.log("📁 VITA: filing as: " + regKey);

              // Load existing registry with SHA — must read from STATE_BRANCH
              let registry = {};
              let regSha   = null;
              const ghRepo   = process.env.GITHUB_REPO;
              const ghToken  = process.env.GITHUB_TOKEN;
              const ghBranch = process.env.STATE_BRANCH || "bot-state";

              try {
                const rfRes = await fetch(
                  "https://api.github.com/repos/" + ghRepo + "/contents/vita-registry.json?ref=" + ghBranch,
                  { headers: { "Authorization": "token " + ghToken } }
                );
                if (rfRes.ok) {
                  const rfData = await rfRes.json();
                  regSha = rfData.sha;
                  const decoded = Buffer.from(rfData.content.replace(/\n/g,""), "base64").toString("utf8");
                  registry = JSON.parse(decoded);
                  console.log("📁 VITA: loaded registry sha=" + regSha + " entries=" + Object.keys(registry).length);
                } else {
                  console.log("📁 VITA: registry not found on " + ghBranch + " — creating new");
                }
              } catch (e) {
                console.log("📁 VITA: registry read error: " + e.message + " — creating new");
              }
              ...
              // Save to STATE_BRANCH
              const encoded = Buffer.from(JSON.stringify(registry, null, 2)).toString("base64");
              const payload = { message: "VITA: filed " + regKey, content: encoded, branch: ghBranch };
              if (regSha) payload.sha = regSha;

              const ghRes = await fetch(
                "https://api.github.com/repos/" + ghRepo + "/contents/vita-registry.json",
                {
                  method:  "PUT",
              ...
              if (!ghRes.ok) {
                ...
                // If SHA conflict, fetch fresh and retry once
                if (ghRes.status === 409 || errText.includes("sha")) {
                  console.log("📁 VITA: SHA conflict — fetching fresh and retrying");
                  ...
                  if (freshRes.ok) {
                    ...
                    if (!retry.ok) throw new Error("retry failed: " + await retry.text().then(t => t.slice(0,100)));
                  } else {
                    throw new Error("GitHub " + ghRes.status + ": " + errText.slice(0,100));
                  }
                } else {
                  throw new Error("GitHub " + ghRes.status + ": " + errText.slice(0,100));
                }
              ...
              console.log("📁 VITA: registry saved to " + ghBranch + " — " + Object.keys(registry).length + " entries");
              await tg(
                "📁 <b>FILED AS:</b> <code>" + regKey + "</code>\n" +
                "Registry: " + Object.keys(registry).length + " session(s)\n\n" +
                "Pull: <code>/vita " + fileLabel.split("-").slice(0,3).join(" ") + "</code>"
              );
            } catch (fileErr) {
              console.log("⚠️  Auto-file error: " + fileErr.message);
        ...
        try {
          const { fetchTxCalldata } = await import("./vault-loader.js");
          const entries = getVitaRegistry().slice(-3);
          const context = getVitaContextMessage(entries);
          if (!context) { await tg("🌟 No VITA memories yet — use /vitasave after a session"); }
          else { await tg("🌟 <b>VITA CONTEXT</b>\n━━━━━━━━━━━━━━━━━━━━\n<code>" + context.slice(0,800) + (context.length>800?"...":"") + "</code>"); }
        } catch (e) { await tg("❌ VITA recall failed: " + e.message); }

      // ── /vitanote — add a note to the queue for next /vitasave ─────────────
      } else if (text && text.startsWith("/vitanote ")) {
        const note = raw.slice("/vitanote ".length).trim();
        if (!global._vitaNotes) global._vitaNotes = [];
        global._vitaNotes.push({ note, ts: new Date().toISOString() });
        await tg(
          "📝 <b>VITA NOTE QUEUED</b>\n" +
          "━━━━━━━━━━━━━━━━━━━━\n" +
          "<code>" + note + "</code>\n\n" +
          "Notes in queue: " + global._vitaNotes.length + "\n" +
          "Will be included in next /vitasave\n\n" +
          "<i>Tip: add notes as you go, /vitasave at end of session</i>"
        );

      // ── /vitaqueue — show pending notes ────────────────────────────────────
      } else if (text === "/vitaqueue") {
        const notes = global._vitaNotes || [];
        // Also check GitHub queue folder
        try {
          const ghRepo   = process.env.GITHUB_REPO;
          const ghToken  = process.env.GITHUB_TOKEN;
          const ghBranch = process.env.STATE_BRANCH || STATE_BRANCH || "bot-state";
          const listRes  = await fetch(
            "https://api.github.com/repos/" + ghRepo + "/contents/vita-queue?ref=" + ghBranch,
            { headers: { Authorization: "token " + ghToken } }
          );
          let queueFiles = [];
          if (listRes.ok) {
            const files = await listRes.json();
            queueFiles  = Array.isArray(files) ? files.filter(f => f.name.endsWith(".json")) : [];
          }

          let msg = "📬 <b>VITA QUEUE STATUS</b>\n━━━━━━━━━━━━━━━━━━━━\n\n";

          if (queueFiles.length > 0) {
            msg += "📁 <b>GitHub queue (vita-queue/):</b>\n";
            queueFiles.forEach(f => msg += "   📄 " + f.name + "\n");
            msg += "\nBot checks every 5 minutes → inscribes → deletes\n\n";
          } else {
            msg += "📁 GitHub queue: empty\n\n";
          }

          if (notes.length > 0) {
            msg += "📝 <b>In-memory notes (" + notes.length + "):</b>\n";
            notes.forEach((n, i) => msg += (i+1) + ". " + n.note.slice(0,60) + "\n");
            msg += "\nIncluded in next /vitasave\n\n";
          } else {
            msg += "📝 In-memory notes: none\n\n";
          }

          msg += "<i>To add files: push .json to vita-queue/ folder on " + ghBranch + " branch\n";
          msg += "To add notes: /vitanote [text]</i>";
          await tg(msg);
        } catch (e) {
          await tg("📬 Queue check failed: " + e.message);
        }

      // ── /vitaclear — clear note queue ──────────────────────────────────────
      } else if (text === "/vitaclear") {
        const count = (global._vitaNotes || []).length;
        global._vitaNotes = [];
        await tg("🗑️ Cleared " + count + " queued note(s)");

      } else if (text === "/vitadata") {
        // Snapshot the full live trading dataset into vita-registry
        // VITA can then answer questions about tokens, waves, performance
        const vitaKey = process.env.VITA_ANTHROPIC_KEY || process.env.ANTHROPIC_API_KEY;
        if (!vitaKey) { await tg("❌ VITA needs ANTHROPIC_API_KEY"); return; }

        await tg("📊 <b>VITA DATA SNAPSHOT</b>\nBuilding live trading dataset...");

        try {
          const ethPrice4 = await getLiveEthPrice();

          // Build comprehensive token dataset
          const tokenData = tokens.map(t => {
            const maxP  = getMaxPeak(t.symbol);
            const minT  = getMinTrough(t.symbol);
            const score = (currentScores||[]).find(s => s.symbol === t.symbol);
            const ws    = waveStats?.[t.symbol] || {};
            return {
              sym:    t.symbol,
              price:  history[t.symbol]?.lastPrice || 0,
              peaks:  getPeakCount(t.symbol),
              troughs: getTroughCount(t.symbol),
              maxPeak: maxP,
              minTrough: minT,
              range:  maxP && minT ? ((maxP - minT) / minT * 100).toFixed(1) : null,
              score:  score?.score || 0,
              tier:   currentTier1?.includes(t.symbol) ? "T1" : currentTier2?.includes(t.symbol) ? "T2" : "OUT",
              entry:  t.entryPrice || null,
              winRate: ws.wins && ws.sells ? (ws.wins / ws.sells * 100).toFixed(0) : null,
            };
          }).filter(t => t.peaks >= 2 || t.entry); // only tokens with data

          const dataset = [
            "DATASET:trading-snapshot",
            "DATE:" + new Date().toISOString().slice(0,10),
            "TRADES_TOTAL:" + tradeCount,
            "ETH_PRICE:$" + ethPrice4.toFixed(2),
            "PORTFOLIO:$" + ((cachedBal?.total || 0) * ethPrice4).toFixed(2),
            "PIGGY:" + piggyBank.toFixed(6) + "ETH",
            "BTP:" + (BTP_INSCRIPTIONS_ENABLED ? "ON-strand" + strandNum : "OFF"),
            "STRAND_PROGRESS:" + strandTrades.length + "/" + STRAND_SIZE,

            "TOKEN_WAVES:" + tokenData.filter(t => t.range).map(t =>
              t.sym + ":range=" + t.range + "%,P=" + t.peaks + ",T=" + t.troughs + ",score=" + t.score + "[" + t.tier + "]"
            ).join("|"),

            "OPEN_POSITIONS:" + tokenData.filter(t => t.entry).map(t =>
              t.sym + "@$" + (t.entry||0).toFixed(6)
            ).join(",") || "none",

            "TOP_PERFORMERS:" + tokenData.sort((a,b) => b.score - a.score).slice(0,5).map(t =>
              t.sym + "=" + t.score + "[" + t.tier + "]"
            ).join(","),

            "RECENT_TRADES:" + tradeLog.slice(-20).map(t =>
              t.type + ":" + t.symbol + (t.netUsd !== undefined ? (t.netUsd>=0?"+$":"-$") + Math.abs(t.netUsd||0).toFixed(2) : "")
            ).join("|"),

            "REGISTRY_FILES:vita-registry.json,ledger.json,positions.json,tokens.json,history.json",
          ...
          try {
            const rfRes = await fetch(
              "https://api.github.com/repos/" + ghRepo + "/contents/vita-registry.json?ref=" + ghBranch + "&t=" + Date.now(),
              { headers: { Authorization: "token " + ghToken } }
            );
            if (rfRes.ok) {
              const rfData = await rfRes.json();
              regSha = rfData.sha;
              registry = JSON.parse(Buffer.from(rfData.content.replace(/\n/g,""), "base64").toString("utf8"));
            }
          } catch {}
          ...
          const encoded = Buffer.from(JSON.stringify(registry,null,2)).toString("base64");
          const payload = { message:"VITA: trading data snapshot", content:encoded, branch:ghBranch };
          if (regSha) payload.sha = regSha;
          await fetch(
            "https://api.github.com/repos/" + ghRepo + "/contents/vita-registry.json",
            { method:"PUT", headers:{ Authorization:"token "+ghToken, "Content-Type":"application/json" }, body:JSON.stringify(payload) }
      ...
      // ── /vitalearn TOPIC — inject a knowledge base from text ───────────────
      // Usage: /vitalearn einstein
      //        /vitalearn [paste any text after the command]
      } else if (text && text.startsWith("/vitalearn ")) {
        ...
        if (!vitaKey) { await tg("❌ VITA needs VITA_ANTHROPIC_KEY — run /newvault VITA_ANTHROPIC_KEY"); }
        else {
          // Built-in knowledge bases
          const KNOWLEDGE_BASES = {
            einstein: `Albert Einstein (1879-1955) physicist.
SPECIAL RELATIVITY 1905: laws of physics same in all inertial frames. Speed of light c=299792458m/s constant for all observers. Consequences: time dilation γ=1/√(1-v²/c²), length contraction, mass-energy E=mc². No mass can reach speed of light.
GENERAL RELATIVITY 1915: gravity=curvature of spacetime. Equivalence principle: gravity indistinguishable from acceleration. Einstein field equations: Gμν+Λgμν=(8πG/c⁴)Tμν. Predictions confirmed: light bending (Eddington 1919), gravitational waves (LIGO 2015), GPS time correction.
QUANTUM: explained photoelectric effect (Nobel 1921) — light comes in photons E=hf. Rejected QM completeness: "God does not play dice." EPR paradox 1935 — spooky action at distance.
QUOTES: "Imagination more important than knowledge." "God does not play dice." "Most incomprehensible thing about universe is it is comprehensible." "Everything simple as possible but not simpler."
DATES: Born Mar 14 1879 (Pi Day). Annus Mirabilis 1905 (4 papers). Nobel 1921. Fled Germany 1933. Died Apr 18 1955 Princeton. Never wore socks. Declined Israel presidency 1952.
VERIFY: c=299792458, Nobel=1921, born=1879-03-14, died=1955-04-18, LIGO detection=2015-09-14`,
          };

          const knowledgeText = KNOWLEDGE_BASES[topic.toLowerCase()] || topic;
          const isBuiltIn = !!KNOWLEDGE_BASES[topic.toLowerCase()];
          ...
          try {
            ...
            const encoded = Buffer.from(JSON.stringify(registry,null,2)).toString("base64");
            const payload = { message:"VITA: knowledge "+topic, content:encoded, branch:ghBranch };
            if (regSha) payload.sha = regSha;
            await fetch(
              "https://api.github.com/repos/" + ghRepo + "/contents/vita-registry.json",
              { method:"PUT", headers:{ Authorization:"token "+ghToken, "Content-Type":"application/json" }, body:JSON.stringify(payload) }
      ...
      // ── /waveproof — capped 3-token WAVE live proof (WAVE_PROOF_LIVE, not VITAFEED_PAID)
        ...
        try {
          ...
          const ctx = await buildWaveProofLiveContext({ cdp, bal, ethUsd });
      ...
      // ── /wavetest — WAVE memory-mirror SIM (does not enable VITAFEED_PAID)
        ...
        // ── /vita [question] — answer from memory registry ──────────────────
          ...
          const question = vitaInput;
          await tg("🌟 <b>VITA thinking...</b>");
          try {
            // Load registry directly from state branch — explicit URL avoids any caching
            let registry = {};
            const ghRepo   = process.env.GITHUB_REPO;
            const ghToken  = process.env.GITHUB_TOKEN;
            const ghBranch = process.env.STATE_BRANCH || STATE_BRANCH || "bot-state";
            ...
            try {
              ...
              if (rfRes.ok) {
                ...
                const decoded = Buffer.from(rfData.content.replace(/\n/g,""), "base64").toString("utf8");
                registry = JSON.parse(decoded);
                console.log("🌟 VITA: loaded registry from " + ghBranch + " — " + Object.keys(registry).length + " entries: " + Object.keys(registry).join(", "));
              } else {
                console.log("🌟 VITA: registry not found — status " + rfRes.status + " on branch " + ghBranch);
              }
            } catch (e) {
              console.log("🌟 VITA: registry read error: " + e.message);
            }
            ...
            // Sort newest first
            entries.sort((a, b) => (b[1].filedAt||b[0]) > (a[1].filedAt||a[0]) ? 1 : -1);

            // Find best match — keyword search across all entries
            let bestEntry = null;
            for (const [key, val] of entries) {
              if (key.toLowerCase().includes(qLower) ||
                  (val.label||"").toLowerCase().includes(qLower) ||
                  (val.subject||"").toLowerCase().includes(qLower) ||
                  (val.tokenPacket||"").toLowerCase().includes(qLower.slice(0,20))) {
                bestEntry = val; break;
              }
            }
            // "what did we learn" or "today" → most recent
            if (!bestEntry && (qLower.includes("learn") || qLower.includes("today") || qLower.includes("recent") || qLower.includes("last"))) {
              bestEntry = entries[0]?.[1];
            }
            ...
            // Build context — fetch FULL chunks from blockchain if available
            let fullMemory = "";
            if (bestEntry?.txHashes?.length > 0) {
              try {
                // Fetch all 5 chunks from Base directly
                const { fetchTxCalldata } = await import("./vault-loader.js");
                const parts = [];
                for (const txHash of bestEntry.txHashes) {
                  try {
                    const calldata = await fetchTxCalldata(txHash);
                    // Strip the [VITA:...] header
                    const content = calldata.replace(/^\[VITA:[^\]]+\]/, "").trim();
                    parts.push(content);
                  } catch {}
                }
                if (parts.length > 0) fullMemory = parts.join("");
              } catch {}
            }

            // Fall back to registry tokenPacket if chain fetch failed
            const memContent = fullMemory || (bestEntry?.tokenPacket || "");
            const memSource  = fullMemory ? "FULL BLOCKCHAIN DATA" : "REGISTRY PREVIEW";

            const memContext = bestEntry
              ? "VITA MEMORY [" + bestEntry.date + " — " + (bestEntry.label||bestEntry.subject||"session") + "] [" + memSource + "]:\n" + memContent
              : "No prior memory found.";

            const toc = entries.length > 0
              ? "\n\nALL VITA MEMORY (newest first):\n" + entries.map(([k,v]) =>
                  "- " + k + (v.type === "knowledge-base" ? " [KNOWLEDGE]" : " [SESSION]")
                ).join("\n")
              : "";
            ...
            // ── SOURCE ENFORCER — IKN Integrity Agent Phase 1 ──────────────
            // Scans answer for unsourced factual claims before sending
            const enforcement = enforceSource(answer);
            if (!enforcement.clean) {
              console.log('§INTEGRITY§ Source enforcer flagged: ' + enforcement.issues.join('; '));
            }
            const finalAnswer = enforcement.enforced;

            // Label source in Telegram message
            const isVerified = finalAnswer.includes("BLOCKCHAIN VERIFIED");
            const sourceTag  = isVerified ? "⛓️ blockchain" : "💭 LLM estimate";

            await tg(
              "🌟 <b>VITA</b>\n━━━━━━━━━━━━━━━━━━━━\n" +
              (bestEntry ? "📁 <i>" + (bestEntry.label||bestEntry.subject) + "</i> · " + sourceTag + "\n\n" : "") +
              finalAnswer + "\n\n<i>— VITA | ᛞᚨᚡᛁᛞ | IKN</i>"
            );
      ...
      // ── 🧠 MEMORY COMMANDS ──────────────────────────────────────────────────
        ...
        // Save a cliff note immediately — rides next trade as BTP
        ...
        if (results.length === 0) {
          await tg("🧠 No memories found for: <b>" + query + "</b>\n\nTry /memories to see all");
        } else {
          let msg = "🧠 <b>MEMORY SEARCH: " + query + "</b>\n";
          msg += "Found " + results.length + " result(s)\n━━━━━━━━━━━━━━━━━━━━\n\n";
          for (const m of results.slice(0, 5)) {
            msg += (m.type === "FS" ? "📚" : "📌") + " <b>#" + m.seq + "</b> " + m.date + "\n";
            msg += "   " + m.preview + "\n";
            msg += "   <a href=\"" + m.basescan + "\">↗ BaseScan</a>\n\n";
          }
          if (results.length > 5) msg += "<i>...and " + (results.length-5) + " more.</i>";
          await tg(msg);
        }
        ...
        const recent = getRecentCliffNotes(3);
        if (recent.length === 0) {
          await tg("🧠 No memories yet — use /remember to save your first cliff note");
        } else {
          let msg = "🧠 <b>RECENT MEMORY CONTEXT</b>\n━━━━━━━━━━━━━━━━━━━━\n";
          for (const m of recent) {
            msg += "📌 <b>#" + m.seq + "</b> " + m.date + "\n";
            msg += "   " + m.preview + "\n";
            msg += "   <a href=\"" + m.basescan + "\">↗ BaseScan</a>\n\n";
          }
          await tg(msg);
      ...
      // ── 💓 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        const { fetchTxCalldata } = await import("./vault-loader.js");
        await tg("💓 Fetching VITA memory strand from Base blockchain...");
        try {
        ...
        BTP_INSCRIPTIONS_ENABLED = true;
        await tg(
          "📡 <b>BTP INSCRIPTIONS ON</b>\n" +
          "Every trade now carries a VITA message on Base.\n" +
          "Inscription cost included in profitability gate.\n\n" +
          "⚠️ Only use when wallet is funded enough that\n" +
          "inscription cost is negligible vs trade size."
        );

      } else if (text === "/btpoff") {
        BTP_INSCRIPTIONS_ENABLED = false;
        await tg(
          "📡 <b>BTP INSCRIPTIONS OFF</b>\n" +
          "Trades only — no inscription overhead.\n" +
          "Inscription cost removed from margin calculation.\n\n" +
          "✅ Recommended at small capital sizes.\n" +
          "Turn back on with /btpon when wallet grows."
        );

      } else if (text === "/btpstatus") {
        const pending = btpPendingCount();
        const gasCost = await estimateGasCostEth();
        const gasPriceEth = gasCost / 500_000;
        const btpCostEth  = gasPriceEth * BTP_GAS_UNITS;
        const btpCostUsd  = btpCostEth * (cachedEthUsd || 2100);

        if (!BTP_INSCRIPTIONS_ENABLED) {
          await tg(
            "📡 <b>BTP STATUS — OFF</b>\n\n" +
            "🔴 Inscriptions disabled — saving gas\n" +
            "💰 Saves ~$" + btpCostUsd.toFixed(4) + " per trade\n" +
            "📊 Min margin gate: " + (MIN_NET_MARGIN * 100).toFixed(1) + "%\n\n" +
            "Turn on: /btpon"
          );
        } else if (btpQueue.length === 0 || btpQueue.every(m => m.name === "VITA")) {
          await tg(
            "📡 <b>BTP STATUS — ON</b>\n\n" +
          ...
          await tg(
            "📡 <b>BTP STATUS — ON</b>\n\n" +
            `⏳ <b>${pending} chunk(s) pending</b>\n\n` +
            qLines +
            `\n💰 Cost per trade: ~$${btpCostUsd.toFixed(4)}\n` +
            "Toggle: /btpoff to save gas"
          );
        }
...
async function main() {
  ...
  if (hasRailwayPassword) {
    // ── Legacy mode: DECRYPT_PASSWORD in Railway — load immediately ──────────
    // Works exactly as before. Remove DECRYPT_PASSWORD from Railway to upgrade to Stage 1.
    console.log("🔐 Vault: DECRYPT_PASSWORD in Railway — loading directly (legacy mode)");
    try {
      const vault = await loadVaultKeys();
      exitSafeMode();
      console.log("✅ Vault loaded in legacy mode — " + vault.fromChain + " from chain, " + vault.fromRailway + " from Railway");
    } catch (vaultErr) {
      console.log("⚠️  Vault loader error: " + vaultErr.message);
    }
  } else {
    // ── Stage 1 mode: no password in Railway — request via Telegram ──────────
    // Bot sends unlock request, waits up to 5 minutes for password reply.
    // If no reply → safe mode (monitoring only, no trades).
    console.log("🔑 Stage 1 Vault: no DECRYPT_PASSWORD in Railway — requesting via Telegram...");
    ...
    try {
      await tg(
        "🔑 <b>GUARDIAN VAULT — UNLOCK REQUIRED</b>\n" +
        "━━━━━━━━━━━━━━━━━━━━\n\n" +
        "Bot started but vault is locked.\n" +
        "Send your password to unlock:\n\n" +
        "<code>/unlock yourPassword</code>\n\n" +
        "⏳ You have <b>5 minutes</b> to respond.\n" +
        "After that → safe mode (no trades, monitoring only).\n\n" +
        "🛡️  Safe mode: active until unlocked"
      );
    } catch (e) {
      console.log("⚠️  Could not send unlock request via Telegram: " + e.message);
    ...
    console.log("⏳ Waiting up to 5 minutes for vault unlock via Telegram...");
    ...
    if (isUnlocked()) {
      await tg(
        "✅ <b>VAULT UNLOCKED — Guardian is armed</b>\n" +
        "🔑 Password cleared from memory\n" +
        "⏱️  Session valid for 24 hours\n" +
        "💌 <i>Eureka! VITA lives ♥ — ᛞᚨᚡᛁᛞ</i>"
      );
    } else {
      enterSafeMode("no unlock response within 5 minutes at boot");
      console.log("⏰ No unlock received — entering safe mode");
      try {
        await tg(
          "⏰ <b>VAULT UNLOCK TIMEOUT</b>\n" +
          "No password received within 5 minutes.\n\n" +
          "🛡️  Safe mode active:\n" +
          "   ✅ Monitoring all tokens\n" +
          "   ✅ Sending alerts\n" +
          "   🚫 No trades until unlocked\n\n" +
  ...
  // Writes ikn-boot-context.txt so next Claude session can pull immediately.
  // Non-critical — failure here never stops trading.
  try {
    console.log("📚 IKN: reading knowledge index from vita-registry...");
    const iknContext = await iknBootReader();
    if (iknContext) {
      console.log("📚 IKN: context armed — " + iknContext.split("\n").length + " lines loaded");
    } else {
      console.log("📚 IKN: no prior strands found — fresh registry");
    }
  } catch (iknErr) {
  ...
  // Then cross-reference with ledger.json for entry prices
  // This NEVER fails silently — if ledger is empty we still know what we hold
  try {
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log("🔍 ON-CHAIN SCAN — reading wallet state from Base blockchain...");
    ...
    for (const result of balanceResults) {
      if (result.status !== "fulfilled") continue;
      const { symbol, bal } = result.value;
      tokenBalanceCache[symbol] = bal;
      ...
      if (hasRealHolding) {
        ...
        console.log(`   👻 GHOST ${symbol}: entry=$${token.entryPrice.toFixed(6)} but chain balance=${bal.toFixed(6)} — clearing`);
        token.entryPrice       = null;
        token.totalInvestedEth = 0;
        token.entryTime        = null;
        token.unknownEntry     = false;
    ...
    bootMsg += `\n💰 Total held: ~$${totalHeld.toFixed(2)}\n`;
    bootMsg += `📡 BTP: ${BTP_INSCRIPTIONS_ENABLED ? "ON ✅" : "OFF 🔴"} | Min margin: ${(MIN_NET_MARGIN*100).toFixed(1)}%\n`;
    bootMsg += `<i>Source: Base blockchain · Ledger: ${Object.keys(netPositions).length > 0 ? Object.keys(netPositions).length + " tokens" : "empty"}</i>`;
    await tg(bootMsg);
  ...
  // Desk one-shot: WAVE_PROOF_AUTOFIRE=yes + WAVE_PROOF_LIVE=yes → same 3-send batch (no Telegram).
  try {
    const auto = await maybeAutofireWaveProofOnBoot(process.env);
    if (auto?.fired || auto?.autofire) {
      console.log("🌊 WAVE_PROOF_AUTOFIRE: " + (auto.reason || auto.reply || (auto.fired ? "fired" : "cleared")));
    }
  } catch (e) {
  ...
  // v18: NEVER hardcode USD values. Read every token balance live from chain.
  ...
  try {
    ...
    for (const token of tokens) {
      ...
      try {
        ...
        console.log(`   ⚠️  ${token.symbol}: reconcile failed — ${tokenErr.message}`);
      }
    }
    console.log(`🔗 Chain reconciliation complete: ${reconciled} live positions found`);
    if (reconciled > 0) {
      // Build a clean summary from what we ACTUALLY found on chain
      let posLines = "";
      for (const token of tokens) {
        if (!token.entryPrice || token.frozen) continue;
        const bal = getCachedBalance(token.symbol);
        if (bal < 0.001) continue;
        const price = history[token.symbol]?.lastPrice || token.entryPrice;
        const usd   = bal * price;
        posLines += `  ${token.symbol}: ${bal >= 1 ? Math.floor(bal) : bal.toFixed(4)} tokens ≈ $${usd.toFixed(3)}\n`;
      }
      await tg(
  ...
  // v18.1: Pool health check now covers ALL active (non-frozen) tokens, not a hardcoded list
  const BATCH2_SYMBOLS = tokens.filter(t => !t.frozen && !t.disabled).map(t => t.symbol);
  ...
  await tg(
    ...
    `🛑 Drawdown breaker: -${(DRAWDOWN_HALT_PCT*100).toFixed(0)}% halts buys (resets on near-zero)\n` +
    `❄️ Frozen: ${tokens.filter(t=>t.frozen).length} tokens collecting data | Active: ${tokens.filter(t=>!t.frozen&&!t.disabled).length} tokens\n` +
    `🔗 Chain truth: all balances from ${WALLET_ADDRESS.slice(0,8)}...${WALLET_ADDRESS.slice(-6)}\n` +
  ...
  while (true) {
    try {
      ...
      // FIX v18: use live chain balance (bal is already freshly fetched above),
      // and live token balances from refreshTokenBalances() called just below.
      // We compute portfolio AFTER refreshTokenBalances so token values are current.
      let totalPortfolioUsd = bal.total * ethUsd;
      for (const t of tokens) {
        if (t.frozen) continue; // frozen tokens have no capital
        if (t.entryPrice) {
          const p = history[t.symbol]?.lastPrice;
      ...
      // Refresh macro BTC/ETH trend signal every 4 hours
      if (!global._lastMacroRefresh || Date.now() - global._lastMacroRefresh > 4 * 3600_000) {
        global._lastMacroRefresh = Date.now();
        refreshMacroSignal().catch(() => {});
      }
      ...
      if (activeSurfers.length > 0) {
        const riding  = activeSurfers.filter(s => s.status === "RIDING").length;
        const waiting = activeSurfers.filter(s => s.status === "WAITING" || s.status === "IDLE").length;
      ...
      if (BTP_INSCRIPTIONS_ENABLED && !btpAutoSuspended && tradeableUsd < BTP_AUTO_SUSPEND_USD) {
        btpAutoSuspended = true;
        console.log(`📡 BTP AUTO-SUSPENDED: tradeable $${tradeableUsd.toFixed(2)} < $${BTP_AUTO_SUSPEND_USD} — saving gas for trades`);
        tg(`📡 <b>BTP AUTO-SUSPENDED</b>\nTradeable: $${tradeableUsd.toFixed(2)}\nAt < $${BTP_AUTO_SUSPEND_USD} tradeable, inscription gas > trade benefit.\nBTP will auto-resume at $${BTP_AUTO_SUSPEND_USD * 2}+\nUse /btpon to override.`).catch(()=>{});
      } else if (btpAutoSuspended && tradeableUsd >= BTP_AUTO_SUSPEND_USD * 2) {
        btpAutoSuspended = false;
        console.log(`📡 BTP AUTO-RESUMED: tradeable $${tradeableUsd.toFixed(2)} ≥ $${BTP_AUTO_SUSPEND_USD * 2}`);
      }
      // When auto-suspended, treat as if BTP is off for margin calculation
      // (but the user's manual /btpon setting takes precedence)
      const effectiveBtpEnabled = BTP_INSCRIPTIONS_ENABLED && !btpAutoSuspended;
      ...
      // Catches ghost positions, missed sells, and balance drift instantly
      // This is the key lesson from top bots — NEVER trust memory over chain state
      if (!global._lastReconcile || Date.now() - global._lastReconcile > 60_000) {
        global._lastReconcile = Date.now();
        for (const token of tokens) {
          if (!token.entryPrice) continue; // only check open positions
          try {
            const realBal = await getTokenBalance(token.address);
            tokenBalanceCache[token.symbol] = realBal;
            const price   = history[token.symbol]?.lastPrice || token.entryPrice;
      ...
      // Known bags recycle when starved IF leftover after fees > 0 (lose-zero).
      {
        ...
        const moonPiggy = applyPiggyToSell({
          ...
          reason: moonReason,
          token,
          savedEarningsUsd: effectiveSavedEarningsUsd({
            savedEarningsUsd: tokenPiggyLedgers[token.symbol]?.savedEarningsUsd || token.savedEarningsUsd || 0,
            dustUsd: tokenPiggyLedgers[token.symbol]?.dustUsd || ((token.piggyReserve || 0) * price),
            piggyMinUsd: piggyBankMinUsd(process.env, piggyOptsFromToken(token)),
          }),
        });
      ...
      // Phase 1: Chain Validator + Source Enforcer
      // Phase 2: Drift Detector (requires VITA_ANTHROPIC_KEY)
      if (!global._lastIntegrityCheck || Date.now() - global._lastIntegrityCheck > 6 * 60 * 60 * 1000) {
        global._lastIntegrityCheck = Date.now();
        try {
          console.log('§INTEGRITY§ Running scheduled integrity check...');
          await runIntegrityCheck(tg);
        } catch (intErr) {
          console.log('⚠️  Integrity check error (non-critical): ' + intErr.message);
        }
      }

      // ── VITA QUEUE PROCESSOR v17.0 — IKN FILING PROTOCOL ─────────────────
      // Checks vita-queue/ folder on GitHub every 5 minutes.
      // Routes ikn-strand entries through IKN filing (DDC/IKN-X + trust stamp).
      // Legacy knowledge-base entries processed exactly as before.
      if (!global._lastQueueCheck || Date.now() - global._lastQueueCheck > 300_000) {
        global._lastQueueCheck = Date.now();
        try {
          const ghRepo   = process.env.GITHUB_REPO;
          const ghToken  = process.env.GITHUB_TOKEN;
          const ghBranch = process.env.STATE_BRANCH || STATE_BRANCH || "bot-state";

          // List files in vita-queue/ folder
          const listRes = await fetch(
            "https://api.github.com/repos/" + ghRepo + "/contents/vita-queue?ref=" + ghBranch + "&t=" + Date.now(),
            { headers: { Authorization: "token " + ghToken } }
          );

          if (listRes.ok) {
            const files = await listRes.json();
            const queueFiles = Array.isArray(files) ? files.filter(f => f.name.endsWith(".json")) : [];

            if (queueFiles.length > 0) {
              console.log("📬 VITA queue: " + queueFiles.length + " file(s) waiting");
              await tg("📬 <b>VITA QUEUE</b> — " + queueFiles.length + " knowledge file(s) found\n⏳ Processing...");
            }
            ...
            for (const file of queueFiles) {
              try {
                ...
                const entry   = await fileRes.json();
                if (!entry.vitaQueueEntry) continue;

                console.log("📬 Processing: " + file.name + " — " + entry.label);

                // ── IKN STRAND ROUTING ──────────────────────────────────────
                // ikn-strand entries use the IKN filing protocol:
                // DDC anchor + IKN-X call number + trust stamp + registrar update
                if (entry.type === "ikn-strand") {
                  console.log("📚 IKN strand detected — routing to IKN filing protocol");
                  const queueFilePath = "vita-queue/" + file.name;

                  // Inscribe the 5 chunks to Base first (same mechanism as legacy)
                  const text      = entry.tokenPacket;
                  const date      = new Date().toISOString().slice(0,10);
                  const iknX      = entry.iknCard?.["IKN-X"] || "IKN.000." + date;
                  const strandId  = "IKN-" + iknX.replace(/\./g,"-").slice(0,20);
                  const chunkSize = Math.ceil(text.length / 5);
                  const txHashes  = [];
                  let prevHash    = "00000000";

                  for (let i = 0; i < 5; i++) {
                    const content  = text.slice(i * chunkSize, (i+1) * chunkSize);
                    const header   = "[IKN:" + strandId + ":" + String(i+1).padStart(2,"0") + "/05:" + date + ":" + prevHash + "]";
                    const full     = header + content;
                ...
                // ── LEGACY KNOWLEDGE-BASE PROCESSING ───────────────────────
                // Unchanged from v16 — handles all non-ikn-strand entries exactly as before

                // Split tokenPacket into 5 chunks and inscribe
                const text      = entry.tokenPacket;
                const date      = new Date().toISOString().slice(0,10);
                const strandId  = "VITA-KNOW-" + entry.label.toUpperCase().slice(0,12).replace(/[^A-Z0-9]/g,"-");
                const chunkSize = Math.ceil(text.length / 5);
                const txHashes  = [];
                let prevHash    = "00000000";
                ...
                const encoded = Buffer.from(JSON.stringify(registry,null,2)).toString("base64");
                const payload = { message: "VITA: inscribed " + entry.label, content: encoded, branch: ghBranch };
                if (regSha) payload.sha = regSha;
                await fetch(
                  "https://api.github.com/repos/" + ghRepo + "/contents/vita-registry.json",
                  { method:"PUT", headers:{ Authorization:"token "+ghToken, "Content-Type":"application/json" }, body:JSON.stringify(payload) }
                ...
                // Delete the queue file — confirmed on chain, no longer needed
                const delRes = await fetch(
                  "https://api.github.com/repos/" + ghRepo + "/contents/vita-queue/" + file.name,
                  {
                    method: "DELETE",
                    headers: { Authorization: "token " + ghToken, "Content-Type": "application/json" },
                    body: JSON.stringify({ message: "VITA: processed " + file.name, sha: file.sha, branch: ghBranch })
                  }
                );
                ...
                console.log("⚠️  Queue error on " + file.name + ": " + qErr.message);
                await tg("⚠️ Queue processing failed for " + file.name + ": " + qErr.message);
              }
            }
          }
        } catch (queueErr) {
          // Non-critical — queue check failure doesn't affect trading
          console.log("⚠️  Queue check error: " + queueErr.message);
        }
      }

      // ── Vault expiry warning ───────────────────────────────────────────────
      if (shouldWarnExpiry()) {
        await tg(getExpiryWarningMessage());
      }
      // Keep webhook state fresh every cycle
      updateWebhookState();
      // ── Auto-lock on TTL expiry ────────────────────────────────────────────
      if (unlockState.unlocked && unlockState.unlockedAt) {
        if (Date.now() - unlockState.unlockedAt > UNLOCK_TTL) {
          lockVault("24h session expired");
          await tg(
            "🔒 <b>VAULT SESSION EXPIRED</b>\n" +
            "24 hours elapsed — vault locked for security.\n" +
            "Safe mode active — no trades.\n\n" +
