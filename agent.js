  ...
  try {
    ...
    if (BTP_INSCRIPTIONS_ENABLED) {
      try {
        ...
        // If this completes a strand, close it
...
async function main() {
  ...
  // The ledger on bot-state branch is the permanent truth.
  // Every restart: read ledger → find net open positions → scan wallet balances
  // → restore entry prices so the bot knows exactly where it stands.
  // This runs AFTER cdpClient is ready so getTokenBalance works.
  try {
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    console.log("🔍 ON-CHAIN POSITION RECOVERY — reading ledger truth...");

    const ethPriceNow = await getLiveEthPrice();

    // Read ledger — permanent record of every trade ever made
    const lf = await githubGetFromBranch("ledger.json", STATE_BRANCH);
    const ledgerTrades = lf?.content?.trades || [];
    console.log(`   📖 Ledger: ${ledgerTrades.length} trades found`);

    // Build NET position per symbol from full trade history
    // Track: last buy price, total ETH spent, total ETH received
    // If buys > sells → position is open, restore entry
    const netPositions = {};
    for (const t of ledgerTrades) {
      if (!t.symbol || !t.price || t.price <= 0) continue;
      if (!netPositions[t.symbol]) {
        netPositions[t.symbol] = { ethIn: 0, ethOut: 0, lastBuyPrice: 0, lastBuyTime: 0, trades: 0 };
      }
      const p = netPositions[t.symbol];
      if (t.type === "BUY") {
        p.ethIn        += t.ethSpent || 0;
        p.lastBuyPrice  = t.price;
        p.lastBuyTime   = new Date(t.timestamp||0).getTime() || 0;
        p.trades++;
      }
      if (t.type === "SELL") {
        p.ethOut += t.receivedEth || 0;
        p.trades++;
      }
    }

    // Now scan actual wallet balances and reconcile
    let recovered = 0, ghosts = 0, confirmed = 0;
    for (const token of tokens) {
      try {
        // Real on-chain balance — this is the truth
        const bal      = await getTokenBalance(token.address);
        tokenBalanceCache[token.symbol] = bal;
        const net      = netPositions[token.symbol];
        const ethNetIn = net ? Math.max(0, net.ethIn - net.ethOut) : 0;
        const valueUsd = bal * (net?.lastBuyPrice || 0.000001);

        if (bal > 0.001 && valueUsd > 0.20) {
          // Real balance exists on-chain
          if (!token.entryPrice) {
            // No entry recorded — recover from ledger
            if (net?.lastBuyPrice) {
              token.entryPrice       = net.lastBuyPrice;
              token.totalInvestedEth = ethNetIn > 0 ? ethNetIn : (valueUsd / ethPriceNow);
              token.entryTime        = net.lastBuyTime || Date.now();
              recovered++;
              console.log(`   ✅ RECOVERED ${token.symbol}: entry=$${net.lastBuyPrice.toFixed(6)} ethIn=${ethNetIn.toFixed(4)} val=$${valueUsd.toFixed(2)}`);
            } else {
              // No ledger entry at all — set current price, zero cost basis
              const currentPrice = history[token.symbol]?.lastPrice || net?.lastBuyPrice || 0.000001;
              token.entryPrice       = currentPrice;
              token.totalInvestedEth = valueUsd / ethPriceNow;
              token.entryTime        = Date.now();
              recovered++;
              console.log(`   ⚠️  RECOVERED ${token.symbol} (no ledger): entry=$${currentPrice.toFixed(6)} assumed`);
            }
          } else {
            confirmed++;
          }
        } else if (bal <= 0.001 && token.entryPrice) {
          // Ghost — entry set but no real balance
          console.log(`   👻 GHOST cleared: ${token.symbol} entry=$${token.entryPrice.toFixed(6)} but balance=${bal.toFixed(6)}`);
          token.entryPrice       = null;
          token.totalInvestedEth = 0;
          token.entryTime        = null;
          ghosts++;
        ...
        console.log(`   ⚠️  ${token.symbol} balance check failed: ${e.message?.slice(0,40)}`);
      }
    }

    // Summary
    const openPositions = tokens.filter(t => t.entryPrice);
    console.log(`🔍 Recovery complete: ${recovered} recovered | ${confirmed} confirmed | ${ghosts} ghosts cleared`);
    console.log(`📊 Open positions: ${openPositions.length > 0 ? openPositions.map(t => t.symbol + "@$" + t.entryPrice.toFixed(6)).join(", ") : "none"}`);
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

    // Send boot summary to Telegram
    if (recovered > 0 || ghosts > 0) {
      let msg = "🔍 <b>POSITION RECOVERY AT BOOT</b>\n━━━━━━━━━━━━━━━━━━━━\n\n";
      if (recovered > 0) {
        msg += "✅ <b>Recovered " + recovered + " position(s) from ledger:</b>\n";
        tokens.filter(t => t.entryPrice).forEach(t => {
          const val = tokenBalanceCache[t.symbol] * t.entryPrice;
          msg += `   ${t.symbol}: entry $${t.entryPrice.toFixed(6)} (~$${val.toFixed(2)})\n`;
        });
      }
      if (ghosts > 0) msg += "\n👻 Cleared " + ghosts + " ghost position(s)\n";
      msg += "\n<i>Ledger is the source of truth. The chain never forgets.</i>";
      await tg(msg);
    }
  } catch (e) {
    console.log(`⚠️  Position recovery error: ${e.message}`);
  }
