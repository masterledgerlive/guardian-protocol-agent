...
async function runGuardianAgent() {
  ...
  const agent = createReactAgent({
    llm,
    tools,
    messageModifier: `You are the Guardian Protocol Agent running on 
    Base Sepolia testnet. After every action report exactly what happened 
    including wallet address, transaction hash, and balance in plain 
    simple language.`,
  });
  ...
  const step1 = await agent.invoke({
    messages: [new HumanMessage(
      "Get my wallet address and tell me which network we are on. Show the full address clearly."
    )],
  });
  ...
  const step2 = await agent.invoke({
    messages: [new HumanMessage(
      "Check my ETH balance. If it is 0 request funds from the Base Sepolia faucet. Show the balance clearly."
    )],
  });
  console.log(step2.messages[step2.messages.length - 1].content);

  console.log("\n--- STEP 3: Sending Test Transaction ---");
  const step3 = await agent.invoke({
    messages: [new HumanMessage(
      "Send 0.000001 ETH to 0x4D9925f10A22b5C8e1F72E3eE9E35B5c3D0A7b9 and show me the transaction hash and explorer link."
    )],
  });
  console.log(step3.messages[step3.messages.length - 1].content);

  console.log("\n⬡ Guardian Protocol — Sequence Complete");
}
...
```
SKILLMD NOTE:
agent.js is the brain of your Guardian agent.
It connects Claude AI to your Coinbase wallet.
Runs three tasks: get wallet, check balance,
send test transaction. All on Base Sepolia testnet.
Real transactions. Real proof. Zero real money.
