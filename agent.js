...
// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v14.0
// ═══════════════════════════════════════════════════════════════════════════════
...
const PROCESS_TOKEN_TIMEOUT = 12_000; // max time per token in main loop before we move on
...
const MIN_ETH_TRADE     = 0.0005;  // lowered: auto gas top-up keeps ETH available
const MIN_POS_USD       = 1.50;    // lowered: bot was blocking itself at $2.38 tradeable
...
const DEFAULT_TOKENS = [
  ...
  // NORMIE removed — token is at $0.00000001 (essentially worthless), no recovery possible.
  // Was timing out the main loop every single cycle for zero upside. Removed 2026-03-14.
...
// If a token returns no price for NO_PRICE_SKIP_AFTER consecutive loops AND
// has no open position, skip it entirely until it gets a price again.
// This stops dead/thin tokens from burning 12s timeouts every single cycle.
// Tokens with open positions are NEVER skipped regardless of streak.
const NO_PRICE_SKIP_AFTER = 10;  // skip after 10 straight no-price loops (~2.5 min)
const noPriceStreak = {};        // { [symbol]: count }
...
function checkFibTargetHit(symbol, currentPrice, entryPrice, gasCostEth, tradeEth, ethUsd) {
  ...
  // Find the highest target that has been reached (price >= target.price)
  // We hit targets from the bottom up — each one is a partial exit
  const hitTargets = targets.exitLadder.filter(t => currentPrice >= t.price * 0.998); // 0.2% tolerance
  if (!hitTargets.length) return null;

  // Return the highest hit target — that's the one we act on now
  return {
...
async function processToken(cdp, token, bal) {
  try {
    ...
    // Check if price has hit any Fibonacci extension level above entry.
    // This is the "always hit a mark" system — even if we miss the max peak,
    // we catch the 100% level (prior high) which is always profitable.
    // On each hit, we do a PARTIAL EXIT (sellPct from the fib ladder), not full.
    // This keeps capital compounding upward while locking in real profits at each step.
    ...
    if (shouldFibExit) {
      ...
      const nextTarget = targets.exitLadder.find(t => t.price > target.price);
  ...
  try {
    ...
    for (const [sym, h] of Object.entries(history)) {
      historyToSave[sym] = {
        ...
        // Strip the large candles object from saves — it's re-fetched on boot
        // and was the main cause of history.json growing past the 1MB API limit.
        candles: undefined,
      };
    }

    // Guard: only save if JSON serializes cleanly — corrupt saves break every restart
    let historyJson;
    try {
      historyJson = JSON.stringify(historyToSave);
      JSON.parse(historyJson); // verify it round-trips
    } catch (jsonErr) {
      console.log(`💾 history.json skipped — serialization failed: ${jsonErr.message}`);
      historyJson = null;
    }
    if (historyJson) {
    ...
    for (const upd of data.result) {
      ...
      try {
        ...
        await tg(
          ...
          `<b>Active tokens (28):</b> AERO BRETT VIRTUAL MORPHO CBBTC DEGEN WELL SEAM AIXBT TOSHI KITE KEYCAT DOGINME XCN SKI PRIME LUNA HIGHER MOCHI ZORA BNKR TYBG MIGGLES BENJI ROOST TALENT`
        );
      }
      } catch (cmdErr) {
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════");
  console.log("⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v14.3");
  ...
  console.log("      v14.3: NORMIE removed | skip-dead-tokens | history.json fix");
  ...
  await tg(
    `⚔️💓 <b>GUARDIAN v14.3 — HEARTBEAT EDITION ONLINE</b>\n\n` +
  ...
  while (true) {
    try {
      ...
      for (const token of tokens) {
        try {
          // ── SKIP TOKENS WITH NO PRICE DATA (unless holding a position) ──────
          // Tokens that repeatedly return no price burn the full 12s timeout each
          // loop. If we have no position in a token AND it's been pricesless for
          // NO_PRICE_SKIP_AFTER consecutive loops, skip it this cycle so the bot
          // can focus time on the tokens that actually matter right now.
          const hasOpenPosition = token.entryPrice != null;
          const streak = noPriceStreak[token.symbol] || 0;
          if (!hasOpenPosition && streak >= NO_PRICE_SKIP_AFTER) {
            // Still try a quick price check every 5th cycle to see if it came back
            if (streak % 5 !== 0) {
              await sleep(50); // tiny yield so we don't hog the event loop
              continue;
            }
          }
          ...
          // If we got here without timeout, reset the no-price streak
          noPriceStreak[token.symbol] = 0;
        } catch (e) {
          const msg = e.message || "";
          if (msg.includes("processToken timeout")) {
            // Increment streak — only for tokens without positions
            if (!token.entryPrice) {
              noPriceStreak[token.symbol] = (noPriceStreak[token.symbol] || 0) + 1;
              if ((noPriceStreak[token.symbol]) === NO_PRICE_SKIP_AFTER) {
                console.log(`⏭️  ${token.symbol}: ${NO_PRICE_SKIP_AFTER} timeouts in a row — skipping until price returns`);
              }
            }
          }
          console.log(`⚠️ ${token.symbol}: ${msg}`);
        }
