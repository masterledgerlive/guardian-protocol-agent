cat > /home/claude/agent_new.js << 'ENDOFFILE'
...
async function checkTelegramCommands(cdp, ethBalance) {
  try {
    ...
    for (const update of data.result) {
      ...
      if (text.startsWith("/buy ")) {
        ...
        if (!tokens.find(t => t.symbol === sym)) { await sendAlert(`❓ Unknown token: ${sym}\nTry: DEGEN TOSHI BRETT AERO VIRTUAL AIXBT CLANKER SEAM`); continue; }
        ...
        const sym = raw.split(" ")[1]?.toUpperCase();
        if (!tokens.find(t => t.symbol === sym)) { await sendAlert(`❓ Unknown token: ${sym}`); continue; }
        manualCommands.push({ symbol: sym, action: "sell" });
        await sendAlert(`📱 <b>SELL ${sym} queued</b> — fires within 30s (will cascade proceeds)`);

      } else if (text.startsWith("/sellhalf ")) {
        const sym = raw.split(" ")[1]?.toUpperCase();
        if (!tokens.find(t => t.symbol === sym)) { await sendAlert(`❓ Unknown token: ${sym}`); continue; }
        manualCommands.push({ symbol: sym, action: "sellhalf" });
        await sendAlert(`📱 <b>SELL HALF ${sym} queued</b> — fires within 30s`);
        ...
        const weth = await getWethBalance();
        await sendAlert(
          `💰 <b>GUARDIAN BALANCES</b>\n\n` +
          `ETH:      ${ethBalance.toFixed(6)} (~$${(ethBalance * ETH_USD).toFixed(2)})\n` +
          `WETH:     ${weth.toFixed(6)} (~$${(weth * ETH_USD).toFixed(2)}) ← bot buys with this\n` +
          `Combined: ${(ethBalance + weth).toFixed(6)} ETH (~$${((ethBalance + weth) * ETH_USD).toFixed(2)})\n` +
        ...
        for (const t of tokens) {
          ...
          const sellT = hl ? hl.low + hl.range * SELL_ZONE : null;
          const buyT  = hl ? hl.low + hl.range * BUY_ZONE  : null;
          msg += `${zone} <b>${t.symbol}</b> $${p.toFixed(8)} | Cycle: ${cp !== null ? cp.toFixed(0) : "?"}%\n`;
          if (hl) {
        ...
        await sendAlert(
          ...
          `Total ever skimmed: ${totalSkimmed.toFixed(6)} ETH\n` +
          `[${bar}] ${pct.toFixed(1)}% to 0.5 ETH goal\n\n` +
          `1% from every profitable sell\n` +
          `Never touched — compounds forever ♾️`
        );
        ...
        let msg = `🌊 <b>CASCADE ORDER</b>\n🕐 ${new Date().toLocaleTimeString()}\n\n`;
        msg += `(Bottom → Top = next cascade target first)\n\n`;
        const sorted = [...tokens]
        ...
        for (const t of sorted) {
          ...
          msg += `${zone} ${pos} <b>${t.symbol}</b> ${t.cycle.toFixed(0)}% — $${t.price.toFixed(8)}\n`;
        }
        msg += `\n📍 = has open position`;
        await sendAlert(msg);
        ...
        await sendAlert(
          ...
          `<b>📊 INFO</b>\n` +
          `/status   — full snapshot (all tokens + P&L)\n` +
          `/eth      — ETH + WETH balances\n` +
          `/prices   — live prices + % to buy/sell zone\n` +
          `/positions — open positions + sell-now value\n` +
          `/cascade  — cascade order (bottom → top)\n` +
          `/piggy    — piggy bank progress\n` +
          `/trades   — trade count + profit stats\n\n` +
          `<b>🛒 MANUAL TRADING</b>\n` +
          `/buy SYMBOL      — force buy now\n` +
          `/sell SYMBOL     — force sell + cascade\n` +
          `/sellhalf SYMBOL — sell half position\n\n` +
          `Examples:\n` +
          `/buy AERO\n` +
          `/sell VIRTUAL\n` +
          `/sellhalf BRETT\n\n` +
  ...
  } catch (e) { /* silent — never crash telegram */ }
}
...
ENDOFFILE
echo "Written: $(wc -l < /home/claude/agent_new.js) lines"
