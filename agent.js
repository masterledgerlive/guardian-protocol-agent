...
// ═══════════════════════════════════════════════════════════════
// GUARDIAN CASCADE SYSTEM v7.4 — SELL DASHBOARD
//
// HOW IT WORKS:
...
async function processToken(cdp, token, ethBalance) {
  ...
  if (hl) {
    const sellTrigger  = hl.high * SELL_ZONE;
    const buyTrigger   = hl.low  * (1 + BUY_ZONE);
    const pctToSell    = ((sellTrigger - price) / price * 100);
    const pctToBuy     = ((price - buyTrigger) / buyTrigger * 100);

    if (inSellZone || isBreakout) {
      sellStatus = `🔴 SELL TRIGGER NOW! ($${sellTrigger.toFixed(8)})`;
    } else if (pctToSell > 0) {
      sellStatus = `📈 Need +${pctToSell.toFixed(2)}% to sell trigger ($${sellTrigger.toFixed(8)})`;
    } else {
      sellStatus = `✅ Above sell trigger`;
    }

    if (inBuyZone) {
      buyStatus = `🟢 BUY TRIGGER NOW! ($${buyTrigger.toFixed(8)})`;
    } else {
      buyStatus = `📉 Buy trigger at $${buyTrigger.toFixed(8)} (${pctToBuy.toFixed(2)}% away)`;
    }
  } else {
...
async function sendReport(ethBalance) {
  ...
  await sendAlert(
    `📊 <b>GUARDIAN CASCADE v7.4 — 30 MIN REPORT</b>\n\n` +
...
async function main() {
  console.log("═══════════════════════════════════════════════════");
  console.log("⚔️   GUARDIAN CASCADE SYSTEM v7.4 — SELL DASHBOARD");
  console.log("═══════════════════════════════════════════════════\n");
  ...
  await sendAlert(
    `⚔️ <b>GUARDIAN CASCADE v7.4 — STARTED</b>\n\n` +
