...
const MIN_POS_USD       = 3.00;    // minimum position size — below this gas eats the profit

// ── SMART SLOT SYSTEM ────────────────────────────────────────────────────────
// Dynamically controls how many active positions run at once based on total
// wallet size. Prevents capital from being spread so thin that gas kills profit.
//
// SLOT SCHEDULE (total portfolio USD):
//   < $20   → 2 slots  (concentrate everything, survive)
//   $20-50  → 3 slots  (still tight, each slot ~$6-$16)
//   $50-100 → 4 slots  (healthy, each slot ~$12-$25)
//   $100-250→ 5 slots  (growing, each slot ~$20-$50)
//   $250+   → 6 slots  (cruising, each slot $40+)
//
// HURDLE RULE: when adding a new slot, existing positions must EACH be worth
// more than they were when we only had fewer slots. This means we only spread
// when the wallet is big enough that each slot stays above its prior weight.
// Example: at $50 with 3 slots (~$16/slot), we only open a 4th slot when the
// new per-slot weight ($12.50) still clears MIN_POS_USD by a safe margin.
const MAX_ACTIVE_SLOTS_TABLE = [
  { minUsd:   0, maxSlots: 2 },   // survival mode
  { minUsd:  20, maxSlots: 3 },   // getting started
  { minUsd:  50, maxSlots: 4 },   // building
  { minUsd: 100, maxSlots: 5 },   // growing
  { minUsd: 250, maxSlots: 6 },   // cruising
];

function getMaxActiveSlots(totalUsd) {
  let slots = 2;
  for (const tier of MAX_ACTIVE_SLOTS_TABLE) {
    if (totalUsd >= tier.minUsd) slots = tier.maxSlots;
  }
  return slots;
}

// How much ETH to deploy per new buy — derived from slot count and wallet size.
// Each slot gets an even share of tradeable capital, capped at MAX_BUY_PCT.
// This ensures every buy is meaningful ($3+) and grows proportionally.
function calcSlotSize(totalAvailEth, ethUsd, maxSlots) {
  const totalUsd  = totalAvailEth * ethUsd;
  const perSlotUsd = totalUsd / maxSlots;
  // Safety floor: never deploy less than MIN_POS_USD per slot
  if (perSlotUsd < MIN_POS_USD) return 0; // wallet too thin — don't buy
  return Math.min(totalAvailEth / maxSlots, totalAvailEth * MAX_BUY_PCT);
}
...
async function executeBuy(cdp, token, bal, reason, price, forcedEth = 0, isCascade = false) {
  try {
    ...
    // ── SMART SLOT SIZING ────────────────────────────────────────────────────
    // Count how many positions are currently open
    const openPositions = tokens.filter(t => t.entryPrice != null).length;
    const maxSlots      = getMaxActiveSlots(totalAvail * ethUsd);

    // If we're already at max slots, don't open more (unless this is a cascade
    // replacing a position that just sold — cascade gets a pass)
    if (!isCascade && openPositions >= maxSlots) {
      console.log(`   🛑 ${token.symbol}: max slots reached (${openPositions}/${maxSlots}) — waiting for a position to close`);
      return false;
    }

    // Slot size: even share of tradeable capital, but never below MIN_POS_USD
    const slotEth   = calcSlotSize(totalAvail, ethUsd, maxSlots);
    if (slotEth <= 0) {
      console.log(`   🛑 ${token.symbol}: wallet too thin for a meaningful slot ($${(totalAvail*ethUsd).toFixed(2)} / ${maxSlots} slots < $${MIN_POS_USD})`);
      return false;
    }

    const maxSpend  = Math.min(totalAvail * maxPct, slotEth * 1.5); // allow up to 1.5x slot for PRIORITY
    const minSpend  = MIN_POS_USD / ethUsd;
    const ethToSpend= forcedEth > 0
      ? Math.min(forcedEth, maxSpend)
      : Math.min(Math.max(minSpend, slotEth), maxSpend);
    ...
    console.log(`      📦 Slot ${openPositions+1}/${maxSlots} | Slot size: ${slotEth.toFixed(6)} ETH ($${(slotEth*ethUsd).toFixed(2)})`);
    ...
    await tg(
      ...
      `📦 Slot ${openPositions+1}/${maxSlots} | deploying $${(ethToSpend*ethUsd).toFixed(2)}\n` +
      `💓 ${ind.detail}\n` +
      `━━━━━━━━━━━━━━━━━━━━\n` +
    ...
    for (const upd of data.result) {
      ...
      try {
        ...
        await tg(
          ...
          `<b>Active tokens (27):</b> AERO BRETT VIRTUAL MORPHO CBBTC DEGEN WELL SEAM AIXBT TOSHI KITE KEYCAT DOGINME XCN SKI PRIME LUNA HIGHER MOCHI ZORA BNKR TYBG MIGGLES BENJI ROOST TALENT`
        );
      }
      } catch (cmdErr) {
