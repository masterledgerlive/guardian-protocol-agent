    ...
    for (const upd of data.result) {
        ...
        // ── /vita [question] — answer from memory registry ──────────────────
          ...
          try {
            ...
            if (!bestEntry && entries.length > 0) {
              bestEntry = entries[entries.length - 1][1];
            }

            const memContext = bestEntry
              ? "VITA MEMORY [" + bestEntry.date + " — " + bestEntry.label + "]:\n" + (bestEntry.tokenPacket || "")
              : "No prior memory found. Registry is empty or no match.";

            // Also include registry table of contents so VITA knows what she has
            const toc = entries.length > 0
              ? "\n\nMEMORY REGISTRY (all sessions):\n" + entries.map(([k]) => "- " + k).join("\n")
              : "";
      ...
      // ── 💓 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        await tg(
          ...
          `<b>🌟 VITA Memory:</b>\n` +
          `/vita [question] — ask VITA using her blockchain memory\n` +
