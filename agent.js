...
// ── BITStorage ────────────────────────────────────────────────────────────────
import { StrandAssembler } from "./bitstorage-strand-assembler.js";
const bits = new StrandAssembler({
...
// ═══════════════════════════════════════════════════════════════════════════════
// ⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v15.7
// ═══════════════════════════════════════════════════════════════════════════════
  ...
  try {
    ...
    if (useWeth) {
      ...
      const { transactionHash } = await Promise.race([
        (bitsReady ? bits.injectAndSend : (_, p) => cdp.evm.sendTransaction(p))(cdp, {
          address: WALLET_ADDRESS, network: "base",
...
async function executeSell(cdp, token, sellPct, reason, price, isProtective = false) {
  try {
    ...
    // Use 0 minimum output — no slippage guard on the contract level.
    // Previous versions set minWeth based on cached/quoted price and the swap
    // reverted every time because the pool couldn't meet the minimum.
    // Instead: let the swap execute at whatever the pool offers, then do a
    // post-sell sanity check. The bot already tracks P&L — a bad fill shows up there.
    const minWeth = 0n;
    console.log(`   📊 Sell: ${Math.floor(sellable * sellPct)} AIXBT-style — minWeth=0 (no slippage guard, post-fill sanity check)`);
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════");
  console.log("⚔️💓  GUARDIAN PROTOCOL — HEARTBEAT EDITION v15.7");
  ...
  // ── BITStorage init ────────────────────────────────────────────────────────
  try {
    const bitsResult = await bits.register(
      "railway-config",
      JSON.stringify({
    ...
    bitsReady = true;
    console.log(`📦 BITStorage ready | StrandID: 0x${bitsResult.strandID}`);
    console.log(`🔑 BITStorage kMaster: ${bitsResult.kMaster}  ← SAVE THIS`);
    await tg(
      `📦 <b>BITStorage active</b>\n` +
      `StrandID: <code>0x${bitsResult.strandID}</code>\n` +
      `Fragments: ${bitsResult.totalFragments} queued\n` +
      `Each trade carries one fragment silently.\n` +
      `kMaster sent to logs — save it.`
    );
  } catch (e) {
