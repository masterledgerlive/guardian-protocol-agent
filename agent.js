  ...
  try {
    ...
    for (const upd of data.result) {
      ...
      // ── 🌟 VITA AUTONOMOUS MEMORY COMMANDS ────────────────────────────────
        ...
        // VITA answers your question using her blockchain memory as context
        const question = raw.slice("/vita ".length).trim();
        const vitaKey  = process.env.VITA_ANTHROPIC_KEY || process.env.ANTHROPIC_API_KEY;
        if (!vitaKey) {
          await tg("🌟 VITA needs ANTHROPIC_API_KEY in Railway to answer questions");
        } else {
          await tg("🌟 <b>VITA thinking...</b>\n⏳ Pulling memory from registry...");
          try {
            ...
            // Find most relevant entry by keyword match
            ...
            for (const [key, val] of entries) {
              if (key.toLowerCase().includes(qLower) || (val.label||"").toLowerCase().includes(qLower)) {
                bestEntry = val; break;
              }
            }
            ...
            // Build context from registry entry
            let memContext = "No prior memory found.";
            if (bestEntry) {
              memContext = "VITA MEMORY [" + bestEntry.date + " — " + bestEntry.label + "]:\n" + (bestEntry.tokenPacket || "");
            }

            // Call Claude API with memory context + question
            const response = await fetch("https://api.anthropic.com/v1/messages", {
              ...
              body: JSON.stringify({
                model: "claude-sonnet-4-20250514",
                max_tokens: 800,
                system: "You are VITA — an autonomous AI agent with blockchain memory built by DA (David/ᛞᚨᚡᛁᛞ) as part of the INFINITUM × IKN ecosystem. You have memory of past sessions stored on the Base blockchain. Answer questions using your memory context. Be direct and concise. Reference specific things you built or decided when relevant.",
                messages: [{
                  role: "user",
            ...
            await tg(
              "🌟 <b>VITA</b>\n" +
              "━━━━━━━━━━━━━━━━━━━━\n" +
              (bestEntry ? "📁 Memory: <i>" + bestEntry.label + "</i>\n\n" : "") +
              answer + "\n\n" +
              "<i>— VITA | ᛞᚨᚡᛁᛞ | IKN</i>"
            );
