...
//
// WHAT'S NEW IN v9.1:
//  ✅ Never crashes — all errors caught, bot keeps running
//  ✅ $3 forced initial entry per token
//  ✅ CDP client recreated on key error — auto recovery
//  ✅ All previous v9.0 features intact
// ═══════════════════════════════════════════════════════════════════════
...
async function executeBuy(cdp, token, tradeableEth, reason, price, hl) {
  try {
    ...
    ethToSpend         = Math.max(ethToSpend, minEntryEth); // enforce $3 min
    ...
    // Recreate CDP client on key errors
    if (e.message?.includes("key") || e.message?.includes("auth") || e.message?.includes("Invalid")) {
      console.log(`   🔄 Recreating CDP client...`);
      try { cdpClient = createCdpClient(); } catch {}
    }
  }
}
...
async function processToken(cdp, token, ethBalance) {
  try {
    ...
    const zoneStatus = !hl ? "⏳ Learning..." : isBreakout ? "🚨 BREAKOUT!" : inSellZone ? `🔴 SELL ${cycleNum.toFixed(0)}%`
      : inBuyZone ? `🟢 BUY ${cycleNum.toFixed(0)}%${inDeepBuy ? " 🔥" : ""}` : `⬜ WATCHING ${cycleNum.toFixed(0)}%`;
    ...
    if (sellable > 1) {
      ...
      if (entry && token.entryTime && price < entry * (1 - STOP_LOSS_PCT) && (Date.now() - token.entryTime) > STOP_LOSS_MS) {
        await executeSell(cdp, token, SELL_ALL_PCT, `STOP LOSS — ${((price-entry)/entry*100).toFixed(1)}%`, price, hl);
        return;
      }
      // Capital protect
      if (entry && !inSellZone && !isBreakout && price >= entry * (1 + ENTRY_CUSHION)) {
        await executeSell(cdp, token, SELL_ALL_PCT, "CAPITAL PROTECT", price, hl);
        return;
      }
      // Breakout
      if (isBreakout) {
        delete sellZoneEntryTime[token.symbol];
        await executeSell(cdp, token, SELL_ALL_PCT * 0.5, `BREAKOUT +${(((price/hl.high)-1)*100).toFixed(1)}%`, price, hl);
      } else if (inSellZone) {
        ...
        if (rising && ticks <= TRAILING_TICKS) {
          ...
          delete sellZoneEntryTime[token.symbol];
          await executeSell(cdp, token, SELL_ALL_PCT, `AT HIGH ZONE — ${rising ? "max trailing" : "fading"}`, price, hl);
        }
      } else {
      ...
      if (inBuyZone) {
        ...
        // $3 forced initial entry — no cycle data needed, just buy if no position
        if (!token.entryPrice && token.status === "active") {
          const minEntryEth = MIN_ENTRY_USD / ETH_USD;
          if (totalAvail >= minEntryEth) {
            console.log(`  💵 [${token.symbol}] $3 FORCED ENTRY`);
            await executeBuy(cdp, token, tradeableEth, `$${MIN_ENTRY_USD} FORCED ENTRY — building position`, price, hl);
          }
        }
      }
    }
  ...
  try {
    ...
    for (const update of data.result) {
      ...
      if (text.startsWith("/buy ")) {
        ...
        const weth = await getWethBalance();
        let msg = `⚔️ <b>GUARDIAN v9.1 STATUS</b>\n\n`;
        msg += `💰 ETH: ${ethBalance.toFixed(6)}\n💎 WETH: ${weth.toFixed(6)}\n`;
        ...
        for (const t of tokens) {
          ...
          msg += `${zone} <b>${t.symbol}</b> $${p.toFixed(8)} ${hl?cp.toFixed(0):"?"}%${pnl?` P&L:${parseFloat(pnl)>=0?"+":""}${pnl}%`:""}\n`;
        }
        await sendAlert(msg);
      } else if (text === "/help") {
        await sendAlert(
          `⚔️ <b>GUARDIAN v9.1 COMMANDS</b>\n\n` +
...
async function main() {
  console.log("═══════════════════════════════════════════════════════");
  console.log("⚔️   GUARDIAN CASCADE SYSTEM v9.1 — CRASH PROOF");
  console.log("═══════════════════════════════════════════════════════\n");
  ...
  await sendAlert(
    `⚔️ <b>GUARDIAN CASCADE v9.1 — STARTED</b>\n\n` +
    `💰 ETH: ${ethBalance.toFixed(6)} (~$${(ethBalance*ETH_USD).toFixed(2)})\n` +
    `💎 WETH: ${wethBal.toFixed(6)} (~$${(wethBal*ETH_USD).toFixed(2)})\n` +
    `🪙 Tracking: ${tokens.map(t=>t.symbol).join(", ")}\n\n` +
    `💵 $3 forced entry per token enabled\n` +
    `🛡️ Crash-proof — never stops running\n\n` +
    `Send /help for commands`
  );

  let cachedEth = ethBalance;

  // Telegram loop — runs independently
...
main().catch(e => {
  ...
  // Even on fatal error, try to restart after 30s
