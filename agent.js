...
async function btpInscribe(cdp, tradeLabel) {
  try {
    ...
    if (isStrand && !isFinal) {
      // Individual trade chunk receipt — shows chunk position in strand
      ...
      // Parse key fields from chunk
      const symMatch     = full.match(/SYM:([^|]+)/);
      const entryMatch   = full.match(/ENTRY:\$([^|]+)/);
      const exitMatch    = full.match(/EXIT:\$([^|]+)/);
      const pnlMatch     = full.match(/PNL:([^|]+)/);
      const inMatch      = full.match(/IN:([^|]+)/);
      const outMatch     = full.match(/OUT:([^|]+)/);
      const targetMatch  = full.match(/TARGET:\$([^|]+)/);
      const marginMatch  = full.match(/MARGIN:([^|]+)/);

      let receiptLines = [
        emoji + " <b>STRAND " + sNum + " — CHUNK " + cPos + "/" + cTot + "</b>",
        "━━━━━━━━━━━━━━━━━━━━",
      ];
      ...
      if (isBuy) {
        ...
        const isWin = pnlMatch?.[1]?.startsWith("+");
        receiptLines.push(
          "📊 <b>" + (symMatch?.[1]||"?") + "</b> SELL " + (isWin ? "✅" : "❌"),
          "💰 Exit: $" + (exitMatch?.[1]?.split("|")[0]||"?"),
          "📤 Out: " + (outMatch?.[1]||"?"),
          (isWin ? "🟢" : "🔴") + " P&L: <b>" + (pnlMatch?.[1]||"?") + "</b>",
        );
      }

      receiptLines.push(
        "🔗 <a href=\"https://basescan.org/tx/" + transactionHash + "\">Block " + cPos + "/" + cTot + " on Base ↗</a>",
        "<i>📡 " + (parseInt(cTot) - parseInt(cPos)) + " more chunk(s) until strand complete</i>"
      );

      await tg(receiptLines.join("\n"));

    } else if (isFinal) {
      // FINAL 5/5 receipt — full strand math summary
      const strandMatch  = name.match(/STRAND-(\d+)/);
      const sNum         = strandMatch?.[1] || "?";
      const totalInM     = full.match(/TOTAL_IN:\$([^|]+)/);
      const totalOutM    = full.match(/TOTAL_OUT:\$([^|]+)/);
      const grossM       = full.match(/GROSS_PNL:([^|]+)/);
      const gasM         = full.match(/GAS_COST:([^|]+)/);
      const netM         = full.match(/NET_PNL:([^|]+)/);
      const winsM        = full.match(/WINS:(\d+)/);
      const lossesM      = full.match(/LOSSES:(\d+)/);
      const verdictM     = full.match(/VERDICT:([^|]+)/);
      const symsM        = full.match(/SYMBOLS:([^|]+)/);
      const isProfit     = (verdictM?.[1]||"").includes("PROFITABLE");

      await tg(
        (isProfit ? "🏆" : "📉") + " <b>STRAND " + sNum + " COMPLETE — FINAL RECEIPT</b>\n" +
        "━━━━━━━━━━━━━━━━━━━━\n" +
        "📊 Tokens: " + (symsM?.[1]||"?") + "\n" +
        "💰 Total invested: $" + (totalInM?.[1]||"?") + "\n" +
        "📤 Total received: $" + (totalOutM?.[1]||"?") + "\n" +
        "📈 Gross P&L: <b>" + (grossM?.[1]||"?") + "</b>\n" +
        "⛽ Gas costs: " + (gasM?.[1]||"?") + "\n" +
        "🏁 Net P&L: <b>" + (netM?.[1]||"?") + "</b>\n" +
        "✅ Wins: " + (winsM?.[1]||"0") + " | ❌ Losses: " + (lossesM?.[1]||"0") + "\n" +
        "📋 Verdict: <b>" + (verdictM?.[1]||"?") + "</b>\n" +
        "━━━━━━━━━━━━━━━━━━━━\n" +
        "🔗 <a href=\"https://basescan.org/tx/" + transactionHash + "\">Final receipt on Base ↗</a>\n" +
        "<i>All 5 chunks form a complete provable record on Base blockchain</i>"
      );

    } else if (isDefault) {
      // VITA auto-fill — quiet receipt
      await tg(
