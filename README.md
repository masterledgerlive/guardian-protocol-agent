...
LOT_REBUILD_TXS           ← optional `AERO:0x…,DRB:0x…,BNKR:0x…` extra buy hashes for FIFO rebuild after restart. Evidence fills (AERO `0x94faa542…` / DRB `0xe0f846a8…` / BNKR `0xeef39d62…`) are already seeded. Amounts come from Transfer+WETH receipts — never invented. GitHub Contents uses `Authorization: token ${GITHUB_TOKEN}` (same as githubGet / IKN). HTTP 401 on ledger.json / fifo-lots.json does not skip the seeded rebuild.
