...
LOT_REBUILD_TXS           ← optional `AERO:0x…,DRB:0x…,BNKR:0x…,VIRTUAL:0x…,CLANKER:0x23d8…,CLANKER:0xcb7d…` extra buy hashes for FIFO rebuild after restart. Evidence fills are already seeded (AERO `0x94faa542…` / DRB `0xe0f846a8…` + trough `0x53a00788…` / BNKR `0xeef39d62…` / VIRTUAL `0x33aac652…` / CLANKER array `0x23d8a0c5…`+`0xcb7dd5a6…` + add-on `0xcb7dd5a6…`). Live env-only two hashes cannot merge on a usable first-slice lot (boot latched VIRTUAL, CLANKER stayed unknown) unless `rebuildHashes` / addon / sibling — the durable path is the code seed. Desk book / ledger fills alone do not latch Railway (`fifo-lots.json` is not on bot-state). Amounts come from Transfer+WETH receipts — never invented. GitHub 401 on ledger/fifo-lots/positions still runs this on-chain path (live `token` + `STATE_BRANCH=bot-state`; no Bearer). VIRTUAL sealed sell `0x88105ec1…` auto-appends from persist/ledger receipt rebuild (do not hardcode-invent in `EVIDENCE_SELL_TXS`).
