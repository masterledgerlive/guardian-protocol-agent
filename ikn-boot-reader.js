...
async function bootReader() {
  ...
  // IKN strands live under registry._ikn.strands (namespaced to avoid breaking VITA)
  const iknStrands = registry._ikn?.strands || [];

  // Also surface any flat legacy ikn-strand entries for context
  const legacyIKN  = Object.values(registry)
    .filter(v => v && typeof v === 'object' && v.type === 'ikn-strand')
    .slice(0, 3);

  const allCandidates = [...iknStrands, ...legacyIKN];
  const strandsArr    = allCandidates.filter(Boolean);
