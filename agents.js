...
dotenv.config();
// ═══════════════════════════════════════════════════════════════════════
GUARDIAN CASCADE SYSTEM v9.0 — MASTER EDITION
// //
// WHAT'S NEW IN v9.0:
...
const STOP_LOSS_MS = 7200000; // 2 hours before stop loss kicks in
// ── SAFETY ───────────────────────────────────────────────────────────────────
const GAS_RESERVE = 0.0003; // always locked for gas
const SELL_RESERVE = 0.001; // always kept for sell gas
const MAX_BUY_PCT = 0.15; // max 15% of available per buy
const MIN_ETH_TRADE = 0.0008; // minimum ETH/WETH to attempt a buy
const MIN_TRADE_USD = 0.05; // trade must be worth at least $0.05
const FEE_SAFETY = 5; // trade value must be 5x gas cost
const MAX_GAS_ETH = 0.002; // skip trade if gas spike above this
const ETH_USD = 1940; // approx ETH/USD
// ── TRADING ──────────────────────────────────────────────────────────────────
const BUY_ZONE = 0.15; // buy bottom 15% of cycle
const SELL_ZONE = 0.80; // sell top 20% of cycle
const DEEP_BUY_ZONE = 0.07; // deep zone — spend 50% more
const PROFIT_TARGET = 500; // $500 target per cycle
const HISTORY_DAYS = 3; // days of price history used
const BUY_CONFIRM_MS = 120000; // 2 min confirm before buying
const LOTTERY_PCT = 0.02; // 2% kept forever as lottery
const SELL_ALL_PCT = 0.98; // sell 98%, keep 2% lottery
const ENTRY_CUSHION = 0.01; // capital protect 1% above entry
const STOP_LOSS_PCT = 0.08; // stop loss -8% after 2hrs
const TRAILING_TICKS = 3; // trailing sell: wait up to 3 cycles at peak
// ── GITHUB ───────────────────────────────────────────────────────────────────
...
function getClient() {
return createPublicClient({ chain: base, transport: http(RPC_URLS[rpcIndex % RPC_URLS.lengt
}
...
if (e.message?.includes("429") || e.message?.includes("rate limit") || e.message?.inclu
...
{ symbol: "DEGEN", address: "0x4ed4e862860bed51a9570b96d89af5e1b0efefed", status: "active
{ symbol: "TOSHI", address: "0xAC1Bd2486aAf3B5C0fc3Fd868558b082a531B2B4", status: "active
{ symbol: "BRETT", address: "0x532f27101965dd16442E59d40670FaF5eBB142E4", status: "active
{ symbol: "AERO", address: "0x940181a94A35A4569E4529A3CDfB74e38FD98631", status: "active
{ symbol: "VIRTUAL", address: "0x0b3e328455c4059EEb9e3f84b5543F74E24e7E1b", status: "active
{ symbol: "AIXBT", address: "0x4F9Fd6Be4a90f2620860d680c0d4d5Fb53d1A825", status: "active
{ symbol: "CLANKER", address: "0x1bc0c42215582d5a085795f4badbac3ff36d1bcb", status: "active
{ symbol: "SEAM", address: "0x1C7a460413dD4e964f96D8dFC56E7223cE88CD85", status: "active
];
// ── STATE ────────────────────────────────────────────────────────────────────
let tokens = [];
let history = {};
...
let approvedTokens = new Set(); // approval cache — saves gas
...
`https://api.github.com/repos/${GITHUB_REPO}/contents/${path}?ref=${GITHUB_BRANCH}&t=${
{ headers: { Authorization: `token ${GITHUB_TOKEN}`, Accept: "application/vnd.github.v3
);
...
tokens tokensSha = tf.sha;
= DEFAULT_TOKENS.map(def => ({ ...def, ...(saved.find(s => s.symbol === def.sym
} else {
...
console.log(` console.log(` Positions restored: ${restored || "none"}`);
...
console.log(" Saving...");
tokensSha = await githubSave("tokens.json", { tokens, lastSaved: new Date().toISOString
...
try {
const res = await fetch(`https://api.geckoterminal.com/api/v2/simple/networks/base/token
const data = await res.json();
...
return { direction: avg > 0.0002 ? "up" : avg < -0.0002 ? "down" : "neutral", speed: Math.a
}
...
Gas eats profit — $${tradeValue.toFixed(3)} vs gas $${gasCostUsd.toFix
}
return true;
}
// ── ENCODE ───────────────────────────────────────────────────────────────────
function encodeSwap(tokenIn, tokenOut, amountIn, recipient, fee = 3000) {
const p = (v, isAddr = false) => (isAddr ? v.slice(2) : BigInt(v).toString(16)).padStart(64
return "0x04e45aaf" + p(tokenIn, true) + p(tokenOut, true) + p(fee)
+ p(recipient, true) + p(amountIn) + p(0) + p(0);
}
function encodeApprove(spender, amount) {
return "0x095ea7b3" + spender.slice(2).padStart(64, "0") + amount.toString(16).padStart(64,
}
function getFeeTier(tokenAddress) {
// CLANKER only has 1% pool — everything else is 0.3%
if (tokenAddress.toLowerCase() === "0x1bc0c42215582d5a085795f4badbac3ff36d1bcb") return 100
return 3000;
}
// ── APPROVAL CACHE — approve once, never again ───────────────────────────────
async function ensureApproved(cdp, tokenAddress, amountIn) {
const MAX_UINT256 = BigInt("0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
...
console.log(` ${ethToSpend.toFixed(6)} ${useWeth ? "WETH" : "ETH"} @ $${price.toFixed(
...
transaction: {
to: SWAP_ROUTER,
data: encodeSwap(WETH_ADDRESS, token.address, amountIn, WALLET_ADDRESS, getFeeTier(
},
});
...
` Spent: ${ethToSpend.toFixed(6)} ${useWeth ? "WETH" : "ETH"} (~$${(ethToSpend ` Entry: $${price.toFixed(8)}\n` +
...
await sendAlert(` <b>${token.symbol} BUY FAILED</b>\n${e.message}`);
}
}
// ── SELL ─────────────────────────────────────────────────────────────────────
...
const isProtect = reason.includes("CAPITAL PROTECT") || reason.includes("STOP LOSS") if (token.entryPrice && price < token.entryPrice && !isProtect) {
...
console.log(` ${(sellPct * 100).toFixed(0)}% of ${sellable.toFixed(0)} tokens @ console.log(` Keeping ${lotteryHold} ${token.symbol} forever`);
try {
...
|| rea
$${pri
data: encodeSwap(token.address, WETH_ADDRESS, amountToSell, WALLET_ADDRESS, getFeeTie
},
});
lastTradeTime[token.symbol] = Date.now();
tradeCount++;
...
await sendAlert(` <b>${token.symbol} SELL FAILED</b>\n${e.message}`);
}
}
// ── PROCESS ONE TOKEN ────────────────────────────────────────────────────────
...
const inBuyZone const inDeepBuy const inSellZone const isBreakout = hl && cycleNum <= BUY_ZONE * 100;
= hl && cycleNum <= DEEP_BUY_ZONE * 100;
= hl && cycleNum >= SELL_ZONE * 100;
= hl && price >= hl.high * 1.03;
// Log targets every cycle
if (hl) {
const buyT = hl.low + hl.range * BUY_ZONE;
const sellT = hl.low + hl.range * SELL_ZONE;
if (inBuyZone) {
console.log(` [${token.symbol}] IN BUY ZONE${inDeepBuy ? " DEEP" : ""}! $${pr
} else {
const pctToBuy = ((price - buyT) / buyT * 100).toFixed(1);
const pctToSell = ((sellT - price) / price * 100).toFixed(1);
console.log(` [${token.symbol}] Buy: $${buyT.toFixed(8)} (${pctToBuy}% away) | Sell
}
} else {
console.log(` [${token.symbol}] Building data... $${price.toFixed(8)}`);
}
// Dashboard box
const zoneStatus = !hl ? " Learning..." : isBreakout ? " : inBuyZone ? ` BUY ${cyclePos}%${inDeepBuy ? " " : ""}` : ` BREAKOUT!" : inSellZone ? `
WATCHING ${cyclePos}%
const confirmInfo = buyConfirmations[token.symbol]
? ` ${Math.max(0, Math.ceil((BUY_CONFIRM_MS - (Date.now() - buyConfirmations[token.sym
console.log(`\n ┌─[${token.symbol}]──────────────────────────────────`);
console.log(` │ $${price.toFixed(8)} | ${zoneStatus}${confirmInfo} | ${momentum.directi
...
// ── SELL SIGNALS ───────────────────────────────────────────────────────────
if (sellable > 1) {
// Stop loss: -8% after 2 hours
if (entry && token.entryTime && price < entry * (1 - STOP_LOSS_PCT) && (Date.now() - toke
console.log(` STOP LOSS ${token.symbol} — -${(STOP_LOSS_PCT * 100)}% after 2hr
await executeSell(cdp, token, SELL_ALL_PCT, `STOP LOSS — ${((price - entry) / entry * 1
return;
}
// Capital protect: drifted 1% above entry, not in sell zone
if (entry && !inSellZone && !isBreakout && price >= entry * (1 + ENTRY_CUSHION)) {
console.log(` CAPITAL PROTECT ${token.symbol}`);
await executeSell(cdp, token, SELL_ALL_PCT, "CAPITAL PROTECT — 1% above entry", price,
return;
}
if (isBreakout) {
console.log(` BREAKOUT SELL ${token.symbol}`);
delete sellZoneEntryTime[token.symbol];
await executeSell(cdp, token, SELL_ALL_PCT * 0.5, `BREAKOUT +${(((price / hl.high) - 1)
} else if (inSellZone) {
if (!sellZoneEntryTime[token.symbol]) sellZoneEntryTime[token.symbol] = { firstMs: Date
...
console.log(` [${token.symbol}] Trailing sell — still rising (${ticks}/${TRAILING
} else {
}
delete sellZoneEntryTime[token.symbol];
...
[${token.symbol}] BUY ZONE — 2 min clock started`);
console.log(` await sendAlert(
` <b>${token.symbol} HIT BUY ZONE${inDeepBuy ? " ` $${price.toFixed(8)} | Cycle: ${cyclePos}%\n` +
...
buyConfirmations[token.symbol] = { firstSeenMs: Date.now(), priceAtFirst: price };
console.log(` [${token.symbol}] Still falling — clock reset at $${price.toFixe
} else if (elapsed < BUY_CONFIRM_MS) {
console.log(` [${token.symbol}] ${remaining}s to confirm | ${priceChg >= } else {
...
`LOW ZONE CONFIRMED${inDeepBuy ? " (DEEP )" : ""} — held ${(elapsed / 60000).to
}
}
} else {
...
// Initial entry: only if no history yet AND cycle < 50% (not buying near top)
if ((!hl || hl.readings < 15) && balance < 1 && token.status === "active" && cycleNum <
console.log(` BUYING ${token.symbol} — initial entry (${cycleNum.toFixed(0)}% cyc
await executeBuy(cdp, token, tradeableEth, "INITIAL ENTRY — building cycle data", pri
}
}
}
}
...
const zone = hl && cp >= SELL_ZONE * 100 ? " const entry = t.entryPrice || price;
const pct = ((price - entry) / entry * 100).toFixed(1);
" : hl && cp <= BUY_ZONE * 100 ? " const lottery = Math.floor(bal * LOTTERY_PCT);
lines += `\n${zone} <b>${t.symbol}</b> $${price.toFixed(8)} Cycle: ${hl ? cp.toFixed(0) :
lines += ` ${Math.floor(bal)} ($${(bal * price).toFixed(2)}) | P&L: ${parseFloat(pct) >
...
const cid = process.env.TELEGRAM_CHAT_ID;
if (!tok || !cid) return;
const res = await fetch(`https://api.telegram.org/bot${tok}/getUpdates?offset=${lastUpda
const data = await res.json();
...
if (!tokens.find(t => t.symbol === sym)) { await sendAlert(` manualCommands.push({ symbol: sym, action: "buy" });
await sendAlert(` <b>BUY queued:</b> ${sym} — fires next cycle (≤30s)`);
Unknown: ${sym}`); con
} else if (text.startsWith("/sell ") && !text.startsWith("/sellhalf")) {
const sym = raw.split(" ")[1]?.toUpperCase();
if (!tokens.find(t => t.symbol === sym)) { await sendAlert(` Unknown: ${sym}`); con
...
const zone = hl && cp >= SELL_ZONE * 100 ? " " : hl && cp <= BUY_ZONE * 100 const pnl = t.entryPrice ? ((p - t.entryPrice) / t.entryPrice * 100).toFixed(1) : n
msg += `${zone} <b>${t.symbol}</b> $${p.toFixed(8)} Cyc:${hl ? cp.toFixed(0) if (pnl) msg += ` P&L:${parseFloat(pnl) >= 0 ? "+" : ""}${pnl}%`;
msg += "\n";
? " "
: "?"}
}
...
const hl = getRecentHighLow(t.symbol);
const cp = hl ? getCyclePosition(t.symbol, p) : 50;
const zone = hl && cp >= SELL_ZONE * 100 ? " " : hl && cp <= BUY_ZONE * 100 msg += `${zone} <b>${t.symbol}</b> $${p.toFixed(8)} | ${hl ? cp.toFixed(0) : if (hl) msg += ` Buy: $${(hl.low + hl.range * BUY_ZONE).toFixed(8)} | Sell: $${(h
msg += "\n";
? " "
"?"}%\
}
...
msg += ` Now: $${p.toFixed(8)}\n`;
msg += `${parseFloat(pnl) >= 0 ? " " : " "} P&L: ${parseFloat(pnl) >= 0 ? "+" : "
...
`Profit skimmed: ${totalSkimmed.toFixed(6)} ETH ($${(totalSkimmed * ETH_USD).toFixe
` Piggy: ${piggyBank.toFixed(6)} ETH`
);
} else if (text === "/help" || text === "help") {
await sendAlert(
` <b>GUARDIAN v9.0 COMMANDS</b>\n\n` +
`<b>INFO</b>\n` +
...
`/trades — trade stats\n\n` +
`<b>MANUAL TRADING</b>\n` +
`/buy SYMBOL `/sell SYMBOL — force buy now\n` +
— force sell all now\n` +
...
async function main() {
console.log("═══════════════════════════════════════════════════════");
console.log(" GUARDIAN CASCADE SYSTEM v9.0 — MASTER EDITION");
console.log("═══════════════════════════════════════════════════════\n");
await loadFromGitHub();
// Railway stores PEM keys with literal \n — restore real newlines
const apiKeySecret = (
...
await sendAlert(
` <b>GUARDIAN CASCADE v9.0 — STARTED</b>\n\n` +
...
// Telegram polling — independent loop, 3 second response time
...
cachedEth = eth;
console.log(`\n${"═".repeat(58)}`);
console.log(`${new Date().toLocaleTimeString()} | ETH: ${eth.toFixed(6)} | WETH: ${weth
console.log(`Tradeable: ${getTradeableEth(eth).toFixed(6)} ETH | Piggy: ${piggyBank.toF
...
await sendAlert(` <b>GUARDIAN ERROR</b>\n${e.message}`);
}
